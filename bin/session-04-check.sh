#!/usr/bin/env bash
#
# The Session 4 gate. It does not replace bin/session-01-check.sh, which must
# still exit 0, nor the Session 2 and Session 3 gates, which still own their own
# sessions' verdicts.
#
# Three modes, and the third one's return is a decision (D82):
#
#   --mode offline    a checkout: contracts, schemas, models, allocation logic.
#   --mode host       the deployment host: two clusters, two poolers, four
#                     transports, the client fixtures, the restart matrix.
#   --mode external   a different network: what the world can reach, and what a
#                     developer's helper can reach through SSH.
#
# Session 3 dropped external mode because nothing new was visible from outside a
# cluster that published no port and joined no edge network. Session 4 restores
# it, and unlike Session 3's case it is not vacuous -- for a reason that changed
# under measurement. The plan restored it because Session 4 publishes host
# ports; ADR 0044 then established that nothing is published at all. The mode
# survives that intact, because what it proves was never "the publication is
# bound to loopback": it is that **no database transport is reachable from
# off-host**, which is now true structurally and goes red the day a publication
# is introduced. The external mode is also the only place `bin/connect.sh` and
# the access broker can be exercised as what they are -- a developer's programs,
# reaching the host over SSH.
#
# The claims this gate is answerable for are cumulative: Session 2's and Session
# 3's are proved here too, because the product did not stop making those
# promises when it grew a pooler (ADR 0039). Session 4 records eleven.
#
# **Session 4's evidence has two halves and cannot be written from one.**
# `transport_boundary` and `connection_tooling` are measured from off-host (ADR
# 0045), and the writer refuses a session document that is silent about a claim.
# Run both modes, then merge.
#
...
usage() {
  cat <<'USAGE'
Usage: bin/session-04-check.sh --mode offline
       sudo bin/session-04-check.sh --mode host --host FILE \
            --project-a-outputs FILE --project-b-outputs FILE \
            [--sentinel-file FILE] [--rotated-from-file FILE] \
            [--after-reboot] [-k EXPRESSION]
       bin/session-04-check.sh --mode external --public-ipv4 ADDR \
            --project-a-outputs FILE --ssh-destination USER@HOST \
            [--public-ipv6 ADDR] [--project-b-outputs FILE] [-k EXPRESSION]

  --mode offline   Contracts, schemas, models, allocation logic. No host.
  --mode host      Everything measurable on the deployment host, over two live
                   clusters and two poolers. Needs root: it reads root-only
                   state -- the port registry, the secret generations, systemd.
  --mode external  What the public internet can reach, and what bin/connect.sh
                   can reach through SSH. MUST run from a network that is not
...
This command verifies and never deploys. Use ./deploy.sh --through-session 4
...
mode_offline() {
  ...
  printf '\n\033[1msession-04-check: offline PASSED\033[0m\n'
}
...
mode_host() {
  ...
  if ! evidence_is_supportable; then
    announce_no_evidence
    printf '\n\033[1msession-04-check: host PASSED\033[0m\n'
    return 0
  fi
...
mode_external() {
  ...
  printf 'session-04-check: this mode is only meaningful from a network that is\n'
  ...
  if ! evidence_is_supportable; then
    announce_no_evidence
    printf '\n\033[1msession-04-check: external PASSED\033[0m\n'
    return 0
  fi
