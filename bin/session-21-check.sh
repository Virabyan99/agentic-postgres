#!/usr/bin/env bash
#
# The Session 21 gate: the agent plane opened to a tenant's domain (ADR 0200,
# ADR 0201). The scope vocabulary derived from the reviewed surface, the
# runtime's roster compiled from the lock, a project's own capability manifest
# joined into its lock, the scaffold that writes one, a project's evaluation
# report, and a disaster kit read at a later release. It does not replace
# bin/session-01-check.sh, which must still exit 0, nor the Session 2-20
# gates, which still own their sessions.
#
# **Derived from bin/session-20-check.sh by diff, not retyped** (D505, D507,
# D678, D693, D703, D1108, D1109). `readonly SESSION=21` is the only session
...
# what happened.
#
#   --mode offline    a checkout: everything Session 20 checked, plus Session
#                     21's own offline halves -- the derived vocabulary and the
#                     issuer's ceiling over two locks (test_scope_vocabulary,
#                     test_scope_registry), the roster registered from the
#                     lock by kind and shape (test_lock_roster, test_mcp_tools),
#                     a project's contract and the joint lock
#                     (test_project_agent_surface), the scaffold against the
#                     example project (test_agent_command), the project's
#                     report (test_evaluation_harness), outputs v18's migrator
#                     and the version-aware kit verifier (test_disaster_kit).
#                     Step 6 checks the example project's committed contract
#                     and report beside the release's. No host, no root.
#   --mode host       the deployment: beta's lock carrying the tenant write
#                     and its vocabulary the tenant scope, with ALPHA as the
#                     control that serves exactly the six and refuses the same
#                     grant; a reader on beta that can neither see nor call
#                     the tenant write; the write reached through the plane,
#                     refused pending approval and audited; the deployed lock
#                     recording the scaffold's contract; beta's document
#                     publishing the digest its own report carries. Needs
#                     root, host.yaml in the checkout, and the declarations for
#                     every claim that is about something that happened
#                     elsewhere.
#   --mode external   a different network: what a stranger reaches. Session 21
#                     adds no external claim of its own -- a lock, a manifest
#                     and an audit row are reachable from nowhere but the host
#                     -- and still needs this mode for the cumulative reason
#                     below.
#
# **Session 21's own claims are `host` and offline.** The gate still has three
# modes because `claims_through_session(21)` is CUMULATIVE: a Session 21 document
...
# (D1133, reversing D1021's reading). `claims_through_session(21)` runs Session
...
#
#   1. Both projects deployed `--through-session 21`, which publishes outputs
#      **v18** (`capabilities.project` rendered, `mcp.project_capabilities`
#      deployed). Session 21 adds NO migration.
...
#      does not" a comparison. Checked in step 4c; without it every `AGT-*`
#      live half fails on the same missing thing.
#   3. **The issuer answering on both projects.** A deploy through 21 changes
#      the lock's bytes, so `auth` and `mcp` are RECREATED on both (ADR 0155):
#      a rolling change to the issuer. Step 4c reads `/auth/login` on both
#      before anything else, because a gate that ran a suite against an issuer
#      that had not come back would report forty refusals about one restart.
...
#      for every `IDN-*`, Session 16 and Session 21 live proof: the ceiling at
#      agent creation is measured through the admin endpoint.
...
USAGE
  ...
  --mode offline   Contracts, schemas and models, plus Session 21's own offline
                   halves: the scope vocabulary derived from the reviewed
                   surface and the issuer's ceiling over two locks, the roster
                   registered from the lock by kind and shape, a project's own
                   contract and the joint lock, the scaffold against the
                   example project, the project's evaluation report, outputs
                   v18's migrator, and the kit verifier reading an older
                   document. Step 6 checks the example project's committed
                   contract and report. No host, no root.
  --mode host      Beta's lock carrying the tenant write and its vocabulary the
                   tenant scope, with ALPHA as the control that serves exactly
                   the six and refuses the same grant at issue and at creation;
                   a reader on beta that can neither see nor call the write;
                   the write reached through the plane, refused pending
                   approval and audited; the deployed lock recording the
                   scaffold's contract; beta's document publishing the digest
                   its own report carries. Needs root, host.yaml in the
                   checkout, and the declarations for every claim that is about
                   something that happened elsewhere.
  --mode external  What the public internet reaches. Session 21 adds no external
                   claim -- a lock, a capability manifest and an audit row are
                   reachable from nowhere but the host, and the one surface a
                   stranger CAN reach is the published OpenAPI, which
                   API-CONTRACT-001 already measures -- but the inherited ones
                   still need it, because claims_through_session(21) is
