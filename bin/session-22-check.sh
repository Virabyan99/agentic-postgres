#!/usr/bin/env bash
#
# The Session 22 gate: `apg dev`, a disposable local PostgreSQL cluster built
# from a project's rendered document and the release alone (ADR 0203), and the
# evidence model's third mode (ADR 0202). It does not replace
# bin/session-01-check.sh, which must still exit 0, nor the Session 2-21
# gates, which still own their sessions.
#
# **Derived from bin/session-21-check.sh by diff, not retyped** (D505, D507,
# D678, D693, D703, D1108, D1109). `readonly SESSION=22` is the only session
...
#
# **This gate is the first one whose own session closes in OFFLINE mode**, and
# that is the difference an operator should read first. Session 22 makes no
# host trip (D1163): four of its six claims are about a command a developer
# runs on their own machine, and no deployment can answer a question about
# one. Offline mode therefore WRITES A HALF here -- evidence/session-22-
# offline.json -- where every previous gate's offline mode wrote nothing.
#
...
# what happened.
#
#   --mode offline    a checkout, and for the first time a checkout that
#                     produces evidence: everything Session 21 checked, plus
#                     Session 22's own offline halves -- what `apg dev` tells
#                     docker and what the resulting cluster IS
#                     (test_dev_environment, test_dev_environment_cluster),
#                     the command's refusals (test_dev_command), the seed
#                     door, the declared offline mode and the writer's
#                     checkout_commit (test_evidence_claims), and the churn
#                     numbers with their machine and cache state
#                     (test_capacity_envelope). Step 8 runs the five verbs
#                     against project.example.yaml and step 9 writes the half.
#                     **Requires docker**, which no earlier offline mode did.
#                     No host, no root.
#   --mode host       the deployment: unchanged from Session 21 except that the
#                     sweep now also collects Session 22's two host claims --
#                     `plane_confirmed_count` (the document's tool count is the
#                     one the running plane confirmed through its own probe)
#                     and `agent_tenant_read` (an agent reads a tenant's rows
#                     through a tenant tool on beta). **No new flag**: both read
#                     declarations this gate already takes. Session 24's trip is
#                     where they are collected; until then they are `not_run`,
#                     which is the honest verdict and the reason the offline
#                     mode is a declaration rather than an inference.
#   --mode external   a different network: what a stranger reaches. Session 22
#                     adds no external claim of its own -- a local cluster on a
#                     developer's machine is reachable from nowhere, by
#                     construction -- and still needs this mode for the
#                     cumulative reason below.
#
# **Session 22's own claims are offline and host.** The gate still has three
# modes because `claims_through_session(22)` is CUMULATIVE: a Session 22
...
# and the merge now REQUIRES `--offline-input`, exactly because this session has
...
# (D1133, reversing D1021's reading). `claims_through_session(22)` runs Session
...
#
#   1. **Docker running.** Step 8 stands a cluster up; steps 3 and 4 run the
#      cluster fixtures. Without a daemon the cluster proofs SKIP, and a skip is
#      not a pass -- `dev_environment` would report `not_run` and this gate
...
#
#   1. Both projects deployed `--through-session 22`, which publishes outputs
#      **v18** -- unchanged, because Session 22 moves no outputs schema and adds
#      NO released migration. What a deploy through 22 does add on a project
#      declaring the example set is that set's SECOND migration
#      (`20260914120002`), the grants D1189 found missing.
...
USAGE
  ...
  --mode offline   Contracts, schemas and models, plus Session 22's own offline
                   halves: what `apg dev` tells docker and what the resulting
                   cluster IS, the command's refusals, the seed door, the
                   declared offline mode and the writer's checkout_commit, and
                   the churn numbers with their machine and cache state. Step 8
                   runs the five verbs against project.example.yaml and step 9
                   WRITES A HALF -- evidence/session-22-offline.json -- which no
                   earlier gate's offline mode did. Requires docker. No host,
                   no root.
  --mode host      Session 21's sweep, unchanged, plus Session 22's two host
                   claims: the deployed document's tool count is the one the
                   running plane confirmed through its own probe, and an agent
                   reads a tenant's rows through a tenant tool on beta. **No
                   new flag** -- both read declarations this gate already takes,
                   `--project-a-outputs`, `--project-b-outputs` and
                   `--admin-password-file`. Session 22 makes no trip, so this
                   mode is Session 24's to run; until then those two claims are
                   `not_run`. Needs root, host.yaml in the checkout, and the
                   declarations for every claim that is about something that
                   happened elsewhere.
  --mode external  What the public internet reaches. Session 22 adds no external
                   claim -- a disposable cluster on a developer's own machine
                   publishes on 127.0.0.1 and is reachable from nowhere, which
                   is a property of its construction rather than something a
                   scan could confirm -- but the inherited ones still need it,
                   because claims_through_session(22) is cumulative. MUST run
                   from a network that is not the deployment host.
...
mode_offline() {
  ...
  docker version --format '{{.Server.Version}}' >/dev/null 2>&1 \
    || die 3 "docker is not available, and offline mode now needs it: DEV-ENV-001's
proofs stand a cluster up with the product's own command, and step 8 runs the
five verbs. Without a daemon they SKIP, and a skipped proof makes the claim
not_run rather than failing -- a half that proves less than it appears to.
Start Docker and run this again."
  ...
  # Session 22 (ADR 0202). **The first gate whose offline mode writes a half.**
  # Four of this session's six claims are about a command a developer runs on
  # their own machine, and a claim no deployment can answer is measured here or
  # nowhere. It is written from step 3's JUnit -- the run that selected
  # everything -- for the reason every other half is: a verdict computed from a
