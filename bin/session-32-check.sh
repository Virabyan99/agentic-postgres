#!/usr/bin/env bash
#
# The Session 32 gate: the durable step substrate and workflow execution. A
# workflow definition compiled against the project's lock and installed by the
# deploy, four `app_private` tables nobody may read, a loop inside the auth
# process that executes each step as the agent that started the run, three
# routes for agent tokens, and the readers -- the doctor's twelfth check, the
# `worker-restart` rehearsal and the restore drill's member (ADR 0226-0229).
# It does not replace bin/session-01-check.sh, which must still exit 0, nor
# the Session 2-31 gates, which still own their sessions.
#
# **Derived from bin/session-31-check.sh by diff, not retyped** (D505, D507,
# D678, D693, D703, D1108, D1109, D1482, D1488). The `SESSION` assignment is the
# only session literal; everything else that differs is a named substitution,
# and every substitution in the derivation was asserted to match exactly once.
# **No flag was added or removed**: the sorted set of every flag token in this
# file equals the previous gate's, and the derivation compared the two (D1133).
# This session adds no environment gate -- every live proof reads variables the
# Session 31 sweep already exported.
#
...
# D693's guard is scoped to the session number an operator TYPES and is right
# not to flag prose -- so both halves are read line by line at every
# derivation, and the next one should expect to do the same.
#
# **There is no skipped session between 31 and 32.** 19, 26, 27 and 29 are
# absent from the registry each for its own reason; 31 registered ten claims
# and this session follows it directly. `claims_through_session(32)` inherits
# every earlier session's claims unchanged.
#
# **This session ships ONE migration, 0034, and it is the floor once applied**
# (ADR 0162 §3). Expect the ledger to read 34 on alpha and 34 + 2 on beta
# after the deploy -- and read the LEDGER, never the migrator's line (D941).
# **0034 was amended three times while it was applied nowhere** (D1686, D1687,
# D1696); after this trip's deploy it is fixed forward like every other.
# No outputs, capability, lock, project-manifest, secret or host schema moves.
# `VERSION` moves to 1.10.0.
#
# **This release's deploy recreates `auth`**, because its image moved (the
# loop, three routes, `step_token`), and adds a step 6d that installs the
# project's workflow definitions -- beta installs two, alpha none and says so.
# D1581's question (why `auth` was recreated by one generation-moving deploy
# and not another) is NOT observable here: the image moving recreates `auth`
# regardless (D1665).
#
# **EIGHT of this session's thirteen claims are declared offline** (ADR 0202),
# and each is a property of a CHECKOUT. `capacity_ceilings` is a pure grouping
# over an inspect payload this suite builds. `workflow_definition` is a schema
# and a compiler over a committed lock. `workflow_substrate` is migration 0034
# under a real cluster this workstation starts. `workflow_worker` is the loop
# over fakes that record. `workflow_surface` is the auth app with fakes behind
# it. `workflow_command` is a command's closed tables and argv.
# `workflow_install` is an AST scan of the deploy plus `apg dev up` here.
# `workflow_reading` is three readers over canned inputs and a rehearsal driven
# against a rig. A deployment would answer none of the eight differently.
#
# **FIVE claims are HOST claims and are deliberately NOT declared.**
# `ceilings_read`: D1636 closed on production, the reading compared with an
# independent sum of every running container's cap. `workflow_run`: a
# three-step run on beta as the agent that started it, each step correlated to
# the plane's own audit row by the request id THE PLANE minted -- the join
# D1696 repaired before it ever ran, because the loop's first rig used a fake
# plane that believed what the loop believed. `workflow_resume`: a crash state
# constructed and replayed exactly once, proved by the idempotency and notes
# tables, and a run parked under `worker-restart` finishing at attempt 2.
# `workflow_revocation`: a revoked agent's run stopping at its next boundary
# with no call. `workflow_restore`: a drill carrying the live cluster's runs.
# **None of those twelve proofs has ever executed.**
#
# **The sweep itself kills the auth process once** -- the parked-run proof runs
# `rehearse.sh worker-restart` against beta while a step is parked -- so run it
# when nothing else is using beta, and after the Sheet B4 rehearsal has been
# reversed. The proofs leave their probe agents REVOKED rather than deleted
# (D1700): a run references its agent, and the runs are the record the drill
# then reads.
#
# **`--redeploy-before-file` is given on THIS trip** and the sentinel recipe
# is Session 30 Sheet A2's with `s32-` in its title. **`--kit-dir` must keep
# pointing at `kit-2026-09-11`** (D1282): REC-KIT-003's claim IS the version
# gap and its proof fails on exactly that, so aiming it at the newest kit
# destroys the proof without failing. **`--candidate-manifest` is still
# given**, at Session 31's `/home/op/s31-third.yaml`, because
# `admission_live` is inherited and its proof reads it.
#
# **The signing-key rotation is NOT this gate's business.** No flag here
# performs one. `--rotated-jwt-from-file` admits a proof about a rotation that
# has ALREADY happened, and the retired JWKs from Session 30's rotation are at
# /home/op/s30-retired-<key>-jwk.json, each a single JWK object, for the first
# sweep that passes it (D1469).
#
...
USAGE
  ...
  --mode offline   Contracts, schemas and models, plus Session 32's own EIGHT
                   offline halves: the ceilings grouped by Compose's own
                   project label, so the database's cap is counted (D1636);
                   the workflow definition schema and its compiler over the
                   project's lock; migration 0034 under a real cluster; the
                   loop over recording fakes; the three agent-token routes;
                   `bin/workflow.sh`; the deploy's step 6d and `apg dev up`
                   installing the example project's definitions; and the
                   doctor's twelfth check, the restore drill's member and the
                   `worker-restart` rehearsal driven against a rig. Step 3's
                   sweep carries all eight by their marks. Step 6 checks the
                   committed example client is CURRENT -- this release's bump
                   moves `templateVersion` in it (D1238); step 7 checks the
                   ...
                   code; step 9 WRITES A HALF -- evidence/session-32-offline.json.
                   The EIGHT claims it reports are `capacity_ceilings`,
                   `workflow_definition`, `workflow_substrate`,
                   `workflow_worker`, `workflow_surface`, `workflow_command`,
                   `workflow_install` and `workflow_reading`. Requires docker.
                   No host, no root.
  --mode host      Session 31's sweep with NO flag added or removed. It answers
                   for THIS SESSION ALONE: 31 swept its own trip, so there is
                   no earlier half owed.
                   `ceilings_read`: `doctor capacity` on this host, each
                   project's figure equal to an independent sum of its running
                   containers' caps with the database among them.
                   `workflow_run`: a three-step run on beta completes as the
                   agent that started it; each step's request id -- the one the
                   PLANE minted and returned (D1696) -- is on exactly one
                   agent-plane audit row; the auth role never holds more than
                   its connection budget; a dry run lands no row; alpha, which
                   installs no definition, answers `no_such_workflow`.
                   `workflow_resume`: a post-crash state is CONSTRUCTED -- the
                   last write performed as the agent under the step's own key
                   -- and the tables show one replay and one row; the same key
                   with another body is refused; a run parked on a retry
                   survives `rehearse.sh worker-restart`, WHICH THIS SWEEP RUNS
                   against beta, and finishes at attempt 2.
                   `workflow_revocation`: a revoked agent's run stops at its
                   next boundary with no call, and its twin's completes.
                   `workflow_restore`: a drill restores the live runs.
                   **None of those twelve proofs has ever executed.**
                   **`--redeploy-before-file` is given again**, so
                   `deployment_convergence` and `stage_release` move. **This
                   release's deploy applies migration 0034** -- read the ledger
                   and expect 34, and 34 + 2 on beta -- and recreates `auth`.
                   Needs root, host.yaml in the checkout, and the declarations
                   for every claim that is about something that happened
                   elsewhere.
                   (What follows is Session 31's text, kept because its claims
                   are inherited and its flags are still passed.)
                   Session 30's sweep with ONE new flag, `--candidate-
                   manifest`, and `--host` now also exported.
                   `admission_live`: a third project whose claim exceeds the
                   safe available is refused with exit 12 and the six lines,
                   and **the same manifest at the release's default budget is
                   admitted** -- the control is what says the rule does not
                   refuse everything, which is what D1611 found it doing. The
                   `admission-refused` rehearsal is run BEFORE this sweep and
                   this reads the record it wrote.
                   `usage_read`: eight figures on both projects, and the
                   capacity reading compared with `free -m` and `df -Pk` at the
                   path the reading says it measured.
                   `telemetry_read`: the project's own Prometheus answers, read
                   inside its container because it publishes no port, and
                   every series it returns must name this project.
                   `studio_tenant_read` (Session 30): Studio's query view,
                   forwarded as the human and holding only the human's own
                   token, returns that human's rows and none of a second
                   registered subject's -- and the second subject's row is
                   proved to EXIST through the same surface, so an empty result
                   is told apart from a filtered one (ADR 0195).
  --mode external  What the public internet reaches. Session 32 adds no external
                   claim -- its three routes hang off the application route
                   that already exists and they are proved from the host, and
                   the rest is a checkout or the host itself -- but the
                   inherited ones still need it, because
                   claims_through_session(32) is cumulative. MUST run from a
  ...
  --redeploy-before-file  Opened before a redeploy: the generation and the
                        ...
                        unproved.
  --candidate-manifest FILE
                        The THIRD project's manifest, offered for admission and
                        refused (ADR 0221). `/home/op/s31-third.yaml`, NOT in
                        the checkout: a third manifest inside the release
                        dirties it and every deploy refuses (D971). Its
                        `database.shared_buffers_mb` is raised so that its
                        `unreclaimable_mb` exceeds what this host has left; the
                        control lowers that ONE field back to the release's
                        default and expects an admission, which is what makes
                        the refusal a statement about capacity rather than
                        about a rule that refuses everything. Without it
                        `admission_live` reports not_run.
  ...
  --rehearsal-evidence-dir DIR
                        Where the rehearsals wrote their records
                        (evidence/ by default). OPS-REHEARSE-002..008 read one
                        record each; -001 reads them all and the host after.
                        Session 32's parked-run proof WRITES one here too: it
                        runs `worker-restart` and reads the record back.
...
parse_arguments() {
  while [ "$#" -gt 0 ]; do
    case "$1" in
      ...
      --candidate-manifest)
        [ "$#" -ge 2 ] || die 2 "--candidate-manifest requires a value."
...
mode_host() {
  ...
  for file in "${SENTINEL_FILE}" "${ADMIN_PASSWORD_FILE}" "${ROTATED_FROM_FILE}" \
              ...
              "${REPLACEMENT_BOOTSTRAP_STATE}" "${RESTORE_EVIDENCE_FILE}" \
  ...
  # Session 31 (ADR 0221), and both are declarations rather than facts a
  # test could find. `--host` has existed since Session 2 and was never
  # exported, because until this release nothing in the suite read the HOST's
  # manifest -- admission and the capacity reading are the first.
  export APG_HOST_MANIFEST="${HOST_MANIFEST}"
  [ -n "${CANDIDATE_MANIFEST}" ] &&
    export APG_CANDIDATE_MANIFEST="${CANDIDATE_MANIFEST}"
