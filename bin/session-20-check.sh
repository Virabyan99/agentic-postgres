...
#
# **Derived from bin/session-17-check.sh by diff, not retyped** (D505, D507,
# D678, D693, D703). `readonly SESSION=20` is the only session literal;
...
# what happened.
#
#   --mode offline    a checkout: everything Session 17 checked, plus Session
#                     18's own offline halves -- the mirror's render, secret
#                     contract and verbs with a recorded compose (test_backup_mirror),
#                     the kit's contents and its refusal of a value and adoption
#                     against a recorded control plane (test_disaster_kit), the
#                     restore's plan, refusals and measured argument vectors
#                     against a recorded docker (test_node_restore), and the
#                     eight rehearsal plans, the command against a recorded
#                     runner with real files, and the registry refusal in every
#                     reader (test_rehearsal). Session 18 adds no offline step
#                     of its own beyond step 3: all four modules run there.
#   --mode host       the deployment: both permanent projects mirrored, each
#                     with a completed copy the doctor reads and a third timer
#                     enabled; the mirror's credential not the primary's and
#                     the archiver naming no mirror; and, DECLARED through the
#                     files the operator brings back, the kit exported from
#                     this host verifying and holding no value, the
#                     replacement adopted by the recorded id, the restore from
#                     the mirror alone verified with the original identity,
#                     and the eight rehearsal records with their reversals.
#   --mode external   a different network: what a stranger reaches. Session 18
#                     adds no external claim of its own -- a mirror, a kit and
#                     a rehearsal are reachable from nowhere but the host, by
...
#
#   1. Both projects deployed `--through-session 18`, which publishes outputs
#      **v16** (`backup.mirror`, `backup_state.mirror`) from a project manifest
#      at schema 4 with `backup.mirror` enabled. Session 18 adds NO migration.
#   2. **The mirror enabled and copied on both permanent projects**: the pair
#      in Infisical under /backup, the manifests at schema 4, a deploy, the
#      mirror units installed and enabled (`provision-host.sh --apply`, then
#      `backup.sh schedule enable`), and the first copy by hand:
#      `sudo bin/backup.sh --outputs <outputs.json> mirror`. Checked in step
#      4b; without it every `REC-REPO-*` proof fails on the same missing thing.
...
#   4. **Beta must carry a migration set, and alpha must not.** Beta's manifest
#      on the host goes to `schema_version: 5` with
#      `migrations: {set: projects/example}` before its deploy; alpha stays at
#      schema 1, which is the control that a version 1 manifest still deploys
#      under a schema 5 release (D930) and what makes "beta serves these objects
#      and alpha does not" a comparison rather than an assertion. Checked in
#      step 4b; without it every `TEN-*` live half fails on the same missing
#      thing.
#
#      **No new declaration flag.** Session 18's five are gone: they admit that
#      session's kit, replacement and rehearsal proofs, and every live half this
#      session adds reads a roster variable this gate already exports.
#   5. **`--fresh-host-outputs` and `--dx-record-file`** admit `fresh_host` and
#      `documented_path` (Session 12): the replacement's document and the
#      outsider's record. Session 18 arranges both; without them they stay
#      `not_run` as they have since Session 12.
#   6. **`--removed-project-file` admits `project_removal`**, inherited from
#      Session 17's trip: the record written BEFORE retiring an ephemeral
#      project. **`--admin-password-file` is still required** for every `IDN-*`
#      and Session 16 live proof this gate inherits; Session 18's own live
#      proofs need root, the two deployed documents and the declarations above.
#   7. **`host.yaml` in the checkout**: the retirement plan proof passes it to
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      ...
      --dx-record-file)
        ...
        shift 2
        ;;
      # The plan's own name for the first of these predates Run 1's reversal
      # (D1021): there is no second repository to check, and the mirror's
      # readiness is a precondition, not a flag.
...
mode_host() {
  ...
  # Session 20 exports NOTHING new, and that is the design (D687 read from the
  # other end). Every live half this session adds reads `APG_LIVE_HOST`,
  # `APG_PROJECT_A_OUTPUTS` and `APG_PROJECT_B_OUTPUTS`, which are already in
  # the roster and already exported above -- so `pytest --setup-plan` can answer
  # "will these run" before the trip rather than during it, and there is no
  # fourth declaration for an operator to forget.
  #
  # Session 18's five declaration flags are gone from this gate. They admit that
  # session's kit, replacement and rehearsal proofs, none of which this session
  # touches, and `bin/session-18-check.sh` still runs and still takes them. A
  # gate carrying flags its own proofs never read is a gate an operator supplies
  # arguments to for nothing, which is what D1021 refused a flag by name over.
