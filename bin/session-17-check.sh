#!/usr/bin/env bash
#
# The Session 17 gate: multi-project operation. The fleet inventory, the
# project lifecycle, the retirement verb and the backup schedule. It does not
# replace bin/session-01-check.sh, which must still exit 0, nor the Session
# 2-16 gates, which still own their sessions.
#
# **Derived from bin/session-16-check.sh by diff, not retyped** (D505, D507,
# D678, D693, D703). `readonly SESSION=17` is the only session literal;
...
# what happened.
#
#   --mode offline    a checkout: everything Session 16 checked, plus Session
#                     17's own offline halves -- the inventory composed from a
#                     fixture host with the doctor run for real and nothing
#                     written, project schema version 3 and outputs version 15
#                     with `permanent` as the meaning of every earlier version,
#                     the retirement plan against a fixture host with the
#                     subprocess layer recorded and its order asserted, and
#                     the schedule verb's two refusals with a recorded systemd.
#   --mode host       the deployment: the inventory over the real state root
#                     writing nothing and printing no credential, both
#                     permanent projects publishing their lifecycle at outputs
#                     v15, a retirement plan against a real project mutating
#                     nothing, and **both projects' backup timers enabled** --
#                     the proof that goes red on the deployment as it was on
#                     2026-09-04, when no timer unit was installed (D944).
#   --mode external   a different network: what a stranger reaches. Session 17
#                     adds no external claim of its own -- an operator's read
#                     over the host's own files is reachable from nowhere else
#                     by decision (ADR 0185) -- and still needs this mode for
#                     the cumulative reason below.
#
# **Session 17's own claims are `host` and offline.** The gate still has three
# modes because `claims_through_session(17)` is CUMULATIVE: a Session 17 document
...
#
#   1. Both projects deployed `--through-session 17`, which publishes outputs
#      **v15** (`project.lifecycle`). Session 17 adds NO migration: everything
#      it built is host state, a manifest field and three verbs, and the
#      cluster is untouched. Migrations 0027-0030 are applied and their
#      re-freeze window is closed (D912).
#   2. **The backup units installed and both timers enabled on both permanent
#      projects**: `sudo bin/provision-host.sh --host host.yaml --check`, then
#      `--apply` (or the units installed by hand, section 4 of the plan), then
#      `sudo bin/backup.sh --outputs <outputs.json> schedule enable` per
#      project. Without this `backup_schedule` comes back `failed`, which is the
#      finding rather than a suite defect.
...
#   4. **`--removed-project-file` admits `project_removal`** (`DEP-REMOVE-001`,
#      Session 12): the record `bin/project-retire.sh --record PATH` wrote
#      BEFORE retiring the trip's third, ephemeral project. Without it that
#      claim stays `not_run`, as it has since Session 12, and this gate exits 5.
#   5. **`--admin-password-file` is still required** for every `IDN-*` and
#      Session 16 live proof this gate inherits; Session 17's own live proofs
#      need root and the two deployed documents and nothing else.
#   6. **`host.yaml` in the checkout**: the retirement plan proof passes it to
...
USAGE
  ...
  --mode offline   Contracts, schemas and models, plus Session 17's own offline
                   halves: the fleet inventory against a fixture host with the
                   doctor run for real and nothing written, project schema
                   version 3 and outputs version 15 with permanent as the
                   meaning of every earlier version, the retirement plan and a
                   recorded real run in the only order the steps may take, and
                   the schedule verb's refusals with a recorded systemd. No
                   host, no root.
  --mode host      The inventory over the real state root, writing nothing and
                   printing no credential; both permanent projects publishing
                   their lifecycle at outputs v15; a retirement plan against a
                   real project mutating nothing and refusing the wrong
                   confirmation; and both projects' backup timers enabled --
                   the proof that goes red on the deployment as it was on
                   2026-09-04 (D944). Needs root, host.yaml in the checkout,
                   and --removed-project-file for the Session 12 removal claim.
  --mode external  What the public internet reaches. Session 17 adds no external
                   claim -- an operator's read over the host's own files is
                   reachable from nowhere else by decision (ADR 0185) -- but
                   the inherited ones still need it, because
                   claims_through_session(17) is cumulative. MUST run from a
...
mode_offline() {
  ...
  # Session 17: the bounds document carries the lifecycle relation, and the
