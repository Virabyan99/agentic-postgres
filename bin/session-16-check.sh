#!/usr/bin/env bash
#
# The Session 16 gate: agent capability governance v2 and the evaluation
# harness. Versioned, classified and bounded capabilities, a closed denial
# taxonomy, a windowed quota, idempotency keys, a dry-run, project profiles, and
# a harness whose adversarial cases are derived from the contract. It does not
# replace bin/session-01-check.sh, which must still exit 0, nor the Session 2-15
# gates, which still own their sessions.
#
# **Derived from bin/session-15-check.sh by diff, not retyped** (D505, D507,
# D678, D693, D703). `readonly SESSION=16` is the only session literal;
# everything else that differs is a named substitution.
#
# **This header and the usage block below were REWRITTEN, not patched.** Session
# 15 recorded that its own predecessor had copied Session 13's header whole, one
# derivation after Session 13 had written the warning about exactly that (D853)
# -- and then Session 15's own derivation rewrote the header and missed the
# usage block's mode descriptions, which is the half an operator reads (D858).
# D693's guard is scoped to the `--session N` an operator TYPES and is right not
# to flag prose, so both halves are prose this derivation had to read line by
# line, and the next one should expect to do the same.
#
...
# what happened.
#
#   --mode offline    a checkout: everything Session 15 checked, plus Session
#                     16's own offline halves -- the capability manifest at
#                     schema version 3 and the project manifest at version 2, the
#                     compiler's refusals (a retired capability, a widening
#                     profile), the denial taxonomy compared against its
#                     migration, the quota, replay and rehearsal against a
#                     fixture cluster, and the evaluation harness: every derived
#                     and written case run against the runtime's own request
#                     builders, and the report current.
#   --mode host       the deployment: the agent plane through its published
#                     route, with the lock the deploy compiled and mounted.
#                     Every Session 16 claim has a live half here, because
#                     `claim_mode` refuses a claim whose every proof is offline
#                     -- and **none of the seven had executed when this gate was
#                     written** (D938). Runs 2-8 proved their planes in a
#                     checkout; Run 10's trip is what runs these.
#   --mode external   a different network: what a stranger reaches. Session 16
#                     adds no external claim of its own, and still needs this
#                     mode for the cumulative reason below.
#
# **Session 16's own claims are `host` and offline.** The gate still has three
# modes because `claims_through_session(16)` is CUMULATIVE: a Session 16 document
...
#      Every Session 16 live proof reaches the agent plane through `routes.mcp`
#      and creates its agent through `routes.app`; `app_base` refuses a route
#      that is not ready rather than composing a URL from the domain -- where
#      every negative assertion would pass against a 404.
#   2. Both projects deployed `--through-session 16`. Migrations 0027-0030 are
#      this session's -- the taxonomy column, the quota table, the idempotency
#      table and the dry-run -- and none of the seven claims is there without
#      them. **Project B may NOT lag**: Session 10's restore drill compares a
#      restored cluster against the release's full migration ledger, and a
#      release that adds four migrations to a project that never received them
#      fails that comparison, correctly. Then take a **full backup of project B**.
#      **This trip also closes the re-freeze window on 0028-0030** (D912): ADR
#      0028 attaches immutability to APPLIED, and after this deploy the only
#      remedy for any of the four is a new migration.
...
#   4. **`--admin-password-file` is required for every Session 16 live proof**,
#      as it was for every `IDN-*` one: the probe agent is owned by a subject
#      the suite registers through an administrator session. Without it they
#      skip, and a skip is not a pass -- the document comes back `not_run` and
#      this gate exits 5. That is D686 reaching the operator, not a suite
#      failure.
#   5. **The lock the plane obeys is the one the deploy compiled**, from the
#      COMMITTED canonical contract and the INSTALLED project manifest (ADR
#      0183, D930). The host's `capabilities.yaml` is not an input to it, so the
#      deployment serves a version 3 lock whatever that file says; and both
#      host manifests are version 1, so the lock carries no `profile` and the
#      profile proof measures exactly that.
#
# **The outputs version is NOT this session's; it stays at v14.** Nothing this
# session built needs publishing: the contract digest the harness compares was
# already in the `mcp` block, and a schema bump out of habit would be a
...
USAGE
  ...
  --mode offline   Contracts, schemas and models, plus Session 16's own offline
                   halves: the compiler's refusals of a retired capability and
                   of a widening profile, the denial taxonomy against its
                   migration, the quota, the replay and the rehearsal against a
                   fixture cluster, and the evaluation harness -- every derived
                   and written case run against the runtime's own request
                   builders, and the report current. No host, no root.
  --mode host      The agent plane through its published route, obeying the
                   lock the deploy compiled and mounted: a write recording its
                   capability version and contract hash, a denied read recording
                   its taxonomy reason, a quota refusing the third call, a
                   replay performing the work once, a rehearsal writing nothing,
                   the approval declaration and its enum member on the
                   deployment, the lock matching the installed manifest's
                   profile, and the published contract digest matching the one
                   the harness evaluated. Needs root -- it reads the installed
                   lock and manifest and reaches the Docker daemon -- and every
                   one of these needs --admin-password-file. Without it they
                   skip, the document comes back not_run, and this gate exits 5.
  --mode external  What the public internet reaches. Session 16 adds no external
                   claim -- an agent plane's governance is measured through its
                   own front door or on the host -- but the inherited ones still
                   need it, because claims_through_session(16) is cumulative.
...
mode_offline() {
  ...
  # Session 16: each example project's profile applied to the approved contract
  ...
  # Session 16: the evaluation report is current, and the render REFUSES when
...
mode_host() {
  ...
  for file in "${SENTINEL_FILE}" "${ADMIN_PASSWORD_FILE}" "${ROTATED_FROM_FILE}" \
              ...
              "${FRESH_HOST_OUTPUTS}" "${REMOVED_PROJECT_FILE}" "${DX_RECORD_FILE}"; do
  ...
  if [ -z "${ADMIN_PASSWORD_FILE}" ]; then
    ...
    printf 'administrator session will skip, and every claim they support -- all seven of\n'
    printf 'this session'"'"'s among them -- will report not_run.\n'
  fi
