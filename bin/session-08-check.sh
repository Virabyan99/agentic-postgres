#!/usr/bin/env bash
#
# The Session 8 gate: the agent plane. It does not replace
# bin/session-01-check.sh, which must still exit 0, nor the Session 2, 3, 4, 5,
# 6 and 7 gates, which still own their own sessions' verdicts.
#
...
#                     agent plane's own contract suite.
#   --mode host       the deployment host: the agent plane as it is served --
#                     one published path, a private health surface, the tokens
#                     it accepts, what its container holds, and what an agent
#                     can read.
#   --mode external   a different network: what a stranger reaches of a plane
#                     whose health routes answer 200 on the container's socket
#                     and are kept private by the ABSENCE of a route.
#
# Session 8's evidence has two halves and cannot be written from one.
# `public_agent_boundary` is measured from off-host, and the writer refuses a
# session document that is silent about a claim. Run both, then merge.
#
...
# reporting success.
#
# **Two things must be true of the deployment before host mode means anything**,
...
#   2. The deployed document must be at the current outputs version. Both hosts
#      carried v11 while this tree rendered v12, and `evidence.py` refuses a
#      deployed document that is not current -- so the redeploy is a
#      prerequisite of the gate, not a consequence of it.
#
...
usage() {
  cat <<'USAGE'
Usage: bin/session-08-check.sh --mode offline
...
")" \
  ...
  the scan then fails to find what it planted.
       bin/session-08-check.sh --mode external --public-ipv4 ADDR \
...
Each mode writes one evidence half. Session 8 needs BOTH -- `public_agent_boundary`
...
This command verifies and never deploys. Use ./deploy.sh --through-session 8
...
# than a note. Every Session 8 host claim measures a served plane; if the route
...
mode_offline() {
  ...
  # only offline check that touches this session's new deployment module.
  ...
  # Session 8's own: the compiled, project-neutral capability contract, and the
  ...
  printf '\n\033[1msession-08-check: offline PASSED\033[0m\n'
}
...
mode_host() {
  ...
  if [ -z "${ADMIN_PASSWORD_FILE}" ]; then
    printf 'session-08-check: no --admin-password-file, so the proofs needing an\n'
  ...
  step "4. Host-local acceptance suite, over two projects"
  ...
  if ! evidence_is_supportable; then
    announce_no_evidence
    printf '\n\033[1msession-08-check: host PASSED\033[0m\n'
    return 0
  fi

  step "5. Static proofs of the claims this run records"
  run_claim_proofs host "${EVIDENCE_DIR}/${EVIDENCE_PREFIX}-host-claims.xml"

  step "6. Host evidence"
  write_evidence host

  printf '\n\033[1msession-08-check: host PASSED\033[0m\n'
  printf 'This is one half. Session 8 also needs --mode external; see --help.\n'
}
...
mode_external() {
  ...
  printf 'session-08-check: this mode is only meaningful from a network that is\n'
  ...
  if ! evidence_is_supportable; then
    announce_no_evidence
    printf '\n\033[1msession-08-check: external PASSED\033[0m\n'
    return 0
  fi
  ...
  printf '\n\033[1msession-08-check: external PASSED\033[0m\n'
  printf 'This is one half. Session 8 also needs --mode host; see --help.\n'
}
