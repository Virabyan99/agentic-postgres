...
# D678, D693, D703). `readonly SESSION=18` is the only session literal;
...
#   4. **The four declarations that admit the kit, the replacement and the
#      rehearsals**: `--kit-dir` (the kit exported from THIS host),
#      `--replacement-host-outputs` (the restored project's deployed document,
#      fetched from the replacement), `--restore-evidence-file` (the record
#      `bin/restore.sh` wrote there), and `--rehearsal-evidence-dir` (where the
#      eight rehearsals wrote their records). Without them the proofs they admit
#      skip and `disaster_kit`, `replacement_host_restore` and
#      `failure_rehearsal` report `not_run`, as they must.
...
mode_host() {
  ...
  # Session 18, D687 applied on the day: every variable the four new proof
  # modules read is exported here, or the proof can only skip.
