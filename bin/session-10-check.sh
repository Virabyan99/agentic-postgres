#!/usr/bin/env bash
#
# The Session 10 gate: encrypted backups, continuous WAL archiving, and a
# restore that was rehearsed. It does not replace bin/session-01-check.sh, which
# must still exit 0, nor the Session 2-9 gates, which still own their own
# sessions' verdicts.
#
# **Derived from bin/session-09-check.sh by diff, not retyped** (D505, D507).
# `readonly SESSION=10` is the only session literal; everything else that differs
# is a named substitution, and everything unnamed is Session 9's byte for byte.
#
...
#
#   --mode offline    a checkout: contracts, schemas, models, the rendered
#                     pgbackrest configuration, the secret contract's third
#                     format, the drill's disposability guard driven against a
#                     stubbed docker, and the backup schedule.
#   --mode host       the deployment host: a point-in-time restore into a
#                     disposable instance, the live cluster read before and
#                     after it, the restored instance queried, and a broken
#                     archiver made visible and repaired.
#   --mode external   a different network: what a stranger reaches. Session 10
#                     adds no external claim of its own -- there is nothing about
#                     a backup repository a stranger can measure -- and it still
#                     needs this mode, for the reason below.
#
# **Session 10's own five claims are all `host`.** The gate still has three modes
# because `claims_through_session(10)` is CUMULATIVE: a Session 10 document must
...
#
# **Inventing an external backup claim to make the shape symmetric was refused**
# (ADR 0065): a proof that reaches an end state by a route the product does not
# take proves the end state is reachable, not that the product reaches it.
#
...
#      **This session DOES move the version, 12 -> 13.** Both deployed projects
#      are on v12, so both must be redeployed before this gate means anything --
#      and unlike Session 9's, that is not a formality: `backup` and
#      `backup_state` are new blocks and nothing on the host has them.
#   3. **The repository must be ready**, and this is Session 10's own
#      precondition: the stanza exists, at least one full backup exists, and the
#      archiver is not failing. The deploy's step 6c creates and checks the
#      stanza, but the **first full backup is an operator command at a TTY** --
#      deliberately, because it is the first operation that writes a meaningful
#      amount to a repository nobody has paid for yet. Without it, four
#      requirements fail on one missing thing.
#
...
usage() {
  cat <<'USAGE'
Usage: bin/session-10-check.sh --mode offline
...
")" \
  ...
  the scan then fails to find what it planted.
       bin/session-10-check.sh --mode external --public-ipv4 ADDR \
...
mode_offline() {
  ...
  printf '\n\033[1msession-10-check: offline PASSED\033[0m\n'
}
...
mode_host() {
  ...
  if [ -z "${ADMIN_PASSWORD_FILE}" ]; then
    printf 'session-10-check: no --admin-password-file, so the proofs needing an\n'
  ...
  if ! evidence_is_supportable; then
    announce_no_evidence
    printf '\n\033[1msession-10-check: host PASSED\033[0m\n'
    return 0
  fi
...
mode_external() {
  ...
  printf 'session-10-check: this mode is only meaningful from a network that is\n'
  ...
  if ! evidence_is_supportable; then
    announce_no_evidence
    printf '\n\033[1msession-10-check: external PASSED\033[0m\n'
    return 0
  fi
