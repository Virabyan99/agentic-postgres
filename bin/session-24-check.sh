#!/usr/bin/env bash
#
# The Session 24 gate: `apg studio`, a loopback page over a deployment that
# holds the human's token and hands the browser a launch cookie (ADR 0205), and
# migration 0032, which makes the audit reader return the boundary that refused.
# It does not replace bin/session-01-check.sh, which must still exit 0, nor the
# Session 2-23 gates, which still own their sessions.
#
# **Derived from bin/session-23-check.sh by diff, not retyped** (D505, D507,
# D678, D693, D703, D1108, D1109). `readonly SESSION=24` is the only session
# literal; everything else that differs is a named substitution.
#
# **This header and the usage block below were REWRITTEN, not patched.** Two
...
#
# **This gate's session closes in OFFLINE mode too, and the split runs where
# Session 23's did.** Two of its four claims are declared offline --
# `studio_boundary` (where Studio binds, what it holds, what it ships, how its
# command behaves) and `studio_surface` (the four launch answers, the query
# builder, the audit page, the session plane). Every proof behind them runs
# against the source or against a rig this checkout builds: a dev cluster, the
# real auth application, the pinned PostgREST verifying its published JWKS, and
# the pinned Traefik in front. A production deployment would answer none of
# them differently.
#
# The other two were deliberately NOT folded in. `studio_revocation` is about a
# RUNNING plane refusing an agent's NEXT request -- a fact about a process that
# re-reads an agent's claims on every request, which no checkout has -- and
# `audit_boundary_reported` is about a boundary a deployed plane actually
# recorded, as opposed to one a test wrote into a row it then read back. Both
# are `not_run` at this session's close, by design, and the trip collects them.
#
# **--mode host now answers for Sessions 22, 23 AND 24 at once** (D1244).
# `claims_through_session` is cumulative, so one sweep in this mode collects
# every host claim those three sessions left owed: Session 22's
# `plane_confirmed_count` and `agent_tenant_read`, Session 23's
# `generated_client_hash` and `agent_lock_reported`, and this session's two.
# Step 9 here writes the host half for this session only; the merges are three,
# one per session, and they belong to the trip sheet.
#
...
# what happened.
#
#   --mode offline    a checkout that produces evidence: everything Session 23
#                     checked, plus Session 24's own offline halves -- Studio's
#                     decisions with no I/O in them (test_studio_core), the
#                     server's five checks against a real socket
#                     (test_studio_server), the command's refusals and exit
#                     codes (test_studio_command), Studio against a loopback
#                     deployment (test_studio_runtime), the three first-party
#                     files and what is not in them (test_studio_assets), and
#                     Studio's three latencies (test_capacity_envelope). Step 6
#                     checks the committed example client is current -- the
#                     release bump moves `templateVersion` in it, so a bump that
#                     forgot the regeneration stops here (D1238); step 8b
#                     typechecks it on the pinned toolchain; step 8c asserts
#                     Studio ships no third-party code. **Requires docker**, as
#                     Session 23's does and for the same reason: the runtime rig
#                     is a cluster, a PostgREST and an edge. No host, no root.
#   --mode host       the deployment, and it answers for THREE sessions at once
#                     (D1244). This session's two: `studio_revocation` (an
#                     agent on alpha revoked THROUGH STUDIO, launched by this
#                     sweep on loopback, and its next MCP request refused --
#                     with a wrong confirmation refused first and the agent
#                     still served, which is the control) and
#                     `audit_boundary_reported` (that refusal's row, read both
#                     through Studio and directly through the endpoint, each
#                     carrying the boundary the plane recorded). Plus the four
#                     Sessions 22 and 23 left owed, because
#                     `claims_through_session` is cumulative and neither
#                     session made a trip. **No new flag**: every one of them
#                     reads a declaration this gate already takes. **The sweep
#                     needs docker**, which it has -- the doctor and every
#                     Session 18 rehearsal use it as root.
#   --mode external   a different network: what a stranger reaches. Session 24
#                     adds no external claim of its own -- Studio binds loopback
#                     and there is no flag for anywhere else, so nothing it does
#                     is reachable from a network at all -- and still needs this
#                     mode for the cumulative reason below.
#
# **Session 24's own claims are offline and host.** The gate still has three
# modes because `claims_through_session(24)` is CUMULATIVE: a Session 24
...
# offline claims (ADR 0202). **Sessions 22's and 23's halves are still owed
# too**, and they are two further merges into evidence/session-22.json and
# evidence/session-23.json; one host sweep feeds all three (D1244).
#
...
# (D1133, reversing D1021's reading). `claims_through_session(24)` runs Session
...
#
#   1. **Docker running**, and this gate needs it for more than Session 23's
#      did. Step 8 stands a cluster up; step 8b builds the toolchain image and
#      typechecks the committed client; step 3's sweep runs TWO runtime modules
#      that each stand up a cluster, a PostgREST configured from compose.yaml's
#      own block and the pinned Traefik -- Session 23's client rig and Session
#      24's Studio rig. Without a daemon all of it SKIPS, and a skip is not a
#      pass -- `dev_environment`, `generated_client`,
#      `generated_client_toolchain`, `studio_boundary` and `studio_surface`
#      would each report `not_run` and this gate would exit 5 having written a
#      half that proves less than it looks like. So the daemon is checked up
...
#
#   1. Both projects deployed `--through-session 24`, which publishes outputs
#      **v18** -- unchanged, because Session 24 moves no outputs schema and no
#      project migration. **It DOES add one released migration**: 0032, a DROP
#      and CREATE of `auth_list_agent_audit` at the same arity, re-issuing the
#      grant the DROP took with it. So the ledgers move by one on each project,
#      and they are READ after the deploy rather than taken from the migrator's
#      line (D941). The deploy also RECREATES the auth/mcp container, whose
#      mounted lock digest moved (ADR 0155) -- which is what makes
#      `list_resources` answer with its `lock` member at all, and what Session
#      23's two owed claims need. **Read the container, never the file** (D1152,
#      D1153). A deploy through 22 first applies the example set's second
#      migration on beta, the grants D1189 found missing.
#   2. **Beta's manifest at schema 6 naming `projects/example` for BOTH keys**
#      (`migrations.set` and `mcp.capabilities`), and alpha's at schema 1, which
#      is the control that a version 1 manifest still deploys under a schema 6
#      release (D930) and what makes "beta serves the tenant write and alpha
#      does not" a comparison. Checked in step 4c.
...
#      from Session 18 and checked in step 4b.
...
#      `documented_path` (Session 12); **`--removed-project-file`** admits
#      `project_removal` (Session 17); **`--admin-password-file` is required**
#      for every `IDN-*`, Session 16, Session 21 and Session 22 live proof: the
#      ceiling at agent creation is measured through the admin endpoint, and so
#      is the agent this session's tenant read is performed as.
...
USAGE
  ...
  --mode offline   Contracts, schemas and models, plus Session 24's own offline
                   halves: Studio's decisions with no I/O in them, its server's
                   five checks against a real socket, its command's refusals and
                   exit codes, Studio against a loopback deployment (a dev
                   cluster, the real auth application, the pinned PostgREST
                   verifying its published JWKS, the pinned Traefik), the three
                   first-party files and what is not in them, and Studio's three
                   latencies. Step 6 checks the committed example client is
                   CURRENT -- this release's bump moves `templateVersion` in it
                   (D1238); step 8b typechecks it on the pinned toolchain with
                   no network; step 8c asserts Studio ships no third-party code;
                   step 9 WRITES A HALF -- evidence/session-24-offline.json. The
                   two claims it reports are `studio_boundary` and
                   `studio_surface`. Requires docker. No host, no root.
  --mode host      Session 23's sweep, unchanged in arguments, and it now
                   answers for THREE sessions at once (D1244): Session 22's
                   `plane_confirmed_count` and `agent_tenant_read`, Session 23's
                   `generated_client_hash` and `agent_lock_reported`, and this
                   session's two. `studio_revocation`: an agent on alpha is
                   revoked THROUGH STUDIO -- launched here as a subprocess by
                   this sweep, on loopback -- and its next MCP request is
                   refused, with a wrong confirmation refused first and the
                   agent still served as the control. `audit_boundary_reported`:
                   that refusal's row is read both through Studio and directly
                   through GET /admin/audit, and both must carry the boundary
                   the plane recorded. **No new flag** -- the live module reads
                   `--project-a-outputs` and `--admin-password-file`, which this
                   gate already takes. **The sweep runs docker**, which it has,
                   and it launches Studio as root on this host. Step 9 writes
                   the host half for THIS session only; the 22 and 23 halves are
                   three separate merges. Needs root, host.yaml in the checkout,
                   and the declarations for every claim that is about something
                   that happened elsewhere.
  --mode external  What the public internet reaches. Session 24 adds no external
                   claim -- Studio binds loopback and there is no flag for
                   anywhere else, so nothing it does is reachable from a network
                   at all -- but the inherited ones still need it, because
                   claims_through_session(24) is cumulative. MUST run from a
...
Run bin/lock-versions.sh --check."
  ...
  # Session 24 (STU-SUPPLY-001). Cheap, and the SENTENCE is what it is for: the
  ...
  # Session 22 introduced this step (ADR 0202) and Session 24 keeps it. TWO of
  # this session's four claims are declared offline -- `studio_boundary` and
  # `studio_surface` -- and the two that are not are the reason the declaration
  # is worth having: a checkout cannot say whether a RUNNING plane refuses a
  # revoked agent's next request, nor what boundary that plane recorded, and
  # folding those in would report a revocation nobody performed as green. The
  ...
  if evidence_is_supportable; then
    ...
    printf -- 'Sessions 22 and 23 still owe theirs; --mode host answers for all\n'
    printf -- 'three at once, and the merges are three (D1244).\n'
  else
    announce_no_evidence
  fi
