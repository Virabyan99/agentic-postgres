#!/usr/bin/env bash
#
# The Session 28 gate: an audit acted on, the on-ramp for a fork made before
# `projects/<slug>/` existed, what the agent record keeps, and the reading
# before a tag (ADR 0210-0215). It does not replace bin/session-01-check.sh,
# which must still exit 0, nor the Session 2-25 gates, which still own their
# sessions.
#
# **Derived from bin/session-25-check.sh by diff, not retyped** (D505, D507,
# D678, D693, D703, D1108, D1109). `readonly SESSION=28` is the only session
...
# nor wrote an evidence document, so neither has a half to write; the session
# number goes 25 -> 28 and the gap is the record, the way 19's is (D1063).
# `claims_through_session(28)` inherits every earlier session's claims
# unchanged.
#
# **This session ships one released migration and one operator command.**
# `20260917120033` adds two prune functions over the agent record, granted to
# NOBODY, and a size reading granted to the record's existing reader; nothing
# in this release calls either prune. `apg release-reading` states the facts
# before a tag is cut. A project's lock moves to schema 3 and records whether
# its `follows_release_version` was computed or declared. ADR 0162 prices it a
# MINOR, `VERSION` moves to 1.7.0 -- and **Session 28 cuts no tag and takes no
# trip**, so the price is confirmed by Session 29's `upgrade plan` on the
# deployment rather than here. A `major` required there is a stop condition
# rather than a number to write down.
#
# **TWO of this session's three claims are declared offline** (ADR 0202), one
# fewer than Session 25 declared, and the line runs where Session 23's did.
# `project_set_release_record` is what a CHECKOUT'S project lock records and
# what the command that writes it refuses: a deployment holds no project lock,
# so there is nothing a running system could answer differently.
# `release_reading` is a command that reads `git` in a clone and prints; its
# subject IS the checkout.
#
# **`agent_record_retention` is a HOST claim and is deliberately NOT declared.**
# The offline half proves the functions, the refusals, the grants that are
# absent and that nothing calls a prune -- against a cluster the proof itself
# seeded. What only a deployment can say is what two tables written to by nine
# sessions of trips are carrying, and whether a prune removes a row of that
# record; a retention claim proved against a table somebody arranged is D940
# exactly. It is `not_run` at this session's close, by design, and Session 29's
# trip collects it AFTER applying the migration.
#
# **--mode host answers for Session 28 alone, and it cannot be run yet.**
# Migration 0033 is in this release's set and is applied by no session; the
# deployment is at 1.6.0 and `deployed_through_session` 25. Session 29 deploys
# this commit first and sweeps after, which is the order D1425 exists for --
# and it is also why `stage_release`'s live half is RED for the whole gap
# between this commit and that deploy. That is a consequence of the shape, not
# a defect, and the trip inheriting it should not be surprised by it.
#
...
# what happened.
#
#   --mode offline    a checkout that produces evidence: everything Session 25
#                     checked, plus Session 28's own offline halves -- a project
#                     set's record of the release it was frozen against and the
#                     flag that declares it (test_project_migration_sets), and
#                     the reading before a tag (test_release_reading). Step 3's
#                     sweep carries both by their marks; nothing here is
#                     selected by name that the sweep does not already collect.
#                     **Requires docker**, as Session 25's does and for the same
#                     reasons. No host, no root.
#   --mode host       the deployment, and it answers for THIS session and the
#                     two standing Session 12 claims below. `agent_record_
#                     retention`: both projects carrying 0033's three functions,
#                     neither prune reachable by any request role, the size
#                     reading answering on a cluster with real history, a
#                     bounded prune measured inside a transaction the proof
#                     rolls back, and the doctor's agent-record check reporting
#                     that deployment's own numbers. **Nothing here deletes a
#                     row of the agent record**: the record is evidence (ADR
#                     0135, ADR 0142). **No new flag.** **The sweep needs
#                     docker**, which it has.
#   --mode external   a different network: what a stranger reaches. Session 28
...
#
# **Session 28's own claims are offline and host.** The gate still has three
# modes because `claims_through_session(28)` is CUMULATIVE: a Session 28
...
# It is taken by a person at a workstation with a full clone, at a session's
# close, and the argument against wiring it into a gate is the measurement: the
# reading needs tags, and two of CI's three jobs check out without any. A step
# that printed *the reading cannot be taken here* on every run is a line nobody
# reads.
#
...
# (D1133, reversing D1021's reading). `claims_through_session(28)` runs Session
...
#
#   1. **Docker running**, and this gate needs it for everything Session 25's
...
#      stand up a cluster, and the agent-plane module that proves 0033's
#      functions needs one too. Without a daemon all of it SKIPS, and a skip is
#      not a pass -- every offline claim that depends on a cluster would report
#      `not_run` and this gate would exit 5 having written a half that proves
#      less than it looks like. So the daemon is checked up front and its
#      absence is exit 3, not a skip.
...
#
#   1. Both projects deployed `--through-session 28`, which publishes outputs
#      **v18** -- unchanged, because Session 28 moves no outputs schema. **The
#      ledger MOVES on both projects**, which is the first deploy in four
#      sessions of which that is true: `20260917120033` is applied by that
#      deploy and by nothing else. Read the ledger rather than the migrator's
#      line (D941), and read the container rather than the lock file (D1152,
#      D1153).
...
#      session did not repair it: the audit's own rule is that a session which
#      repairs what the readers found and then declares victory without a THIRD
#      reader has not closed it. **`--removed-project-file`** admits
#      `project_removal` (Session 17); **`--admin-password-file` is required**
#      for every `IDN-*`, Session 16, Session 21 and Session 22 live proof.
...
#
# **The signing-key rotation is NOT this gate's business** and no flag here
# performs one. `--rotated-jwt-from-file` admits a proof about a rotation that
# has ALREADY happened; the rotation itself is an operator sequence with an
# irreversible `promote` in it, and Session 28's Appendix R is the sheet. If the
# rotation is performed on the same trip, the sweep goes AFTER step 7 (D1470):
...
USAGE
  ...
  --mode offline   Contracts, schemas and models, plus Session 28's own offline
                   halves: a project set's record of the release it was frozen
                   against and the flag that declares it, and the reading before
                   a tag. Step 3's sweep carries both by their marks. Step 6
                   checks the committed example client is CURRENT -- this
                   release's bump moves `templateVersion` in it (D1238); step 7
                   checks the project's own tool catalog beside the registry's
                   documents; step 8b typechecks the client on the pinned
                   toolchain with no network; step 8c asserts Studio ships no
                   third-party code; step 9 WRITES A HALF --
                   evidence/session-28-offline.json. The TWO claims it reports
                   are `project_set_release_record` and `release_reading`.
                   Requires docker. No host, no root.
  --mode host      Session 25's sweep, unchanged in arguments, and it answers
                   for THIS SESSION ALONE: 26 and 27 registered nothing, so
                   there is no earlier half owed.
                   `agent_record_retention`: both projects carrying migration
                   0033's three functions with the identity argument lists this
                   release creates, neither prune reachable by `auth_service`,
                   `agent_writer`, `agent_reader` or `authenticated`, the size
                   reading answering on a cluster whose history was written by
                   the deployed plane, a bounded prune measured INSIDE A
                   TRANSACTION THE PROOF ROLLS BACK with the row count before
                   and after as its control, and the doctor's agent-record check
                   reporting that deployment's own numbers with no threshold.
                   **It runs after the deploy that applies 0033, and it deletes
                   nothing.** `stage_release` moves too, because this trip
                   deploys the release the tree names before it sweeps -- which
                   is the arrangement D1425 exists for and the end of D1401's
                   third occurrence. **No new flag.** **The sweep runs
  ...
  --mode external  What the public internet reaches. Session 28 adds no external
                   claim -- it ships no service, no port and no route, and its
                   two offline claims are about a checkout -- but the
                   inherited ones still need it, because
                   claims_through_session(28) is cumulative. MUST run from a
