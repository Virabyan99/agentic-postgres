#!/usr/bin/env bash
#
# The Session 31 gate: the node as a finite resource. Capacity declared in
# host.yaml, admission decided at every deploy, every project service bounded
# in processes and CPU, the collector consumed, a secret's value checked
# against its declared kind, and -- on the deployment -- a third project
# refused with the same control that says the rule does not refuse everything
# (ADR 0221-0225). It does not replace bin/session-01-check.sh, which must
# still exit 0, nor the Session 2-30 gates, which still own their sessions.
#
# **Derived from bin/session-30-check.sh by diff, not retyped** (D505, D507,
# D678, D693, D703, D1108, D1109, D1482, D1488). `readonly SESSION=31` is the
...
#
# **There is no skipped session between 30 and 31.** 19, 26, 27 and 29 are
# absent from the registry each for its own reason; 30 registered six claims
# and this session follows it directly, so the gate number goes 30 -> 31 with
# no gap to explain. `claims_through_session(31)` inherits every earlier
# session's claims unchanged.
#
# **This session ships NO migration and moves NO product schema.** The ledger
# does not move -- expect 33 on alpha and 33 + 2 on beta, exactly as before --
# and no outputs, capability, lock, project-manifest or secret schema changes.
# What DOES move is an OPERATOR manifest: `host.yaml` goes to schema 3, and the
# move is ADDITIVE. Schema 2 still validates and declares nothing; there is no
# migrator; a schema 2 document carrying a `capacity` block is refused rather
# than quietly read. `VERSION` moves to 1.9.0.
#
# **THE ONE THING A DERIVATION MUST NOT READ PAST: this release's deploy
# recreates EVERY container, the database included.** Every service's
# definition moved -- twenty gained `pids_limit`, nine gained `cpus`, mcp
# gained `APG_OTLP_ENDPOINT` -- so the mounted-content digest differs for all
# of them (ADR 0155). The Session 30 gate said a redeploy recreates nothing
# whose mounted content did not move, and that sentence is now false for this
# release. `docker ps` after the deploy shows every container younger than it.
#
# **SIX of this session's ten claims are declared offline** (ADR 0202), one
...
# `capacity_reading` is a parser over fixture text and a reporter over readings
# this suite builds -- the FIGURES belong to a host and the parsing does not.
# `admission_decision` is arithmetic over those same synthesised readings,
# which is exactly what `decide` being pure buys: the probe that feeds it is a
# separate module for this reason (D1610). `process_limits` is a compose model
# plus two containers this workstation can start, and the fork proof runs under
# Docker here, on CI and on the host -- which is how D1602 was found.
# `telemetry_bounded` is a rendered collector configuration and an in-process
# meter provider. `secret_kind_checked` is a pure check over values the tests
# build, deliberately malformed. A deployment would answer none of the six
# differently.
#
# **FOUR claims are HOST claims and are deliberately NOT declared.**
# `admission_live`: `decide` against a real declaration with a real committed
# total, and the control beside it is the half that matters -- a rule that
# refused everything would satisfy the refusal alone, and this repository
# shipped exactly that until D1611. `usage_read`: eight figures off two
# clusters with nine sessions of data in them, compared with `free -m` and
# `df -Pk` at the path the reading says it measured. `telemetry_read`: whether
# a metric crosses an OTLP connection into a Prometheus that is routed nowhere,
# which has never happened in this product's life because
# `mcp_metrics.configure` had no production caller until this release.
# `agent_write_method`: Session 9's pair, registered by this session's orphan
# triage (D1597) -- they have run on every sweep since Session 9 and belonged
# to no claim, so their results were read into nothing.
#
# **TWO NEW DECLARATION FLAGS, and both are paths an operator supplies.**
#   `--host FILE` already existed and is now ALSO exported, as
#   `APG_HOST_MANIFEST`: admission and the capacity reading are the first
#   things in this product that read the HOST's declaration rather than a
#   project's, and the path is not derivable -- `/etc/agentic-postgres/
#   host.yaml` is where a deploy installs one and the trip's copy is the
#   operator's working file, and a test that guessed would measure whichever
#   it happened to find.
#   `--candidate-manifest FILE` is the THIRD project's manifest, the one
#   offered for admission and refused. It lives at `/home/op/s31-third.yaml`
#   and deliberately NOT in the checkout: a third manifest inside the release
#   dirties it and every deploy refuses (D971).
# Both are in `tests/conftest.py`'s closed roster (D687), so a typo reads as an
# error rather than as a proof that quietly never runs.
#
# **`--redeploy-before-file` is given on THIS trip.** `deployment_convergence`
# is re-taken, as it was on Session 30's, and the sentinel recipe is Session 30
# Sheet A2's with `s31-` in its title. **`--kit-dir` must keep pointing at
# `kit-2026-09-11`** (D1282): REC-KIT-003's claim IS the version gap and its
# proof fails on exactly that, so aiming it at the newest kit destroys the
# proof without failing.
#
...
# sweep that passes it (D1469). ADR 0224 changed what `retire` REPORTS and
# changed nothing about what is published.
#
...
USAGE
  ...
  --mode offline   Contracts, schemas and models, plus Session 31's own SIX
                   offline halves: host.yaml schema 3 and its four capacity
                   members; the capacity reading, where every figure has three
                   outcomes and the third is reported; the admission decision,
                   which may fail closed because it is a decision and not a
                   report; the process and CPU limits, whose fork proof starts
                   two containers here; the collector's const labels and the
                   runtime's two instruments; and a secret's value checked
                   against its declared kind. Step 3's sweep carries all six by
                   their marks. Step 6 checks the committed example client is
                   ...
                   evidence/session-31-offline.json. The SIX claims it reports
                   are `capacity_declared`, `capacity_reading`,
                   `admission_decision`, `process_limits`, `telemetry_bounded`
                   and `secret_kind_checked`. Requires docker. No host, no
                   root.
  --mode host      Session 30's sweep with ONE new flag, `--candidate-
                   manifest`, and `--host` now also exported. It answers for
                   THIS SESSION ALONE: 30 swept its own trip, so there is no
                   earlier half owed.
                   ...
                   path the reading says it measured -- not at
                   /var/lib/docker, which is the assumption D1611 took out of
                   the product.
                   `telemetry_read`: the project's own Prometheus answers for
                   the first time in this product's life, read inside its
                   container because it publishes no port, and every series it
                   returns must name this project.
                   **None of those eight proofs has ever executed.**
                   **`--redeploy-before-file` is given again**, so
                   `deployment_convergence` and `stage_release` move. **No
                   migration is applied by this release's deploy** -- read the
                   ledger and expect it unchanged -- but **every container is
                   recreated**, because every service definition moved.
                   Needs root, host.yaml in the checkout, and the declarations
                   for every claim that is about something that happened
                   elsewhere.
                   (What follows is Session 30's text, kept because its claims
                   are inherited and its flags are still passed.)
                   Session 28's sweep with ONE flag added, and it answers for
                   ...
                   **No migration is applied by that deploy** -- read the ledger
                   ...
                   happened elsewhere.
  --mode external  What the public internet reaches. Session 31 adds no external
                   claim -- it ships no service, no port and no route; its six
                   offline claims are about a checkout and its four host claims
                   are about the machine itself, which is the opposite of what
                   external mode looks at -- but the inherited ones still need
                   it, because claims_through_session(31) is cumulative. MUST
                   run from a network that is not the deployment host.
