#!/usr/bin/env bash
#
# The Session 6 gate. It does not replace bin/session-01-check.sh, which must
...
#
# Session 6's evidence has two halves and cannot be written from one. Three of
...
usage() {
  cat <<'USAGE'
Usage: bin/session-06-check.sh --mode offline
...
")" \
  ...
  the scan then fails to find what it planted.
       bin/session-06-check.sh --mode external --public-ipv4 ADDR \
  ...
  --rotated-jwt-from-file FILE
                        ...
                        Session 6 does NOT rotate the signing key -- ADR 0088's
...
Each mode writes one evidence half. Session 6 needs BOTH -- three of its claims
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      ...
      --ssh-destination=*)
        ...
        ;;
      --baseline-only)
        die 2 "--baseline-only belongs to bin/session-02-check.sh; Session 6 measures a deployed identity plane."
        ;;
...
# editing this script.
#
# Session 6 has more of these than any session before it: four of its seven
...
mode_offline() {
  ...
  printf '\n\033[1msession-06-check: offline PASSED\033[0m\n'
}
...
mode_host() {
  ...
  if [ -z "${ADMIN_PASSWORD_FILE}" ]; then
    printf 'session-06-check: no --admin-password-file, so the proofs needing an\n'
  ...
  if ! evidence_is_supportable; then
    announce_no_evidence
    printf '\n\033[1msession-06-check: host PASSED\033[0m\n'
    return 0
  fi
  ...
  printf '\n\033[1msession-06-check: host PASSED\033[0m\n'
  printf 'This is one half. Session 6 also needs --mode external; see --help.\n'
}
...
mode_external() {
  ...
  printf 'session-06-check: this mode is only meaningful from a network that is\n'
  ...
  if ! evidence_is_supportable; then
    announce_no_evidence
    printf '\n\033[1msession-06-check: external PASSED\033[0m\n'
    return 0
  fi
  ...
  printf '\n\033[1msession-06-check: external PASSED\033[0m\n'
  printf 'This is one half. Session 6 also needs --mode host; see --help.\n'
}
