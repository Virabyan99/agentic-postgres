#!/usr/bin/env bash
#
# The Session 23 gate: `apg generate`, a typed TypeScript client over the
# surface a project publishes, and the claim such a client makes about it
# (ADR 0204). It does not replace bin/session-01-check.sh, which must still
# exit 0, nor the Session 2-22 gates, which still own their sessions.
#
# **Derived from bin/session-22-check.sh by diff, not retyped** (D505, D507,
# D678, D693, D703, D1108, D1109). `readonly SESSION=23` is the only session
...
#
# **This gate's session closes in OFFLINE mode too, and the split is not the
# same one.** Session 22's four offline claims were about a command with no
# deployment to be measured against at all. Session 23 HAS a deployment to be
# measured against -- a client meets a surface exactly there -- and two of its
# four claims are declared offline anyway, because what they are about is the
# ARTEFACT and its generator: what the IR reads, what the emitter writes, how
# a version is derived, what the command refuses, and whether the emitted
# JavaScript computes the digest Python computes. Every one of those is settled
# by four committed files and a container.
#
# The other two were deliberately NOT folded in, and that is the line ADR 0202
# exists to let a session draw. A checkout cannot say whether a DEPLOYMENT
# serves the surface a client claims -- PostgREST serves a document per role,
# so the question is about a running service and a caller's identity -- nor
# which lock a RUNNING plane loaded, which is the fact Session 21's trip spent
# eight minutes not knowing (D1152). `generated_client_hash` and
# `agent_lock_reported` are `not_run` at this session's close, by design, and
...
# what happened.
#
#   --mode offline    a checkout that produces evidence: everything Session 22
#                     checked, plus Session 23's own offline halves -- what the
#                     IR reads and refuses (test_client_ir), what the emitter
#                     writes and may never write (test_client_typescript), the
#                     command's refusals and the drift check
#                     (test_generate_command), the hash-locked toolchain and
#                     the JavaScript canonical form
#                     (test_generated_client_toolchain), the client against a
#                     surface a real PostgREST serves
#                     (test_generated_client_runtime), and the generation times
#                     (test_capacity_envelope). Step 6 checks the committed
#                     example client is current; step 8b typechecks it on the
#                     pinned toolchain. **Requires docker**, as Session 22's
#                     does and for more reasons. No host, no root.
#   --mode host       the deployment: unchanged from Session 22 except that the
#                     sweep now also collects Session 23's two host claims --
#                     `generated_client_hash` (the committed example client
#                     runs in the toolchain image against beta and answers
#                     `ok`, and against alpha answers `stale_contract` naming
#                     both digests) and `agent_lock_reported` (`list_resources`
#                     on beta reports the lock the plane confirmed). **No new
#                     flag**: both read declarations this gate already takes.
#                     **The sweep needs docker**, which it has -- the doctor and
#                     every Session 18 rehearsal use it as root. Session 23
#                     makes no trip, so this mode is Session 24's to run; until
#                     then those two claims are `not_run`.
#   --mode external   a different network: what a stranger reaches. Session 23
#                     adds no external claim of its own -- a generated client is
#                     a file, and a file publishes nothing -- and still needs
#                     this mode for the cumulative reason below.
#
# **Session 23's own claims are offline and host.** The gate still has three
# modes because `claims_through_session(23)` is CUMULATIVE: a Session 23
...
# offline claims (ADR 0202). **Session 22's three halves are still owed too**,
# and are a separate merge into evidence/session-22.json.
#
...
# (D1133, reversing D1021's reading). `claims_through_session(23)` runs Session
...
#
#   1. **Docker running**, and this gate needs it for more than Session 22's
#      did. Step 8 stands a cluster up; step 8b builds the toolchain image and
#      typechecks the committed client; step 3's sweep runs the runtime module,
#      which stands up a cluster, a PostgREST configured from compose.yaml's own
#      block and the pinned Traefik. Without a daemon all of it SKIPS, and a
#      skip is not a pass -- `dev_environment`, `generated_client` and
#      `generated_client_toolchain` would each report `not_run` and this gate
#      would exit 5 having written a half that proves less than it looks like.
#      So the daemon is checked up front and its absence is exit 3, not a skip.
...
#
#   1. Both projects deployed `--through-session 23`, which publishes outputs
#      **v18** -- unchanged, because Session 23 moves no outputs schema and adds
#      NO released migration and NO project migration. What a deploy through 23
#      does is RECREATE the auth/mcp container, whose mounted lock digest moved
#      (ADR 0155) -- and that is what makes `list_resources` answer with its
#      `lock` member at all. **Read the container, never the file** (D1152,
...
USAGE
  ...
  --mode offline   Contracts, schemas and models, plus Session 23's own offline
                   halves: what the IR reads and refuses, what the emitter
                   writes and may never write, the command's refusals, the
                   hash-locked toolchain and the JavaScript canonical form, the
                   client against a surface a real PostgREST serves, and the
                   generation times with their contract sizes. Step 6 checks
                   the committed example client is CURRENT and step 8b
                   typechecks it on the pinned toolchain with no network; step
                   9 WRITES A HALF -- evidence/session-23-offline.json. The two
                   claims it reports are `generated_client` and
                   `generated_client_toolchain`. Requires docker. No host, no
                   root.
  --mode host      Session 22's sweep, unchanged, plus Session 23's two host
                   claims. `generated_client_hash`: the committed example
                   client runs in the toolchain image against beta and answers
                   `ok`, and against alpha answers `stale_contract` naming both
                   digests. `agent_lock_reported`: `list_resources` on beta
                   reports the lock the running plane confirmed, with alpha as
                   the control. **No new flag** --
                   both read declarations this gate already takes,
                   `--project-a-outputs`, `--project-b-outputs` and
                   `--admin-password-file`. **The sweep runs docker**, which it
                   has. Session 23 makes no trip, so this mode is Session 24's
                   to run; until then those two claims are `not_run`. Needs
                   root, host.yaml in the checkout, and the declarations for
                   every claim that is about something that happened elsewhere.
  --mode external  What the public internet reaches. Session 23 adds no external
                   claim -- a generated client is a file an adopter holds, and a
                   file publishes nothing -- but the inherited ones still need
                   it, because claims_through_session(23) is cumulative. MUST
...
Run bin/lock-versions.sh --check."
  ...
  # Session 22 introduced this step (ADR 0202) and Session 23 keeps it. TWO of
  # this session's four claims are declared offline -- `generated_client` and
  # `generated_client_toolchain` -- and the two that are not are the reason the
  # declaration is worth having: a checkout cannot say what a deployment serves
  # or which lock a plane loaded, and folding those in would report eight
  # minutes of beta serving the wrong lock as green (D1152). The half is
  # written from step 3's JUnit -- the run that selected everything -- for the
  # reason every other half is: a verdict computed from a differently-selected
  # run is a verdict about a different collection.
  #
  ...
  if evidence_is_supportable; then
    ...
    printf 'Session 22 still owes its own three halves; that is a separate merge.\n'
  else
    announce_no_evidence
  fi
