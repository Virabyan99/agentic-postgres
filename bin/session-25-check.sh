#!/usr/bin/env bash
#
# The Session 25 gate: the hardening pass over the three developer surfaces, the
# instrument the outsider's second walk is measured by, and the Stage 3 release
# (ADR 0207). It does not replace bin/session-01-check.sh, which must still exit
# 0, nor the Session 2-24 gates, which still own their sessions.
#
# **Derived from bin/session-24-check.sh by diff, not retyped** (D505, D507,
# D678, D693, D703, D1108, D1109). `readonly SESSION=25` is the only session
...
#
# **This session ships no plane.** There is no new service, no new Compose
# profile, no released migration and no schema move anywhere: two operator
# verbs (`apg completion`, `apg dx-record`), one environment variable the
# dispatcher reads (`APG_PROJECT`), one optional generated document per project,
# and a great deal of measurement of what the last four sessions built. ADR 0162
# prices it a MINOR, `VERSION` moves to 1.6.0, and Run 7's `upgrade plan` on the
# host is what confirms the price -- a `major` required there is a stop
# condition rather than a number to write down.
#
# **THREE of this session's four claims are declared offline**, which is one
# more than any previous session has declared. `dx_context` is what the
# developer surface gained -- a context variable the dispatcher applies by
# derivation from the verb's own `--help`, and a completion script that embeds
# no verb and no flag. `dx_walk_instrument` is what the walk is measured BY: the
# record reader and the documentation the walker reads. `dx_hardening` is the
# matrix that holds all three DX surfaces against every security invariant this
# stage declares, and names a collectible node id for each invariant on each
# surface rather than asserting the coverage in prose. Every proof behind the
# three runs in a checkout: a dispatcher is a shell script, a completion is a
# bash session the test starts, a record is a JSON file, a document is a file in
# this tree, and the hardening scan reads sources and a `.generated/.dev/<key>`
# this checkout builds with Docker.
#
# **`stage_release` is a HOST claim and is deliberately NOT declared.** The
# offline half of `REL-STAGE-001` says the tree names one release consistently
# and prices it. Only a deployment can say whether it RUNS that release and
# whether upgrading to it costs the operator anything -- and declaring it
# offline would let the tree grade its own release. It is `not_run` at this
# session's close, by design, and the trip collects it.
#
# **--mode host answers for Session 25 alone.** Session 24's trip paid Sessions
# 22, 23 and 24 from one sweep (D1244) and those three merges are done; this
# gate's host mode has one session's half to write. What it also carries, and
# what makes this trip worth making, are TWO STANDING CLAIMS FROM SESSION 12
# that this sweep is expected to MOVE: `documented_path` on the record the
# second walk writes (`--dx-record-file`), and `fresh_host` on the outsider's
# deployed document if the operator locates it (`--fresh-host-outputs`). Both
# flags have existed in every gate since 12 and neither has ever been given.
# `documented_path` is ANSWERED and the answer is no; if the walk's record is
# not clean the claim is reported `failed`, which is a different thing from
# `not_run` and is never softened into one.
#
...
# what happened.
#
#   --mode offline    a checkout that produces evidence: everything Session 24
#                     checked, plus Session 25's own offline halves -- the
#                     dispatcher's context variable (test_apg_dispatcher), the
#                     completion script sourced in a real bash
#                     (test_completion_command), the walk record's readers
#                     (test_dx_record, test_dx_record_command), the
#                     documentation an adopter reads
#                     (test_session12_documented_path,
#                     test_documentation_index, test_mcp_catalog), the release's
#                     own account of itself (test_release_contract), and the
#                     48-cell hardening matrix (test_dx_surfaces_hardening).
#                     Step 3's sweep carries every one of them by their marks --
#                     nothing here is selected by name that the sweep does not
#                     already collect. Step 7 gains one drift check the release
#                     catalog's has had since Run 4: a project's OWN tool
#                     catalog. **Requires docker**, as Session 24's does and for
#                     the same reasons plus one: the hardening scan builds a
#                     `.generated/.dev/<key>` and reads what is in it. No host,
#                     no root.
#   --mode host       the deployment, and it answers for THIS session and the
#                     two standing Session 12 claims above. `stage_release`:
#                     both projects' deployed documents at `template_version`
#                     1.6.0 and `deployed_through_session` 25, `upgrade verify`
#                     exiting 0 for each against the candidate this checkout
#                     renders from that deployment's own installed manifest, and
#                     `upgrade plan --json` on alpha pricing that candidate at
#                     the class the constant's comment proposes. **No new flag**:
#                     the release proof reads documents and runs `upgrade`, and
#                     `--dx-record-file` and `--fresh-host-outputs` have existed
#                     since Session 12. **The sweep needs docker**, which it has.
#   --mode external   a different network: what a stranger reaches. Session 25
...
#
# **Session 25's own claims are offline and host.** The gate still has three
# modes because `claims_through_session(25)` is CUMULATIVE: a Session 25
...
# (D1133, reversing D1021's reading). `claims_through_session(25)` runs Session
...
#
#   1. **Docker running**, and this gate needs it for everything Session 24's
#      did plus the hardening scan. Step 8 stands a cluster up; step 8b builds
#      the toolchain image and typechecks the committed client; step 3's sweep
#      runs two runtime modules that each stand up a cluster, a PostgREST
#      configured from compose.yaml's own block and the pinned Traefik, and the
#      hardening module builds a dev environment to scan what it wrote. Without
#      a daemon all of it SKIPS, and a skip is not a pass -- `dev_environment`,
#      `generated_client`, `generated_client_toolchain`, `studio_boundary`,
#      `studio_surface` and `dx_hardening` would each report `not_run` and this
#      gate would exit 5 having written a half that proves less than it looks
#      like. So the daemon is checked up front and its absence is exit 3, not a
#      skip.
...
#
#   1. Both projects deployed `--through-session 25`, which publishes outputs
#      **v18** -- unchanged, because Session 25 moves no outputs schema, no
#      project migration and no released migration. **No ledger moves on either
#      project**, which is the first deploy in four sessions of which that is
#      true, and it is what makes `stage_release`'s reading simple: the document
#      is stamped and the artefacts are the ones already running. Read the
#      ledger anyway rather than the migrator's line (D941), and read the
#      container rather than the lock file (D1152, D1153).
...
#      `documented_path` (Session 12), and this is the sweep those two flags
#      were written for; **`--removed-project-file`** admits `project_removal`
#      (Session 17); **`--admin-password-file` is required** for every `IDN-*`,
#      Session 16, Session 21 and Session 22 live proof.
...
USAGE
  ...
  --mode offline   Contracts, schemas and models, plus Session 25's own offline
                   halves: the dispatcher's context variable, the completion
                   script sourced in a real bash, the walk record's readers, the
                   documentation an adopter reads, the release's own account of
                   itself, and the 48-cell hardening matrix over the three DX
                   surfaces. Step 3's sweep carries every one of them by their
                   marks. Step 6 checks the committed example client is CURRENT
                   -- this release's bump moves `templateVersion` in it (D1238);
                   step 7 checks the project's own tool catalog beside the
                   registry's documents; step 8b typechecks the client on the
                   pinned toolchain with no network; step 8c asserts Studio
                   ships no third-party code; step 9 WRITES A HALF --
                   evidence/session-25-offline.json. The THREE claims it reports
                   are `dx_context`, `dx_walk_instrument` and `dx_hardening`.
                   Requires docker. No host, no root.
  --mode host      Session 24's sweep, unchanged in arguments, and it answers
                   for THIS SESSION ALONE: Session 24's trip paid 22, 23 and 24
                   from one sweep (D1244) and those merges are done.
                   `stage_release`: both projects' deployed documents at this
                   release's `template_version` and `deployed_through_session`,
                   `upgrade verify` exiting 0 for each against the candidate
                   this checkout renders from that deployment's own installed
                   manifest, and `upgrade plan --json` on alpha pricing that
                   candidate at the class the constant's comment proposes.
                   **This sweep is also expected to MOVE TWO STANDING CLAIMS
                   from Session 12**, and they are the reason the trip is worth
                   making: `documented_path` on the record the second walk
                   writes (`--dx-record-file`) and `fresh_host` on the
                   outsider's deployed document if it is located
                   (`--fresh-host-outputs`). Both flags have existed in every
                   gate since 12 and neither has ever been given. A record that
                   is not clean makes `documented_path` `failed`, which is
                   reported as what it is. **No new flag** -- the release proof
                   reads documents and runs `upgrade`. **The sweep runs
                   docker**, which it has. Needs root, host.yaml in the
                   checkout, and the declarations for every claim that is about
                   something that happened elsewhere.
  --mode external  What the public internet reaches. Session 25 adds no external
                   claim -- it ships no service, no port and no route, and its
                   three offline claims are about a checkout -- but the
                   inherited ones still need it, because
                   claims_through_session(25) is cumulative. MUST run from a
...
Run bin/lock-versions.sh --check."
  ...
  # Session 22 introduced this step (ADR 0202) and Session 25 keeps it. THREE of
