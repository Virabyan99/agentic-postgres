...
| `AGT-BUDGET-001` | 8 | Row and response-size budgets are enforced server-side and are independent numbers: the row ceiling is the lock's and a caller's limit can only lower it, and the byte ceiling is not expressible by a caller at all. A result exceeding either is refused rather than truncated, because a truncated page that does not say so is a wrong answer. Elapsed time and concurrency are Run 8's and are deliberately not claimed here. |
