...
>
> **Outstanding: the rest of the host sequence.** The restart matrix passed on
> its own and both projects are redeployed at this commit with their allocations
> intact. The reboot, the rotation and both evidence halves remain, and the
> external half needs a machine that is not the host.
