...
*Host, and off-host for the negative proof. The first run that changes what is reachable.* **Blocked on D115.**

> **Reached:** Runs 4–8 transported, host gate green at its own commit, `provision-host.sh --apply` clean with zero deviations, both Session 4 secrets seeded, materialization writing ten files per project, Project A deployed through session 4 with `CONNECTION LIMIT 42` computed from the running server, migration 0006 applied, and the pooler healthy in transaction mode with `max_prepared_statements 100`.
>
> **Blocked at:** the publication. Docker accepts a `ports:` entry for a container on an `internal: true` network, records `HostConfig.PortBindings`, and installs no DNAT rule and no listener (D115). Nothing is reachable; the allocation is `reserved`; no document reports ready.
>
> **Three candidate resolutions, none taken.**
>
> 1. **Attach the pooler to a non-internal network.** Cheapest, and it trades a Session 2 isolation property for a Session 4 convenience: `internal: true` is what `DBX-PG-002` and `SEC-NET-001` rest on, and the container would gain outbound reachability it has never had. If taken, the negative proofs have to be re-derived rather than inherited.
> 2. **A forwarder that straddles.** A minimal process on a non-internal network forwarding to the pooler. Keeps the cluster's network internal and adds a component in the credential path, which is the place this session has been most careful about adding components.
> 3. **No host publication at all.** The host is the gateway for the internal bridge, so `ssh -L 127.0.0.1:15432:172.23.0.3:6432` reaches the pooler with nothing published. Strictly the safest — there is no host port to scan, and `SEC-DBX-001` becomes trivially true — and it costs ADR 0042 its allocated ports, because a container IP is not stable across recreation and is not the identity the volume carries.
>
> **Three defects found by running it**, all recorded: D112 (the transport blocks had readers and no writer), D113 (the identity query read the maintenance database), D114 (a request read as a measurement, by me, in this table).
