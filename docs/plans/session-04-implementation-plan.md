...
| **D114** | *(§4.1, and `DBX-PORT-001` as written in Run 8)* A publication is verified from the host by reading what is listening. | **Nothing listens.** Session 2's `infra/host/daemon.json` sets `"userland-proxy": false`, so Docker implements a published port as an **iptables DNAT rule** and starts no host-side proxy process. `docker inspect` shows `HostConfig.PortBindings` bound to `127.0.0.1`, a TCP connect to that port completes — and `ss -lnt` shows nothing at all, on a host where the publication is entirely correct. | **The host-side check reads the binding and connects; it does not look for a socket.** `docker inspect` says which address the container port is bound to, which is the `0.0.0.0`-versus-loopback question ADR 0040 is about; a connect says something answers. Both, because the binding alone is configuration and the connect alone does not say on which address. The off-host scan is unchanged and is still the only proof of the negative. | **A check that could not pass** — ADR 0035's defect with its sign flipped, and it would have reported a correct publication as broken on every run of `DBX-PORT-001` forever. It also cost a stage of Run 9 to find, because the symptom is identical to "the override never reached Compose". Two things follow. `bin/database-ports.py`'s `bindable()` probe cannot see a DNAT-published port either, so a port another project already publishes looks free to it — the registry is what actually prevents a double allocation, and the probe is a secondary check against *non-Docker* listeners only. And the Session 2 baseline check "only 22, 80 and 443 listen on a public address" was never going to see a database publication in the first place. | no |
