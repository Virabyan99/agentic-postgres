...
| **D1060** | `migrate.sh render` on a project that is deployed here: *"no rendered document for alpha-dev at .generated/alpha-dev/outputs.json; the project was never deployed here"* (exit 4). | **Measured on the host after 1.0.1 was tagged.** `.generated/alpha-dev` is `drwx------ root root` after a root deploy, so `op` cannot traverse it; `ls` on the document answers `Permission denied`, not `No such file`. `.generated/beta-dev` is op-owned and the same command works. The project was deployed here and the reader could not look. | **Open.** ADR 0195's class, third live site, found by using the host rather than by the adopter's account. The repair is the same shape as D1050 and D1052 -- distinguish unreadable from absent -- and it lands after the tag, so it belongs to the next release rather than to a second tag. `sudo chown -R op:op .generated` is the standing workaround and CLAUDE.md already prescribes it. | An operator asking *"what would this apply?"* is told the project does not exist here. That is the answer most likely to be believed and acted on. | 0195 |
