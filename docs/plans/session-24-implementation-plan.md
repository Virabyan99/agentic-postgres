...
| **D1289** | ADR 0206's implementation, and D1288's row, which priced the repair at *"(a) a guard at freeze AND at release-manifest time … (b) give the two sets separate ordering spaces … (c) an idempotent re-stamp"* and called (a) *"cheap, and it would have caught this on a workstation"*. | **(a) and (c) were both measured and both fail.** (c) is dead twice over: the example project's `0001-note-embeddings.sql` is a bare `CREATE TABLE`, so a re-stamped set cannot be re-applied — and it would not help anyway, because beta's already-applied `20260914120001` row stays in `schema_migrations` and `max(applied)` does not move. (a) prevents recurrence and unblocks nothing, and the only stamping rule that satisfies both directions does not exist: a project set must stamp above the release's newest at freeze, while every later release migration must stamp above every applied project version, and those two demands climb past each other for any sequence of releases. **Superseding `0032` was measured and rejected on its cost, not its difficulty**: its SQL is re-runnable (`DROP FUNCTION` then `CREATE FUNCTION` of one signature), but `app_private.migration_ledger` is written `ON CONFLICT (version) DO NOTHING` and `migration_user` deliberately holds no privilege on it, so the retired row can never be removed — alpha would stand at 33 applied against 32 released **permanently**, which `diagnosis.migrations` reports as `WARN … ahead of this checkout`. | **(b), and it is smaller than it looked** (ADR 0206). The pinned dbmate 2.34.1 already takes `--migrations-table` and `--migrations-dir` as globals, and the model already passed the table through an environment value — so the release's directory, mount, table and invocation are **byte-identical** and a project with no set renders exactly what it rendered before. What is new is confined to a project that declares one: `migrations-project/`, a second mount, a second service, a second table, and a one-time manifest-driven move of existing rows. **`-e` was NOT used**: `bin/compose.sh` refuses it with `run` because it is how a credential enters a container through argv, so the second invocation is a second SERVICE whose entrypoint is byte-identical and is asserted to stay so. The ordering rule became a function so the proof could be handed the exact interleaving that refused beta. Battery **6/6**, 109 controls green. | **Two guards caught this run's own mistakes before anything left the workstation**, which is the argument for both of them: `COMPOSE_ENV_KEYS` is a closed roster and silently dropped a key added only to the dict, so the render refused with a named interpolation error; and `RENDERED_FILE_MODES` refused the new directory until it carried a stated mode with a reason (ADR 0154). The battery caught a third: mutation 5's *control* failed, which is a broken PAIRING rather than a survivor — a second new proof was dying on the same mutation and had to be re-paired rather than counted. | **0206**, 0198, 0154, D912, D1096, D1098, D1288 |
| **D1290** | ADR 0206 moves a project set's applied versions into `app_private.project_schema_migrations`. D979's rule: before repairing a function, grep every reader of it. | **Seven readers of `app_private.schema_migrations` outside the bootstrap, and they do not all want the same answer.** Two were caught by a live cluster proof — `bin/dev.py::applied_count`, which prints its number beside the count of PLANNED migrations and so said *"32 of 34"* on a cluster that had applied all thirty-four, and the cluster proof's own version list. **One was caught only by the grep, and it does not run on this workstation at all**: `tests/deployment/test_session22_plane.py` asserts the example project's `20260914120002` is present in `schema_migrations`, and that version is now recorded in the project table — it would have gone red in the host sweep with *"migration is not applied here"* about a migration that is applied, inside `agent_tenant_read`, one of the two Session 22 host claims this trip exists to pay. | **The three that answer *how far has this cluster been migrated* read both tables**: `applied_count` sums them, the cluster proof asserts the union is exactly the planned set AND that each set's versions are in its own table, and the live proof counts across both because a cluster that predates the move still has the row in the old place. The remaining four are **recorded and left**: `bin/restore.py` and `bin/restore-test.py` compare a source cluster to a restored one and compute both sides the same way, so their answer stays self-consistent and merely narrows to the release's set; `bin/session-09-check.sh` lists release versions for a release-plane check; `tests/contract/test_node_restore.py` records a fixture value. Each is a narrowing of what is covered, not a wrong answer, and widening the restore plane's reading is a decision about the restore plane. | **The one that mattered most could not have failed here.** Two of the seven were found by a proof going red, and the third — the one inside a claim the trip is for — by reading the grep. A repair that had stopped at "the failing test now passes" would have shipped it to the host, which is where the last two of this session's four defects were found. | D979, D1288, **0206** |
| **D1291** | ADR 0206's `dbmate-project` service, whose entrypoint is asserted **byte-identical** to `dbmate`'s — which means it reads `/run/secrets/migration_user_password` — and `secrets.required.yaml`, whose per-consumer layout exists so *"one service cannot read another's copy"*. | **The service read a secret nothing materialised for it, and only the host could say so.** Beta's deploy got further than it ever had — the move ran, `0032` **applied** (22.6 ms, the migration that had refused twice) — and then the project invocation died: `cat: can't open '/run/secrets/migration_user_password': No such file or directory`. **Every offline check passed**: `compose config` interpolates and validates a model whose secrets are not mounted, the render validated, the full p0 suite was 5,700 green, and CI was green on all three jobs. Nothing in the repository connected *a service reads `/run/secrets/<file>`* to *this contract materialises `<file>` for that service* — so the one thing that could fail was the one thing no check looked at. And it fails **after the cluster has been migrated**, which is D1096's order exactly, one plane up. | **A consumer entry, and the cost written down in the file's own terms**: a third copy of the migration password and a third file a rotation has to reach, taken for the reason the second was — a service may not be handed another's copy without losing the property the layout is paid for. **And the guard that would have caught it**, derived from `compose.yaml` rather than from a roster: every service whose definition mentions `/run/secrets/<file>` must have a compose consumer declaring that file. Battery 2/2 — mutation 1 removes the consumer and reproduces the host's failure on a workstation, mutation 2 makes the scan match nothing and is killed by the assertion that it found any reader at all. | **Byte-identical was the trap.** The guard that keeps the two entrypoints from drifting is what made the second service read a secret, and the same property made it easy to believe the service was fully specified because its twin was. A duplication check proves the two are the same; it cannot prove that what they both need exists twice. **And the consumer entry then went red in CI on a second guard**: `test_only_the_prisma_fixture_holds_the_migration_credential` is an EQUALITY over the services that hold schema authority, so a third holder cannot be added without the roster being edited deliberately — which is the whole point of writing it as an equality rather than a containment. Both failing jobs failed on that one test and nothing else (`1 failed, 5700 passed`). The roster is widened to the measured set under ADR 0206, which is a widening and not a weakening, and the test was made STRICTER in the same edit: its docstring's actual rule — that no other *fixture* may appear — was described and never asserted, so the equality would have admitted a new client fixture exactly as readily as a new migration service. **The miss was mine**: the grep that found the readers covered `src/` and `compose.yaml` and not `tests/`, which is where this one lived. | D1096, D1288, D979, **0206**, 0010 |
| **D1292** | ADR 0206's `project_migrations` keyword, threaded through `build_override` and `render_override` so a project that declares a set gets its own mount. | **Neither caller in `bin/deploy-project.py` passed it**, so it was `False` at both sites, no project mount was written into the override, and `dbmate-project` started with nothing at `/migrations-project`: `Error: could not find migrations directory /migrations-project`, on the host, after the release set had applied. **The second time in two fixes that a parameter reached its destination and not its source** — D1291 was the same shape one layer along. And the reasoning in the keyword's own docstring was wrong for the deploy layer: *"declared rather than derived from whether `migrations-project/` happens to exist"* is right at the library boundary, which is given a directory NAME and may run before anything is in it, and wrong at the deploy, whose question is whether the directory it is about to name as a bind SOURCE is there. | **Wired at both call sites, derived from the directory** by `_has_project_migrations`, and the docstring now says which layer decides what and why they differ. **Both directions guarded offline**, because only one was live: a wrong `False` is a service that cannot find its migrations, and a wrong `True` is a bind source that does not exist — which does not fail, because Docker creates it and dbmate reports a set with nothing in it (D463). The control asserts the absence rather than assuming it. | Three defects in this repair that no offline check could see and a host found in seconds: a secret a service reads, a roster of credential holders, a mount a service needs. All three are offline-checkable now. | D463, D1291, **0206** |
| **D1293** | Run 7's sheet, step 6: *"`bin/session-24-check.sh --mode offline` on the host as op … as a control that the checkout there is the one measured here"*. And the gate's `--help`: offline mode *"Requires docker. No host, no root."* | **`op` has no docker access, so the half cannot be written on the host by the account the trip uses.** Exit 3, with the gate naming all three places it needs a daemon. The refusal is right and the message is right — *"without a daemon all of it SKIPS, and a skipped proof makes the claim not_run rather than failing"*. It is the plan's expectation that was wrong, and not by accident: Docker access is root-equivalent, which is why `bin/compose.sh --runtime` requires root and why `apg-diag` exists as a separate read-only account (ADR 0071). *"No root"* means the mode does not REQUIRE root, not that an unprivileged account can supply what it needs. | **The half is written on the workstation**, at the same commit, which is where the three merges run anyway. **The host's control is the Session 1 gate**, which did run there as op at that commit: `GATE_01_EXIT=0`, 5,338 contract tests, 0 failed — and that gate is the stronger control for this question in any case, because it is the one that cannot complete on this workstation at all (D1239, PyPI). | Two accounts and three privilege levels, and the plan read a *"no root"* in `--help` as *"any account"*. The sentence is about what the mode requires, not about what it can be given. | 0071, D1239 |
| **D1294** | `tests/deployment/test_session24_studio.py` sets `MCP_ACCEPT = "application/json, text/event-stream"` and calls the agent plane with it. Session 8's module records what that buys: *"every reply `text/event-stream`, SSE-framed, even"* when it is one message. | **It asked for a stream and never parsed one.** The decoder was `json.loads(answer.body)` with a fallback of `{"error": {"message": f"HTTP {answer.status}: …"}}` — so every SUCCESSFUL reply failed to parse and was returned as an error carrying the text `HTTP 200:`. `refused()` reads an `error` key, so three proofs read a successful `list_resources` as a refusal, and the assertion that fired said *"the probe agent's request was refused before anything was revoked"* about a request that had succeeded. **A 200 relayed as a refusal is D433's shape inside a proof rather than a product.** Sessions 16, 21, 22 and 23 each carry the six-line `sse_result` decoder; this module carried the constant and not the parser. It survived because its claims are HOST claims: the sweep was its first execution anywhere. | **The decoder the other four use**, wrapped in `mcp_message`: SSE first, then plain JSON, then an error that names the status, so *"it answered something I cannot read"* stays distinguishable from *"it refused"* (ADR 0195). Verified against the exact body the host returned. **And the structural guard**, in a new `test_deployment_module_shape.py`: a deployment module that requests `text/event-stream` with no `data: ` decoder fails offline. Asserted as a PAIR rather than an implementation, so a module that decodes some other way passes and one that forgets cannot. | Copying a constant is not copying a behaviour, and the module that needed the behaviour was the one that could not run without a host. The guard answers *which proof should have caught this*: there was none, and there is now one covering every future module for free. | D433, D458, 0195 |
| **D1295** | `tests/deployment/test_session23_client.py`'s lock proof calls the agent plane with `token=AGENT_SECRET` — the value it just registered with `auth_create_agent` as the agent's hashed secret. | **The plane answered 401 and the proof read it as a plane that would not report its lock.** The agent plane verifies a JWT this project's auth application signed (`AgentTokenVerifier` over a `LocalKeySet`); an agent's SECRET is what you exchange at `POST /auth/agent-token` for one, and is not itself a bearer token. Sessions 22 and 24 both exchange first. This proof is `agent_lock_reported`'s, `not_run` since Session 23, so the sweep was its first execution and nothing had compared the three. | **The exchange, in the shape the other two use**, with `api_call` and `app_base` added to the proof's signature. | Two of the sweep's seven failures were proofs that had never run, failing on first execution, in the way §7 counts. The plane was right both times; both readings of it were wrong. | D298, D675, 0117 |
| **D1296** | **D1290's own judgement, and it was wrong.** That row split the seven readers of `app_private.schema_migrations`, fixed three, and left four — recording `bin/restore.py` and `bin/restore-test.py` as *"they compare a source cluster to a restored one and compute both sides the same way, so their answer stays self-consistent and merely narrows to the release's set"*. | **`schema_matches_the_release` compares the restored cluster to the RELEASE, not to a source**, and as a SET EQUALITY on purpose — the comment beside it says *"a restored cluster with the right NUMBER of migrations and a different set is a cluster restored from another release, and counting would report it healthy"*. `released_versions(document)` is every set the project applies, so for beta it is 34 while `schema_migrations` alone holds 32. The equality cannot hold for any project that declares a set, and beta's drill ended `restore-test: smoke checks failed: schema_matches_the_release`, exit 6. | **Both readers take the union.** `restore-test.py`'s `present` reads both tables ordered by version across them, matching the order `released_versions` returns; `restore.py`'s `schema_migration_count` counts both, so the record a restore leaves describes the whole cluster rather than the release's half. `schema_version` stays the newest RELEASE version and now says so. | **The reasoning was written down explicitly and was wrong**, and a fifteen-minute drill on a host corrected it. The error was reading *"compares both sides the same way"* into a function whose two sides are a cluster and a manifest. A row that says *left, and here is why* is worth more than one that says *fixed* — and it is also the one that has to be right. | D1290, D1288, **0206**, D145 |
| **D1297** | Run 6's live module: the revocation probe is an agent holding `AGENT_SCOPES = ("meta:read",)` that calls `list_resources`, and `AGT-AUDIT-002` then reads *"the boundary a running plane recorded"* for that same agent. §2's own words: *"the refusal's audit row VISIBLE in the viewer with its boundary"*. | **The probe could not produce an audit row at all, and that is a decision rather than an oversight.** `mcp_tools.py` says it in its first paragraph — *"Two of them reach nothing. `list_resources` and `describe_resource`"* answer from the loaded lock — and `AUDITED_KINDS` is `(KIND_READ, KIND_WRITE)`, so neither is audited. `meta:read` is the scope for exactly those two, so an agent holding only it can make no audited call. The sweep returned `rows: []` and the proof said *"the plane recorded no audit row at all for the revoked agent"* — which was true, and not because anything was wrong with the plane. **Two proofs of mine, written in one run, asking for incompatible things from one probe**: `STU-REVOKE-001` wanted the cheapest call that revocation stops, and `AGT-AUDIT-002` wanted a call the plane writes down. | **The probe gains `notes:read` and the fixture makes an audited call on each side of the revocation** — `query_resource` over the release's own `notes`, which is `KIND_READ` and therefore audited. Alpha's vocabulary carries the scope and alpha declares no project set, which is where these proofs run. Revocation stays proved against the meta call as well: it is the cheaper signal and it already passed. The audited pair is also what makes `denial_reason` mean something — a served row beside a refused one, rather than a column that always carries a value. | The plane's own module states the classification in its opening lines, and the proof was written against the tool name rather than against the kind. `AUDITED_KINDS` was one grep away, in the file the claim is about. | 0135, 0141, 0178, 0200, D483 |
| **D1298** | Run 6's live module indexes the audit row's identifier as `row["audit_id"]` on both sides -- the `GET /admin/audit` reader and the Studio relay -- and `AGT-AUDIT-002` compares the two. | **Neither reader publishes `audit_id`, and the table has no such column.** `GET /admin/audit` serialises `"id": str(row["id"])`, and Studio relays `payload["audit"]` unchanged -- which is itself half of what this proof checks. Migration 0019 declares `id uuid PRIMARY KEY`. The name was invented, and it reads plausibly enough that nothing in review caught it. | **Both sides index on `row["id"]`.** The `-k` loop returned it in 11 seconds rather than 17 minutes, and D1297's repair held on the same run: `studio_rows` is non-empty, so the audited probe does produce rows and the failure had moved on rather than repeated. | Question 6 -- the fixture shared no belief with the code because it was never compared to it. A `KeyError` in a fixture is D386's ERROR, not a kill: the proof never reached its assertion, so a red run here said nothing about the audit surface. Fourth first-execution failure in this module, and the first to cost seconds instead of a sweep. | 0135, D386, D816, D929, D1116, D1247 |
| **D1299** | `AGT-AUDIT-002` reads *"the boundary a running plane recorded"* for a refusal, and Run 6's probe produced that refusal by calling as a **revoked** agent. | **A revoked credential never reaches the audited path.** `agent_claims_are_current` returns NULL in the pre-request hook, which runs BEFORE `agent_audit_begin`: no row is written, and none should be. The sweep said so plainly -- one row, outcome `served` -- and the plane was right again. The refusal that IS audited is one the plane itself makes: the scope check runs AFTER the audit record is opened, which is what the released `test_..._is_audited_before_its_scope_is_checked` pins and what makes ADR 0141 true. | **The refused probe becomes a scope violation made while the credential is still good** -- `query_resource` over `tasks`, which alpha's vocabulary carries and the probe's scopes do not. The served and refused pair is what gives `denial_reason` its meaning; a column that always carried a value would be a constant. | Two refusals that look identical from the caller's side are made at different boundaries, and only one of them is downstream of the audit record. The credential boundary refuses before the plane has begun to write anything down -- which is the correct design and the reason the proof could never have passed. | 0135, 0141, 0178, D1297 |
| **D1300** | D1165 replaced `skipif(os.geteuid() == 0)` with a re-entry as the checkout owner, so the two root-branch proofs in `test_honest_readers.py` would finally run under the identity the gate actually uses. | **The re-entry could not reach what the proof was about.** The tree is built under root's pytest temporary directory, which is `0700`, so the unprivileged re-entry is refused at an ANCESTOR and never reaches the `0000` project directory the proof is about: the message says `Permission denied` on `.generated`, not on the project inside it. A bare `PermissionError` there is D1151's shape, and the proof correctly refused it as `permission_error` rather than the reader's own `evidence_error`. **The control was the better tell**: with the project directory restored to `0755` the helper should have read the document and the assertion should have refused the answer -- it did not raise, because the owner still could not pass the ancestors, so `unreadable` was still the answer, arrived at from the wrong directory. Both proofs were executing for the first time in any environment, and both failed for this one reason. | **One helper makes the ancestors traversable to that owner**, under root only and only beneath the temporary root. It is called from the `rendered` fixture and from the second proof, which builds its own tree from `tmp_path` -- the first fix reached only the fixture and left that one still red (23 passed, 1 failed). Two places needing the same thing is one function, not two copies; the copy is what drifts when the next proof grows its own tree. Spans commits `3724d0b` and `f11686e`. | Question 2, and the shape it keeps taking: a proof that had never executed, written against an environment that is not the one it runs in. A `0000` directory proves nothing about a reader if an ancestor already refused the walk -- the measurement was of the wrong directory, and it agreed with the expected answer. | 0195, D1151, D1165, D1240 |
| **D1301** | D1300's helper walks up from the leaf `while probe.is_relative_to(temporary)`, chmodding each ancestor to `0755` as root. | **A path is relative to ITSELF**, so the loop did not stop at the temporary root: it walked onto `/tmp` and chmodded it to `0755` as root **on the deployment host**. Sticky bit and world-write gone, and every unprivileged write to `/tmp` refused. Found because the next bundle `scp` was denied; the operator remediated it with `sudo chmod 1777 /tmp`. Nothing was destroyed and nothing was exposed -- `/tmp` became less permissive, not more -- but a hardening regression on a live host caused by test code is not a defect to discover, it is one to not write. | **The loop stops at the temporary root by NAME** rather than by the shape of the condition, and the walk is proved to stop rather than assumed to. Recorded with it, because it outlives this instance: this helper chmods as root on paths derived at runtime, and a fixture holding that capability needs an explicit bound. It now has one. | My own repair, one commit old. The bound was expressed as a property that is accidentally true of the boundary case instead of as the boundary itself -- a loop that asks only *"am I still inside X"* includes X. This is D600 and D918's rule in a fixture: guard the class against the definition, never against the case that failed. | 0154, D600, D918, D1233, D1300 |
| **D1302** | §7's table: `honest_readers` is *"`passed` for the first time in both halves"*, its offline half being *"Session 22's D1165 repair, now executed as root"*; `evidence/session-24.json` at a `not_run` count of **8** -- *"Session 21's nine less `honest_readers`. A ninth is a finding, not a footnote."* | **The sweep returned nine, and the one skip is not in the module D1165 repaired.** `claim_result` reports `not_run` on a node id that is missing OR skipped, and exactly one of the claim's 23 is skipped: `tests/contract/test_render_atomicity.py::test_the_owner_is_resolved_upward_when_the_path_itself_cannot_answer`, which carries a bare `if os.geteuid() == 0: pytest.skip(...)` at line 396. Root stats through a `0000` directory, so the walk-up branch cannot be entered and the proof skips on **every gate that could record it**. `test_honest_readers.py` states the rule in `_as_checkout_owner`'s own docstring -- *"A proof that skips under the identity the gate runs as is a proof the gate can never record (D1121, both halves)"* -- and applies the `sudo -n -u #uid` re-entry at three sites. The claim's third root-branch proof lives in another module and never got it. | **Recorded, not repaired, and the repair is named**: the reading is made as the checkout's owner through the mechanism that already exists, which means lifting `_as_checkout_owner` out of `test_honest_readers.py` into something both modules import. It cannot be demonstrated offline -- the skip only happens under root -- so it costs a second 15-minute host sweep, which is a trip decision rather than a run decision. Sessions 20, 21 and 24 have each closed with this claim `not_run`; the count went 3 skips, 2, 1. **The session closes with nine, and says so.** | Question 5, in its sharpest form here: the decision was implemented, the rule was WRITTEN DOWN in the claim's own module, and the grep that would have found the third proof is the claim's own node-id list -- the registry names all 23 across four modules, and D1165 moved the two in the module it happened to be reading. Question 2 as well: this proof has never executed in host mode in any session. **And a process note, mine**: I first reported this as *14 missing node ids*, which was an artefact of reconstructing node ids by hand against an empty directory; the product's own `junit_key`/`claim_result` answered it in one call. The reader existed and I wrote a second one. | 0163, D1060, D1116, D1121, D1131, D1155, D1165, D1240 |
...
### Run 8 — the ordering space, so beta can be deployed at all

**Not in the original plan.** Run 7 stopped at beta on D1288, and the trip
cannot produce evidence with one project two sessions behind — so this run is
the repair the next host visit depends on, offline, on the branch.

**What it measures first**, because D1288's row priced three repairs and two of
them turn out not to work: whether the example project's set can be re-stamped
(no — `0001-note-embeddings.sql` is a bare `CREATE TABLE`, and beta's applied
row keeps `max(applied)` where it is regardless); whether superseding `0032`
is affordable (it works, and costs alpha a permanent `WARN` because
`migration_ledger` is `ON CONFLICT DO NOTHING` and `migration_user` may not
delete from it); and whether the pinned dbmate can separate the two sets at all
(it can — 2.34.1 takes `--migrations-table` and `--migrations-dir` as globals).

**What it builds** (ADR 0206): each set rendered into its own directory and
applied against its own table, with the release's path byte-identical so a
project that declares no set is untouched and alpha needs nothing. A second
`dbmate-project` service rather than an overridden one, because `bin/compose.sh`
refuses `-e` with `run` and that refusal is a secret boundary, not an
inconvenience — with the two entrypoints asserted byte-identical. A one-time,
re-runnable, manifest-driven move of a project set's already-applied versions
out of `schema_migrations`, issued as the superuser before either invocation:
that is the repair beta needs, and it re-applies nothing.

**Done.** 2026-09-13. ADR 0206 accepted and indexed. The render splits, the
ordering rule became `assert_migration_order` so the proof could be handed the
exact interleaving that refused beta, `apg dev` reads each payload from its own
set's directory and records into its own table (ADR 0203's one code path kept),
and the bootstrap creates the project table with the same owner and the same
three grants as the release's. Seven proofs, **battery 6/6 first pass with 109
controls green** — mutation 1 is D1288 itself restored, and mutation 5's
apparent survivor was a broken PAIRING (a second new proof dying on the same
mutation), re-paired rather than counted. Two released guards caught this run's
own mistakes before anything left the workstation: `COMPOSE_ENV_KEYS` dropped a
key added only to the dict and the render refused by name, and
