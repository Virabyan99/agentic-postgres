...
`main`. No run has started. §1 is D1243–D1258 (planning rows, each read from
the tree at `035192d` today or named as the measurement Run 1 owes); the runs
add theirs below the planning rows, so next free after planning is **D1259**.
...
Six columns, next free number after this table **D1259**. Rows D1243–D1258
...
**Done.** *(filled by the run: the three rigs' numbers, the uvicorn port
attribute, the `created_at` resolution, PostgREST's CORS header verbatim, the
JWKS form that worked, the rig's wall time, and every place the tree
disagreed with §0.)*
