...
| **D1696** | 7 | §1 D1667: *"Each step attempt mints a `request_id` (uuid4), stores it on the step row at claim, and sends it as `X-Request-Id`; correlation is `workflow_step.request_id = agent_audit.request_id`"* -- premised on *"the plane's audit row carries `request_id` from the `X-Request-Id` header the caller sends (`mcp_query.py:78`, `:94`)"*. §5 Run 7 item 3: the live correlation join returns 3. | **The plane never reads an inbound `X-Request-Id`, by decision.** `app/request_id.py`: *"Nothing in this module reads an inbound `X-Request-Id`"* -- `StampRequestId` mints one id per HTTP request, `mcp_authorization.current_request_id` returns THAT id to the audit writer, and it goes out on the RESPONSE (ADR 0160: *no caller value is ever trusted*). The lines D1667 cited are the plane forwarding its OWN id to PostgREST. So the step carried the claim's id, the audit carried the plane's, and the join would have returned **zero** on the first real deployment. Rig 32j never saw it: its plane was a fake that recorded the header the loop sent -- a fixture sharing the code's belief (§7 question 6). Found by writing the live proof, reading `mcp_query.FORWARDED_HEADERS`' comment on the way. | **0034 amended while still unapplied anywhere** (D1686/D1687's window, D912 governs an applied migration): the claim CLEARS `request_id` and no longer returns one (D1686's column withdrawn with its premise); `workflow_finish_step` gains `p_request_id uuid` sixth and `workflow_park` fifth, both recording it. The loop sends no request-id header, reads the RESPONSE's, parses it as a uuid, and passes it on every finish and park -- `NULL` where no call was made (refused mint, unresolvable reference, spent margin, transport failure) and where the header is not a uuid (recorded as *no id* rather than costing the step its outcome, ADR 0195). ADR 0227 gains an *Amendment, Run 7* section. `test_the_claim_returns_the_request_id_it_minted` is REPLACED by `test_the_step_records_the_planes_request_id_and_a_claim_clears_it` (three arms: claim clears, park records, re-claim clears, finish records); `test_the_call_carries_the_key_the_request_id_and_the_token` by `..._and_the_token_and_sends_no_request_id` (with an AST arm over `_send`'s header dict), plus `test_the_step_records_the_request_id_the_plane_returned` with three `None` controls. **Battery: four kills, four controls green**, restored byte-identical, the lock re-frozen identical. |
| **D1697** | 7 | §5 Run 7 item 3: the constructed crash state enqueues `notes-roundtrip` *"with `input.title` = a canary"* and performs *"step 2's `create_note` ... with `idempotency_key = "wf-<run>-again"`"*, cleaning up *"every `app.notes` row whose title carries the proof's canary prefix"*. | **`notes-roundtrip` takes no input and carries no reference, deliberately** (its own header: *"a trip's run should fail for reasons about the substrate and not about what the operator typed"*), its step 2 is a READ, and no step is named `again`. Its titles are fixed, so every run writes the same two titles. | **The proof performs the LAST write** (`create_the_second_note`) with the key `wf-<run>-<name>` and the arguments READ FROM THE YAML -- the fingerprint is over them, and a retyped title would prove `PT412` instead of a replay -- and finds the row by the `row_id` the idempotency record holds, never by title. Cleanup is by the probe OWNER's id (the `_registered_subject` shape), which every run's rows carry. |
| **D1698** | 7 | §5 Run 7 item 3: *"alpha's 404 with `no_such_definition`"*. | The route's one 404 body is `{"error": "no_such_workflow"}` for a definition nobody installed, a missing run AND another agent's run (`workflow_routes.py`, ADR 0229's one-answer rule). There is no `no_such_definition` word anywhere in the surface. | The live proof asserts the body the route returns. |
| **D1699** | 7 | §1 D1662: the drill member is *"`{"count": n, "by_status": {…}, "definitions": n}`"*; §5 Run 7: *"assert ... `definitions == 2`"*. | **Run 6 built `{"value": <runs by status>, "reason": ""}`** from `workflow_counts() ->> 'runs'` (`bin/restore-test.py:496-513`), and D1691 carried exactly that into the evidence document. No count, no `by_status` key, no definitions. | **`REC-WF-001` compares the TOTAL** -- the drill's runs summed over statuses EQUAL the live runs created at or before the target (equality: more is a restore past its target, fewer is a lost run; statuses differ legitimately because the loop finishes runs after the target) -- and requires the live count positive, or it compares two zeros. Definitions are not asserted and the requirement says so. Widening the drill to carry them is a product change for a proof's sake; not taken. |
| **D1700** | 7 | §4: *"Every proof deletes its agents and notes in `finally` (the `mcp_writer_session` shape); runs are left as the substrate's record"*. | **Those two cannot both hold.** `workflow_run.agent_id REFERENCES app_private.agents (id)` with no `ON DELETE` (0034), so an agent with a run cannot be deleted -- and `agents.owner_id` holds its owner the same way (D392). The `mcp_writer_session` shape would fail its own teardown on the first sweep. | **The runs stay (they are the record `REC-WF-001` reads) and so do their agents, REVOKED at teardown** -- status `revoked`, `authz_version` bumped -- under names suffixed with a per-sweep random token so the next sweep's cannot collide with `agents_name_normalised_key`. Their owner stays because they reference it; its password was random and never written down, so it is a subject nobody can log in as. The notes and tasks go (by owner), and the revoking administrator is deleted -- it owns nothing. Alpha's probe agent enqueues nothing and IS deleted. **Each sweep leaves four revoked agents and one unusable subject on beta**; recorded in §10 rather than hidden. |
| **D1701** | 7 | §5 Run 7 item 3 and Sheet B4: *"`doctor capacity`'s ceilings line ... 2240 per key, 4480 total"*. | Those are this host's caps today. A proof pinning them fails the day any cap moves for a reason unrelated to the grouping D1636 repaired, and passes on a host where the grouping is wrong but the sum happens to match. | **`NODE-READ-003` compares each project's figure with an independent sum** of `HostConfig.Memory` over every running container carrying that project's compose label, requires the DATABASE container to be one of the summands with a non-zero cap (the whole of D1636), requires the figure under the project KEY (D1673's mapping) and the unbounded count to match. The 2240/4480 stay on Sheet B4 as what the operator should SEE, and the proof prints the numbers it measured. |
...
outputs.json --json` **with the declaration flags Run 7 measured: `--also
migration_added --also api_operation_added`** (D1703) → `bump minor`,
**`requires minor`**, `OK`; the same for `beta-dev`. Without the two flags it
reads `requires patch`, which is the floor the documents alone show and is
wrong in the reassuring direction (D1666). `docker ps --format '{{.ID}} {{.Names}}' --filter label=com.
...
and `/home/op/s32-r8-host-summary.txt` is the reading. **The sweep itself
kills beta's auth process once** -- `test_a_parked_run_survives_the_worker_
restart_rehearsal` runs `rehearse.sh worker-restart` under a parked run -- so
Sheet B4's rehearsal must be REVERSED and `doctor` 12 ok before this line, and
the restart count on beta's `auth` is then two of `on-failure:5`'s five. **While it runs**,
