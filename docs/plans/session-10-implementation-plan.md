...
**Next free number after this table is D606.**
...
| **D596** | `tests/recovery/test_future_pitr.py`: the drill seeds `app.notes (owner_id, title, body)`. | **`app.notes` has no `body` column and has not had one since migration 0007.** 0003 creates the table with `body text NOT NULL DEFAULT ''`; **0007 renames it to `content`**. Every one of the module's six proofs failed identically on the host — five as `ERROR at setup` through the shared `drill` fixture, and `test_wal_archiving_failure_is_visible` as a `FAILED` inside its own `try` — with `ERROR: column "body" of relation "notes" does not exist`. | The column is **dropped from the INSERT** rather than renamed: the drill needs two rows either side of a timestamp, `owner_id` and `title` are sufficient, and `content` keeps its `NOT NULL DEFAULT ''` through the rename. A column the test does not name cannot be renamed out from under it again. | **Five of these six proofs were placeholders until Run 9 and had NEVER EXECUTED.** §8's oldest unbuilt item — *nothing knows which proofs have never run* — for the fourth and fifth time, and Session 9's trip already paid for it three times in one gate. The fact was available offline the whole time: `grep -n 'app.notes' migrations/templates/*.sql` names 0007's rename on one line. What was actually done was read the `CREATE TABLE` in 0003 and stop, which is §6's *does the proof ask for what the artifact itself asks for* — the artifact here is the **released migration set**, not the table's first definition. **The repair direction matters more than the repair**: renaming `body` to `content` would have restored the same coupling and the same silent expiry date. | — |

| **D597** | ADR 0153: "`target` is the full container path, and one function decides it." | **The credential was mounted, correctly, where nothing opens it.** Five Session 4 transport proofs failed on the host with `no PGPASSFILE and /run/secrets/app_runtime_password is not readable` (exit 8) and `ENOENT ... '/run/secrets/migration_user_password'`. The resolved model was **right** — measured: `client-psql secrets: [{source: client-psql__app_runtime_password, target: /run/secrets/app_runtime_password}]`, with all five files present in generation `57f2f386…`. `tests/deployment/conftest.py` then built the mount as `f"{source}:/run/secrets/{filename}:ro"`, yielding **`/run/secrets//run/secrets/app_runtime_password`**. | **`secret_override.mount_target(entry)`** — the inverse of `build_secret_override`, in the module that writes the grant, absolute-aware and accepting Compose's short form too. The harness calls it instead of composing a path. A round-trip test asserts `mount_target(grant) == container_secret_path(consumer)` for **every** consumer in the contract, with a premise check that at least one lands outside `/run/secrets` — without which a reader that hard-codes the prefix would pass it (D509). | **§6 question 5, the one this project answers wrong most often**: *when a decision is implemented, which of its callers got it?* ADR 0153 made `target` absolute so a `pgbackrest` consumer could land in `/etc/pgbackrest/conf.d`; the writer moved, the schema moved, the writer's own contract test was **replaced by a stricter one**, and this reader did not. The failure is the project's signature shape — a value that looked measured and was not — with the extra cruelty that **the symptom is indistinguishable from broken materialization**, so the first two hypotheses (a stale grant surface, then a deploy wiping the override) were both about the deployment and both wrong. The mtime theory was refuted by re-running the five proofs with the override present: still red. `run_client_fixture`'s own docstring already recorded Run 9 making the *same* mistake in the *same* function — mounting the right file where nobody was looking — one session earlier. Battery R1–R2: 2/2 killed; R1 is this defect exactly. | — |

| **D598** | D592: "the deployed document has no `compose` block", repaired for `compose.project_name`. | **The same absent block was read twice more, in the same command.** `bin/restore-test.sh` refused every real invocation with `restore-test: the deployed document names no postgres volume, so nothing here can say which volume the drill must not touch. Refusing rather than guessing.` — exit 5, and with it five of `tests/recovery/`'s six proofs. `build_plan` read `compose.volumes.postgres` and, one line later, `compose.networks.backup`; only the RENDERED document publishes either. | **`naming.postgres_volume_name(key)`**, split out of `derive` as the third such name after `backup_network_name` and `compose_project_name`, and `naming.backup_network_name(key)` for the network. Both derived from the project key, which **both** document kinds carry. | **§6 question 5 for the third time in this session**, and this instance is the sharpest: D592 *was* the answer to this question, applied to the one field that had failed rather than to every field read out of the same absent block. A repair scoped to the symptom. The reason it survived offline is separately worth keeping: **107 contract tests covered the drill and not one of them drove `build_plan`** — every test constructed a `DrillPlan` directly with keyword defaults, so the function that reads the document had no offline caller at all (D498, a real coverage gap). The repair adds two tests that drive it from a **deployed-shaped** document with no `compose` key, plus a premise check that `deployed_output.py` still publishes none. Battery S1–S2: 2/2 killed, each arm restoring one of the two reads. | — |
| **D599** | `test_wal_archiving_failure_is_visible`: "the break is `ALTER SYSTEM` plus a reload, and whether `postgresql.auto.conf` overrides a `-c` option **has not been measured**." | **It does not. A command-line `-c` option overrides `postgresql.auto.conf`.** On the host the test set `archive_command = '/bin/false'`, reloaded, switched a segment — and `archived_count` climbed 26 → 28 with `failed_count` 0 and `last_failed_time` NULL on a cluster that was working perfectly. Rig 10 measured it directly against the locked image: `postgresql.auto.conf` **did** receive `archive_command = '/bin/false'`, while `SHOW archive_command` stayed `/bin/true` with `pg_settings.source = 'command line'`. `compose.yaml` passes `archive_command` with `-c`. | The lever becomes **`docker network disconnect <backup-network> <container>`**, repaired by `connect` in a `finally`. The detach's exit code is asserted, so an arm that breaks nothing cannot report on a state it failed to create (D557). | **The test failed in the direction its own docstring chose**, and that choice is the finding. It said: *if the precedence does not work this way, this fails loudly on a cluster that is fine — that is the failure direction to prefer, because the alternative is a test reporting a working signal after failing to break anything.* It did exactly that, and an hour of measurement turned an unmeasured assumption into a fact. **The rig's first control was defective and the rig caught it**: the control setting was also passed with `-c`, so it could not move either, and the script refused to report anything (D499 — when both go red, repair the control). The second control moved `default` → `configuration file` while the subject stayed on `command line`, which is what made the answer readable. The new lever is also **closer to what `REC-WAL-001` models**: an unreachable repository is what an expired credential, a DNS failure and an R2 outage all look like from the archiver's side. | — |
...
| **D604** | D511: "deliberately not repaired ... the honest repair is a client that finishes writing before it reads; it belongs to API-AUTH-002's owner." | **Repaired, because it cost two host-gate runs in one trip.** `urllib` discards a response it has already received when the write that follows fails, so `API-AUTH-002`'s live half reported status `0` against a deployment behaving exactly as designed — `ECONNRESET`, then `EPIPE`, then a pass, three runs at one release. | **ADR 0156.** `tests/deployment/oversized_request.py`: chunked write, stop as soon as the server has answered, and read **whether or not the write completed**. It never synthesises a status — a connection that broke with nothing received returns `0` and still fails. `api_call` is untouched; only that one request moves. | **The rig had to be repaired before the repair could be measured.** The first version's servers answered immediately and closed, and **every arm passed with the OLD client too**: 64 KiB fits entirely in loopback socket buffers, so `sendall` returned before the server said anything and there was no race to lose. Six green tests measuring nothing — **D374's shape, in the file written to prove a fix**. The condition needs a tiny `SO_RCVBUF` set before `listen` and a server that never drains the body. Two permanent arms now assert `urllib` **fails** there, so the rig cannot quietly stop reproducing the defect. **The battery's first three arms all survived and that was the useful part**: V1 (drop the readability check) and V3 (make a write error fatal) survive *separately* and **kill together** — two independent recovery routes, each sufficient, visible only from the combined arm. V2 survived because it mutated a branch the silent arm never reaches; **V2b**, on the exception branch it does reach, **kills**, which is what proves the D509 guard real. And the combined arm first reported *"control broke"* from a node-id parsing bug in the battery, not from the mutation — read the failures, never the parsed summary (D386). | — |

| **D605** | D604/ADR 0156's rig: a 512 KiB body blocks the client's write, so the server can answer mid-write. | **`SO_RCVBUF` is advisory, and the rig reproduced the defect on one kernel only.** The same three arms **passed on WSL2 and failed on the host** (Ubuntu 26.04): there the whole body landed in the socket buffers, `sendall` returned before the server had said anything, **`urllib` recovered the refusal**, and `test_the_current_client_loses_that_refusal` went red in both arms — correctly. A repair for a flaky *host* proof had become a **deterministically red contract suite**, which is strictly worse than what it replaced. | The payload is **derived from what the kernel actually granted** (`getsockopt(SO_RCVBUF)` after the request), multiplied past any autotuning headroom, with a 32 MiB floor. `wrote_whole_body` stays asserted in every arm, so a payload that is still too small reports that rather than passing for the wrong reason. Verified on **both** kernels: 5 passed locally, 5 passed on the host. | **The premise arms did exactly the job they were written for**, one commit after being written for it. They assert that `urllib` **fails** against those servers, so a rig that stops reproducing the defect says so instead of going quietly green — and this is the second time in two commits that this file was wrong about its own condition (the first was 64 KiB fitting in loopback buffers). **The lesson is not "test on the host too"** — it is that a rig which *constructs* a condition must **measure that it constructed it**, because every environment-dependent knob is a place the construction silently fails. `SO_RCVBUF` is a request; `getsockopt` is the answer; using the request as though it were the answer is D274's shape (*does the proof ask for what the artifact itself asks for*) at the socket layer. | — |
