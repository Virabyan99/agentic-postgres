...
| **D72** | *(D59, ADR 0032, as shipped)* The systemd launcher reads `deployed_through_session` from the project's deployed document, so a Session 3 project restarts as a Session 3 project. | **The launcher that runs on the host is the one from Session 2.** `bin/provision-host.sh` is the only thing that has ever installed `/usr/local/libexec/agentic-postgres/project`, and the host was provisioned in Session 2; the fixed copy shipped inside two installed releases and was never executed. Run 8's first `systemctl restart` ran it: `materialize` with `--session 2` wrote a generation holding Session 2's secret set and **repointed the project at it**, `up` exited `2` for want of `--through-session`, and `ExecStopPost` failed the same way and stopped nothing. Both projects sat in `failed` with their containers still running — nothing visibly wrong, and neither would have come back from a reboot. Every DEP-REL-001 proof passed throughout: each characterises the *release*, and none compared what systemd executes against what the repository ships. | **ADR 0037.** The installed launcher becomes a trampoline that resolves a release and `exec`s `<release>/libexec/project-launcher`; everything session-dependent moves into that new file, which ships inside the release and is deliberately not named `agentic-postgres-*` so it can never be installed. `installed_release.reconcile_launchers` installs the trampolines from the release and the deploy calls it. A launcher's contents are asserted release-independent structurally, and a host test compares the installed bytes against the repository's. | The fix for D59 was correct, tested and shipped, and had no effect for three runs — because the one component that runs at boot with no operator is the one component a deploy did not update. "The release is what runs" was true of everything except the file that decides what runs. | **yes** |
| **D73** | *(§5 Run 6)* `test_the_two_projects_use_separate_volumes` and `test_bootstrap_refuses_a_volume_belonging_to_another_project` read `compose` out of the documents named by `APG_PROJECT_A_OUTPUTS`. | Those name **deployed** documents, and `$defs.deployedDocument` has no `compose`. The volume name, the network names and the Compose project name live only on the rendered branch — which is the D40/ADR 0027 decision working as designed. Both tests need two deployed projects, so neither had ever run; the second produced `KeyError: 'compose'` out of `postgres-bootstrap.py` instead of the exit `11` it asserts. | **Both read the installed rendered document** at `/var/lib/agentic-postgres/rendered/<key>/outputs.json`, through a fixture that asserts `document_kind`. That is also the document `--outputs` actually takes, so the refusal test now feeds `postgres-bootstrap.py` the kind of document it is given in production. | Third instance of the same thing in one session, after D63 and D64: a test written against a shape nobody had run it against. The fixture asserts the kind rather than trusting the path, because the path is what was wrong. | no |
...
| **D79** | *(`tests/contract/test_database_commands.py`, since Run 5)* An `autouse` fixture skips the module under root, because "these assertions are about what happens *without* root". | True of the tests it was written for, and false of several it applies to. Three of them — `DBX-PG-003`'s offline proofs — only **read source files**, where root cannot select a different branch. The Session 3 host gate necessarily runs as root, so those three skipped, and a skip is not a pass: `database_isolation` came out **`failed` on a host where all nine of its live proofs had just passed.** A claim that could not be proved in the only mode that measures it. | **An explicit opt-out, not a narrowed guard.** A test may request `root_is_irrelevant`; the default stays "skip under root", so a privilege test nobody remembered to classify keeps today's safe behaviour. A guard test reads the module with `ast` and fails if anything that opts out also invokes a command. | Narrowing the guard by hand across fifteen tests risked missing one, and a missed one runs its privileged branch under root and passes for the wrong reason — the guard's own failure mode, reintroduced by the mechanism meant to fix it. The decision is ADR 0018's and unchanged: a quiet skip must not read as a clean run. What changed is that three tests stopped claiming they could not look at something plainly visible. | no |
| **D80** | *(the standing non-negotiable)* `bin/session-01-check.sh` exits 0 on a clean tree, on this machine and on the deployment host. | It does — through an activated venv, which is every way anyone had ever run it. Run 9's final step ran it under `sudo`, which resets `PATH` to `secure_path`, and it died on line 23 with `python: command not found`, **exit 127**. Ten bare `python` invocations, in the one command whose exiting 0 is the non-negotiable. `bin/session-02-check.sh` has carried a `python_bin()` resolver since Session 2 precisely for this, and the comment above it says five Session 2 scripts walked into the trap; this one was the sixth and nobody had looked. | **The same `python_bin()` resolver**: the repository's venv, then `python3`, then `python`, and exit 3 naming what it looked for. All ten call sites rewritten. Verified by running the gate under a `PATH` with no venv and no bare `python` — the shape `sudo` produces. | The invocation was mine rather than the documented one, which is exactly why it had never been found: a trap that only fires outside the habitual path stays invisible for as long as the habit holds. It is also not hypothetical — an operator who forgets to activate gets exit 127 from a gate, which reads as a broken gate rather than as a missing venv. | no |
...
**The unit-restart test failed on first execution, and it was right to.** The
launcher installed on the host was the Session 2 copy: `provision-host.sh` is the
only thing that has ever installed one, and the fix D59 wrote in Run 7 shipped
inside two installed releases without ever being executed (**D72**, ADR 0037).
It materialized Session 2's secret set over a Session 3 project, repointed the
project at it, and failed — leaving both units `failed` with their containers
still running, so nothing was visibly wrong and a reboot would have brought
neither back. Two more tests failed for reasons of their own: `compose` is on the
rendered branch of the output schema and not the deployed one (**D73**), and a
password login attempted over loopback inside the cluster's own container proves
nothing, because the image trusts `127.0.0.1` (**D74**).
