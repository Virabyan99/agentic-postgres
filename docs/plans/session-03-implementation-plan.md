...
| **D68** | *(`docs/provider-bootstrap.md`)* `--apply` takes `--operator-credential-file /root/.config/agentic-postgres/infisical.json`. | The path appears in an example and **nothing says what the file contains, who creates it, or that it may legitimately be gone.** `read_operator_credential` requires exactly two non-empty lines — client ID, then client secret — while the example's `.json` implies a document this repository never parses. Session 2 wrote the credential, used it, and (correctly) did not leave it lying on the host; Run 7 needed it again and had nothing to go on. | **The document now says what it is, what format it takes, that it is not the per-project runtime credential, and that removing it after use is the recommendation** — which is why a later session finds it absent and what to do then. | A credential whose provenance is undocumented is one that gets re-created wrongly under time pressure, or left on the host forever because nobody is sure it can be removed. The `.json` extension stays, because it is what exists; the document now says the extension is not a format. | no |
