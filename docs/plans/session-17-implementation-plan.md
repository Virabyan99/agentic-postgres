...
**Next free number after this table is D984.** D944–D958 were written at
planning; D959 and D960 are Run 1's; D961–D963 are Run 2's; D964 and D965 are
Run 3's; D966 and D967 are Run 4's; D968 is Run 6's; D969–D983 are Run 7's,
found on the host in the order listed (D969–D975 on day one, D976–D983 on
day two).
...
| **D968** | `FLEET-EXPIRE-001`: *"no unit, timer, cron, deploy step or command in the release names the retirement verb"*, asserted by a text scan over `systemd/`, `libexec/`, `bin/` and the deploy. | **The Session 17 gate's header names the verb**, in the note telling the operator to run `project-retire.sh --record PATH` before the trip's host gate, and CI reported the gate as a command acting on expiry. The scan read prose. Run 6's targeted list did not include the retirement module, so the first reader of the new gate was CI. | **The scan reads code**: comment lines and the usage heredoc are stripped before matching, exactly as `test_root_script_policy.code_of` does, with a control that a mention in a comment is dropped and a mention on a command line is kept. The inventory's twin guard gets the same reader. | D464's family — a text scan standing in for a construct — and the failure mode a guard over "nothing names X" always has: the documentation that helps an operator is the first thing to name X. A guard that reddens on the sentence explaining why it exists trains the next author to delete the sentence. | — |
| **D969** | Run 3: `expires_at` is *"RFC 3339, UTC, second precision, `Z`"*, a string the schema patterns, and the example manifest's comment shows it bare. Run 3's proofs wrote every manifest through `yaml.safe_dump` of a Python dict, where the value was already a `str` and was emitted quoted. | **The first ephemeral manifest ever written by hand failed to render on the host**: `project.schema.json: project/lifecycle/expires_at: datetime.datetime(2026, 9, 5, 12, 0, tzinfo=...) is not of type 'string'`. PyYAML's implicit timestamp resolver types a bare `2026-09-05T12:00:00Z` before the schema sees it; `StrictLoader` refused merge keys, duplicate keys and non-string keys and let this one through. The workstation reproduced it on the same file. | **The strict loader drops the timestamp resolver** and nothing else, so the text reaches the schema as written; a proof asserts the premise (plain PyYAML still types it) and that the other implicit types are untouched. The trip's first repair, before any deploy. | Question 6 in its purest form: every offline proof of the field wrote the manifest with the same library that would then read it, and the library agreed with itself. The first document written the way an operator writes one -- by hand, bare -- was the first to disagree. **The fixture and the code shared a belief because one tool wrote both.** | — |
| **D970** | This plan's §4: *"`--check` first, at a TTY … If `--check` reports anything but the missing units, the units are installed by hand."* Read as: `--check` reports the missing units. | **`provision-host.sh --check` reported "the host meets the Session 2 baseline", exit 0, on a host missing all four backup units.** Its unit section iterated a list typed into the function -- `docker-firewall`, `edge`, `project@` -- while `install_units` globs `systemd/*.service systemd/*.timer` (D522). The checker never learned what D522 taught the installer. | **The check iterates the installer's glob**, and `test_backup_schedule.py` asserts the two loops read the same glob, anchored on two different functions so the proof cannot read one text twice. The units were installed by hand on the trip (`install -m 0644`, `daemon-reload`), as §4 prescribes for a check that deviates. | Question 5 again, one reader further out: D522 fixed the installer and left its checker with a definition of its own, and D944 then found the deployment in the state the checker said it was not in. A check that cannot go red on the defect it exists to find is a green light (D600, D918). | — |
| **D971** | CLAUDE.md §6 and this plan: *"`host.yaml`, `capabilities.yaml`, `project.*.yaml` are gitignored operator inputs."* | **`.gitignore` names `/project.alpha.yaml` and `/project.beta.yaml` individually**, by a stated decision (*"a glob would silently hide a future `project.third.example.yaml` that ought to be committed"*). The third manifest copied into the checkout was an untracked file; `installed_release.assert_clean` read `git status --porcelain` and refused every deploy: *"the checkout has uncommitted changes; a release must be exactly the commit it is named for."* | **A third project's manifest lives outside the checkout** (`/home/op/project.gamma.yaml`) and every command takes that path; `docs/fleet-operations.md` §6 says so first. The ignore file is not widened: its reason stands. | The handoff described the ignore rule as the glob it deliberately is not -- a premise wrong in the reassuring direction (D930), read by a session that then did what the premise allowed. The release-cleanliness check did its job; the surprise was the operator's, not the product's. | — |
| **D972** | Nothing: no runbook said how a deploy's output is captured, and the trip's own instruction added `> /tmp/r7-deploy-beta.txt 2>&1` to keep the paste short. | **The beta deploy stopped in step 6 and would have waited forever.** `postgres-bootstrap.sh`, `postgres-bootstrap.py` and its first child `docker exec -i apg-beta-dev-postgres-1 psql … SELECT 1` were all in process state **`T`** (stopped) after twelve minutes; nothing had been applied. sudo's `use_pty` runs the command in the background of its pty when the standard streams are not all terminals, and the first child that reads the terminal -- `docker exec -i` attaches stdin -- receives `SIGTTIN`. The probe's own 30-second timeout cannot fire inside a stopped process; it fired the moment the process was continued. Alpha's deploy, identical but unredirected, took minutes. | **`deploy.sh` refuses `--through-session` when stdin is a terminal and stdout or stderr is not**, before the root check, naming the cause; a proof runs it with a pty on stdin and a pipe on stdout (exit 2) and, as the control, with no terminal anywhere (exit 3, the root refusal). Recovery on the trip: `kill -KILL` the stopped `docker exec`, `kill -CONT` its two parents, the deploy exited 5 with the timeout, and the unredirected re-run converged. `docs/fleet-operations.md` §6: the terminal is the log. | A bound that cannot fire is not a bound (D145's family): the timeout was real, tested, and inert in the one process state that needed it. And the instruction that caused it was written to make the evidence easier to read -- the trip's own convenience was the third party nobody measured. | — |
| **D973** | Run 5's Done paragraph, the verb's closing line, `docs/backup-operations.md` step 5 and `docs/fleet-operations.md` §4: *"both timers carry `Persistent=true`, so a run the calendar already owed fires at enable."* Stated as unmeasured. | **Nothing fired.** Read immediately after `schedule enable` on alpha, whose newest full backup was seven days old: both timers `enabled`, both services `inactive (dead)` with empty `ExecMainStartTimestamp`, the full timer's `LastTriggerUSec` **empty**, `NextElapseUSecRealtime` the next calendar slot (Sunday 02:04 with jitter; the incremental Saturday 03:46). `Persistent=true` catches up a slot missed while the timer was already installed, judged against the stamp file the timer writes on its first trigger; a freshly enabled timer has no stamp and owes nothing. | **The verb's line, both documents and the verb's proof say what was measured**: a first enable waits for the next calendar slot, and `list-timers` is how to read it. The plan's Run 5 paragraph stands as the record of what was expected. The first scheduled runs are read from the journal the next morning (§5 Run 7 step 3). | The verb *printed* the expectation, which is how it was caught within a minute of being wrong: a statement in the operator's face is measured by the operator. A statement in a document is not. | — |
| **D974** | `deploy-project.py`, above the D230 gate: *"the administrator is read first, so a project that has none records `unavailable` without spending the observation window polling a route it is not going to publish."* | **The window was spent.** The first ephemeral project has no administrator, and each of its three deploys printed *"no active project administrator; routes.app stays unavailable (D230)"* ten times: the administrator was read first and passed into the polled observation, which returned `unavailable` on every attempt until the deadline. The comment described the intent; the loop did not implement it. | **The gate branches**: with an administrator the observation is awaited; without one it is taken once and recorded. Measured on the next gamma deploy by the line appearing once. | A comment that says what the code does not is the cheapest form of the project's defect: a value that looked measured (*"without spending the window"*) and was the author's belief. Question 6, one author. | — |
| **D975** | `--render-runtime-only`'s closing hint: *"sudo bin/database-ports.sh verify --host <host.yaml> --instance-uuid …"*, a placeholder for a value the process had just read from `--host`. | **Pasted as printed, the shell read the angle brackets as redirections**: `< host.yaml` fed the manifest to stdin and `> --instance-uuid` **created a file named `--instance-uuid` in the checkout**, argparse reported `--instance-uuid` missing, and the checkout was dirty -- the next deploy would have refused it (D971's check). Removed with `rm -- ./--instance-uuid`. | **The hint prints the path it was given.** `test_printed_commands.py` scans the driver's print statements for `--flag <placeholder>` and allows only the two the process cannot know (`--username <name>`, `--display-name <name>`), with `deploy.sh`'s usage text as the control where a placeholder is legitimate. | A printed command is run as printed. The D230 hint two screens up got this right for the same reason and the port hint did not; the guard is the class, not the line. | — |
| **D976** | `docs/session-11-operator-guide.md` §6: *"`TimeoutError` reading Infisical is a transient in an external service. Re-run."* The client has one 30-second timeout and no retry; the materializer catches `InfisicalError` and reads only a 404 as absent (D283). | **Four of five reconverging deploys on 2026-09-05 failed at step 5 on a provider read, and the failure escaped as a traceback.** Measured with the materializer's own client at a 120-second ceiling: login 0.5 s; the **first authenticated read after a login took 8.9 s on alpha and came back HTTP 504 after 60 s on beta**; every read after it 0.4 s. A deploy makes twenty-two reads, and a deploy that had passed the first read still timed out on a later one. The bare `TimeoutError` urllib raises for a read that times out after connecting is not a `URLError`, so it passed the client's two `except` clauses and the materializer's one, and the deploy printed a traceback instead of its named refusal. | **A login and a read are retried up to three times on a timeout or a 502/503/504, with 1 s and 3 s between attempts; a 404, a 401, a DNS failure and every non-idempotent call are attempted once**, opted in per call site because the transport cannot tell a repeatable POST from a DELETE by the verb. A timeout that exhausts its attempts is an `InfisicalError` with `status` None. Six proofs with a scripted `urlopen` and a recording `sleep`, the 404 control among them. | Question 4: the remedy for this failure was written into a guide in Session 11 and the product kept the defect, so the trip paid for it again at one deploy in four. And a retry on a read is the one place D387's family (*the observation does not retry*) is safe to close, because a read is the one call that cannot do anything twice. | — |
| **D977** | Session 11's plan §11 and its operator guide §6 recorded both of this trip's day-one stoppages: the `T+` stop from a piped `docker exec -i` (*"re-run with `< /dev/null`"*) and the provider timeout (*"Re-run"*). Both are D972 and D976. | **Both were re-discovered on the host, each costing the trip about an hour**, because both fixes had gone to the guide and neither to the product, and this session's planning grepped the plans for third parties but not for `SIGTTIN` or `TimeoutError`. The guide's own remedy for the first, `< /dev/null`, would have made the deploy the non-interactive shape D972's guard allows, which is a second hint the product should have refused the interactive-redirected shape years earlier. | **Both are in the product now** (`deploy.sh` refuses the shape; the client retries the read), and the planning rule gains a line: **before a trip, grep the previous trips' §6 "if something goes wrong" sections for every failure they told the operator to work around by hand**, because each is a product defect with a documented workaround. | The cheapest place to put a fix is the guide, and it is the place a fix is least likely to be found by the next trip. Question 4 asks which side got the fix; this row asks how many trips a guide-side fix survives before it is paid for again. Two. | — |
| **D978** | ADR 0186 and Run 3: *"the render refuses an ephemeral project born expired"*, implemented in `validate_project_semantics`, which `load_project_manifest` applies to every manifest it loads. ADR 0187 and Run 4: a retirement *"refuses an unexpired ephemeral project without `--before-expiry`"* -- so the designed case is a retirement AFTER expiry. | **The retirement's provider-destroy step would have refused the expired project it exists for.** Found by the day-two transport: the render of the gamma manifest at 12:57 UTC, 57 minutes past its expiry, exited 2 with *"an ephemeral project cannot be born expired"* -- correct for a render, and the same loader is what `bootstrap-providers.py` calls before it branches on `--mode`, and `retirement.STEP_ORDER`'s sixth step runs `bootstrap-providers.sh --destroy --project /etc/agentic-postgres/projects/gamma-dev/manifest.yaml`. Steps 1–5 would have run, step 6 would have exited 2, and the run would have stopped with the identity unrevoked and the directories in place. `--plan` cannot show it: the plan prints the command and does not load the manifest. | **The born-expired rule is a rule about a birth.** `validate_project_semantics` and `load_project_manifest` take `expiry=True`; the bootstrap passes `expiry=False` for `--mode destroy` and nothing else. Two proofs: the loader with the control (the default still refuses, every other rule still applies), and the command (`--mode destroy` on an expired manifest reaches its next refusal; `--mode plan` on the same file is still refused at the loader). | Question 5 with a twist: the decision *expiry is read, never acted on* had one reader that acted on it -- the loader -- and the retirement's sixth step was the caller nobody listed, because it lives in another command. The one route that meets an expired manifest by design was the one route not walked offline: Run 4's proof of the order recorded the destroy command against a fixture that had not expired. | 0186, 0187 |
| **D979** | D978, two hours earlier: *"the bootstrap passes `expiry=False` for `--mode destroy` and nothing else"*, proved as the command `bootstrap-providers.py --mode destroy`. | **The first real retirement stopped at step 6 with `bootstrap-providers.sh exited 2`.** The wrapper derives the project key before it hands over to the Python, by loading the manifest a second time in a heredoc, and that reader had not moved: `load_project_manifest(Path(sys.argv[1]))`, born-expired refusal, traceback, `die 2 "could not derive the project key"`. D978's proof ran the Python and not the wrapper, and the retirement composes the wrapper. Steps 1–5 had run: the record written, the stack down, the ports released, the edge file removed. | **The wrapper passes its mode to the heredoc** and the heredoc loads with `expiry=(mode != "destroy")`; D978's command proof runs the wrapper as well, where a destroy of the expired manifest reaches the root refusal and a plan is still refused at the derivation. The retirement was re-run with a fresh record path; every step that had already run repeated cleanly (the port release is idempotent by design, the edge file "already absent"). | Question 5 in its purest form, on the same day the first half of it was answered: *when a decision is implemented, which of its callers got it?* D978 repaired the function and one caller; the second caller was a heredoc inside a shell wrapper, and grep for the function name would have found it in a minute. The rule from D600 -- guard the class, never the field that failed -- applied to readers: **grep every reader of `load_project_manifest` before repairing one.** There were seven. | 0187 |
| **D980** | ADR 0187 and Run 4: a failed step *"stops the run and names itself"*, proved by `test_a_failed_step_stops_the_run_and_names_itself` against a fake whose stderr was empty. | **The step named itself and nothing else**: *"step 6 (provider-destroy) failed: bootstrap-providers.sh exited 2. The steps after it did not run"*. The wrapper had printed the traceback and the `could not derive the project key` line to its stderr, which the retirement captured and discarded. Reconstructing the cause took a read of two source files. | **The retirement repeats a failed step's last lines and puts the last one in the reason**, and the proof's fake now has something to say and asserts it is repeated. | A message that names the step and not the cause is D145's shape one layer up: the state was in the child's stderr, and the parent reported the exit code. Every fake in a proof of failure reporting should fail with words, because a fake that fails silently proves the reporter can count. | — |
| **D981** | D957, at planning: *"`bootstrap-providers.py destroy()` revokes the runtime identity, unlinks the two credential files and the state file"* -- read off the source. `test_the_control_plane_authenticates_rather_than_sending_the_credential`: *"`ControlPlane.login(` in source"*. | **`--destroy` had not worked since the control plane started authenticating**, and the second retirement attempt was the first time anyone ran it: step 6, now reporting its own words (D980), said *"the access token contains characters that cannot appear in an HTTP header (length 101)"* -- 36 + 64 + a newline, the two-line credential file passed whole as a bearer token. `apply` exchanges the file through `read_operator_credential` and `ControlPlane.login`; `destroy` did `credential_file.read_text().strip()` into the constructor. The proof of the login change scanned the file for one call and apply supplied it. | **`destroy` authenticates exactly as `apply` does**; the scan requires at least two callers and no client built from the raw file; and a behavioural proof runs `destroy()` against a fake control plane that records the login, the revocation and the unlinks, with the direct construction as the control. | The reassuring direction, twice: D957 described the verb's behaviour from its source and called it measured, and a proof of "authenticates rather than sends the credential" passed on the strength of the one caller that did. A text scan for a construct (D464, D968) passes when any caller has it; the question is always which callers do not. | 0187 |
| **D982** | D953: *"`project_removal` closes on Session 17's gate through the proof that already exists -- no claim moves, no second proof."* `test_removing_one_project_leaves_the_other_whole`, written in Session 12, gated on `APG_REMOVED_PROJECT_FILE` since. | **The proof ran for the first time on 2026-09-05 and died on its last measurement**: `AttributeError: 'str' object has no attribute 'stdout'`. The `sh` fixture returns stdout as text and fails the test itself on a bad exit; the proof read its return as a process object. Everything above that line passed on first execution -- the survivor's database answered with its row count and every route read `ready` after the removal. The line that would have said *nothing of the removed project is still running* never ran. | **The line reads the text it is given**, and the host gate is re-run. No offline proof can catch this: the module is a live proof and the fixture's contract is a live fixture's. | Question 2 and question 6 together: a proof that had never executed, written by an author who believed the fixture returned what `subprocess.run` returns. Nine sessions of "nothing knows which proofs have never executed" (CLAUDE.md §9) and this is the sixth defective never-executed proof a trip has found. The one thing that would have caught it offline is a fixture whose return type is asserted by the proofs that use it. | — |
| **D983** | ADR 0152 §3 and `restore_drill._BACKUP_SET_LINE`: *"restore backup set (?P<label>[0-9]{8}-[0-9]{6}[FDI])\b"*, verbatim from rig 8's full-set line; the type proof's fixture listed an incremental as `20260825-110000I`. | **The first scheduled incremental, the night it ran, broke the restore drill.** Beta's newest set became `20260904-102252F_20260905-034338I` -- the label names the full set it depends on -- and the host gate's drill exited 5 with *"the restore did not name a backup set … an empty capture here means the log level did not take"*: `[FDI]\b` stopped at the underscore. A wrong diagnosis on top of a wrong parse. The evidence proof then read the previous day's drill against this run's timing and failed on the RTO bound, which is the same defect seen through a stale file. **Until 03:43 UTC no repository had ever held anything but full sets**, so the shape had never been printed. | **The label is read whole** -- `F`, optionally `_<stamp>D` or `_<stamp>I` -- with the full line as the control and a bare dependent stamp refused; the type fixture carries the real shape. The drill's second read (`info` must list the label) is unchanged and is what keeps a wrong parse loud. | Question 6 and D702 together: the fixture invented an incremental label of its own and the parser agreed with it, and the parser's line was derived from one observation of one kind of set. The first scheduled backup in the project's history changed which kind was newest, and the drill that had passed on every trip failed on the first morning the schedule existed. | 0152 |
