...
Twelve were named in the brief. The rest were found reading the working tree and building it, and are appended in the order they were found rather than renumbered — a stated count here went stale three times, so there is no longer one to keep current. **D13, D14, D15 and D16 are the ones that break `bin/session-01-check.sh` outright** and should be read first.
...
| **D29** | *(found during Run 2)* §7 lists exactly two launchers under `/usr/local/libexec/agentic-postgres/`: `edge` and `project`. | `agentic-postgres-docker-firewall.service` must run after `docker.service` and before the edge plane, and it needs the same immutable-release indirection as the other two — but it belongs to neither. | Add a **third** launcher, `firewall`. Same shape: resolve the recorded release, validate the commit before it becomes a path component, refuse symlinks, require root ownership, exec the release's script. | The firewall policy is a host concern that stays correct whether or not any project is deployed. Folding it into the edge launcher would make ingress policy a subcommand of the thing it is supposed to gate. | no |
| **D28** | *(found during Run 1)* Phase 4's `bootstrap-state.json` fragment uses `credential_files: {client_id, client_secret}`. | A key literally named `client_secret` is rejected by `config.assert_no_sensitive_keys` — ADR 0008's terminal-token rule — so the runbook's own state document cannot be written. | Rename the fields to **`client_id_path`** and **`client_secret_path`**. Both hold filenames, so the new names are also the accurate ones. The sensitive-key rule is not touched. | The rule reads key names and cannot know this one holds a path; that is a limitation it is entitled to have. Naming the field for its contents is what the `password_secret_ref` precedent already established in Session 1. Weakening the denylist to admit `client_secret` would let a real client secret into a project manifest. | no |
...
| **D30** | *(found during Run 3)* §6 groups Session 2's new tests by execution environment and says nothing about how an absent environment is expressed. | `future` is the only existing mechanism for "this does not run here", and reusing it would destroy the one property that makes it worth having: on the host, where the environment *is* present, the marker would be removed, and the suite would have no way left to say a test is unwritten. | A second marker, **`requires_environment(*names)`**, whose condition is a named variable and nothing else. `tests/contract/test_environment_gates.py` asserts every `live_host`/`external` test carries one, that no test carries both it and `future`, that every named variable is registered, and — the direction that matters — that a present variable actually *admits* a failing body. An unregistered name aborts collection, so a typo is an error rather than a test that silently never runs anywhere. | This is D10b's rule made mechanical. The plan already required the separation; without enforcement the first inconvenient live test would have been marked `future` and nobody would have noticed until Session 4 inherited a placeholder that had already been "done". | no |
| **D31** | *(found during Run 3)* Phase 16's new-requirement table implies one test per requirement. | Session 2's claims each have a structural half that is provable in any checkout and a live half that needs the host. Registering only the live half would leave every Session 2 requirement skipped in CI; registering only the structural half would prove the repository agrees with itself. | Each of the 13 Session 2 requirements registers **both**, and the registry says so in a header comment. Concretely: `SEC-DOCKER-001` names both the offline socket-proxy allowlist assertion and the live 200-versus-403 probe; `SEC-LOG-001` names both the Traefik config assertion and the live sentinel search. | Neither half is sufficient. Structure without measurement misses the leak that went around the structure; measurement without structure passes on the day nobody happened to exercise the path. The two-column registry entry is the honest shape. | no |
| **D32** | *(found during Run 3)* §6 lists `tests/security/test_session2_{secrets,installed_release}.py`. | The secret tests split cleanly into a half needing no secret and a half needing a real one, and a module cannot carry two different `pytestmark` sets. | Split into **`test_session2_secret_model.py`** (offline, runs in CI) and **`test_session2_secrets.py`** (live). Same requirement, two modules. | A single module would have had to gate every test on `APG_SECRET_SENTINEL_FILE`, which would have made the structural assertions — the ones CI can actually run — skip in CI. | no |
| **D33** | *(found during Run 7)* §4's Run 7 promotes ACME to production first, then bootstraps and deploys Project B. | `bin/edge.sh`'s `staging_certificate_exists` refuses to promote until *some* staging certificate has been issued, and `tests/deployment/test_session2_isolation.py::health` verifies through `ssl.create_default_context()` — the system trust store — for **both** hostnames. | **Project B is deployed on staging first; promotion follows.** Order becomes: bootstrap B → materialize B → deploy B → confirm `beta-db` answers on a staging certificate → `promote-acme` → confirm both hostnames on a trusted certificate → the isolation gate. | Promoting first would make `beta-db`'s first-ever certificate request a **production** one against a route that had never issued anything. That is precisely the spend `staging_certificate_exists` exists to prevent, and it can only see `alpha-db` — the guard would pass while the risk it guards against was taken. Failed validations are capped at 5/hour/hostname, so getting `beta-db` wrong under production costs an hour; under staging it costs nothing. The promotion is not deferred, only reordered: both hostnames are still on production certificates before the isolation tests, which cannot pass otherwise. | no |
| **D34** | *(found during Run 7, resolved in Run 8)* §4's Run 8 runs `jq -e '.tests.secret_leakage=="passed" and .tests.isolation=="passed"'` against the merged evidence. | `bin/write-session-evidence.py`'s `write_half` emitted `{"tests": {"<mode>_suite": "passed"}}` — a literal, and the name of a command rather than of a guarantee. Neither key the plan checks could exist, in any run, ever. | **The evidence `tests` map is keyed by *claim*.** A claim names acceptance-registry requirement IDs and its verdict is computed from the JUnit results of exactly the node IDs the registry lists. The plan's command is unchanged and now passes. | The alternative was to change the plan's check to `.tests.host_suite`, which is cheaper and makes the evidence permanently useless: the merged document would still answer "a command exited zero" while Session 2's two headline claims went unstated. **ADR 0025.** | yes |
| **D35** | *(found during Run 8)* §4's Run 8 runs `--mode external` and treats its result as the public-boundary evidence. | The deployment host holds a global IPv6 address (`2a01:4f9:c015:1bb0::1`), and **no network available to run the external gate from has IPv6 transit**. `SEC-NET-001` lists `test_no_service_port_is_publicly_reachable_over_ipv6`, which therefore skips, and a skip is not a pass. | **The IPv6 half of the public boundary is not measured externally, and is recorded as unmeasured** rather than claimed. What *is* measured, host-side: nothing binds a public IPv6 address except SSH (`test_only_ssh_and_the_edge_listen_on_a_public_address` parses `ss -lntup`, which covers both families), `net.ipv6.conf.all.forwarding = 0`, and ufw's `IPV6=yes`. No AAAA record is published. See `docs/host-baseline.md`. | Declaring a `public_boundary` claim over `SEC-NET-001` would make it permanently `failed` — not because the boundary is open but because nobody can look at it from here — which is a blocker invented by the evidence writer. Narrowing the claim to exclude the IPv6 node ID would be silently reconciling the conflict. The external half therefore carries no claim, and its `suites.external.skipped` count is the honest record of the gap. | no |
