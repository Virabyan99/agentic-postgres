...
Six columns, next free number after this table **D1379**. Rows D1303–D1314
...
| **D1336** | This plan's Run 4 step 4: *"`bin/session-01-check.sh`'s generated-documents step **and CI's** gain `--check --project project.example.yaml`"*. | **CI has no generated-documents step of its own.** `.github/workflows/ci.yml` invokes `bin/session-01-check.sh` and nothing else that renders -- `grep -n 'render-'` over the workflow returns two `--render-only` deploys and no `render-*.py` at all. The gate's step 6 IS CI's, which is the property line 3 of the workflow states in as many words: *"This workflow invokes bin/session-01-check.sh. It deliberately does not"* keep a second definition. | The step is added to `bin/session-01-check.sh` step 6 and the workflow is **untouched**. Adding a line to CI would have created the divergent second definition the workflow's own header says it refuses. | A plan sentence naming two places to edit, where the second exists only through the first, is how a repository grows two definitions of passing. Checked rather than obeyed. | -- |
| **D1337** | This plan's D1325: *"Run 4 seeds `projects/example/docs/mcp-tool-catalog.md` with the two markers and commits it"*, so that `main`'s read-before-write finds a file. | **The tree already solved this, in the sibling renderer, and better.** `bin/render-evaluation-report.py` CREATES a project's report from a `PROJECT_REPORT_HEAD` constant when the file is absent, and its `--check` reports *does not exist* with the `--write --project` line rather than reading a file that is not there. A hand-seeded file holding two bare markers would be an artefact no command owns: nothing regenerates its prose, and a project's second catalog would need a human to remember the ritual. | `render-mcp-catalog.py` gains `PROJECT_CATALOG_HEAD` and the same three answers, measured: `--check --project` on an ABSENT catalog exits **5** naming the write command; `--write --project` creates the file and its parent directory; `--check --project` then exits 0; a one-byte edit exits 5 naming the contract it drifted from. The example project's catalog is still committed -- it is written by the command rather than by hand. | ADR 0195 in a generator, which is what D1325 asked for; the disagreement is only about who writes the first copy. Following the sibling means a project's first catalog costs one command and no ritual, and the head cannot rot, because the renderer owns it. | -- |
| **D1338** | This plan's D1309: the project catalog is rendered *"from the project's compiled contract **with the release's rows marked as the release's**"*. | **The project's compiled contract carries no release row to mark.** Measured (rig 25h): `projects/example/contracts/mcp-capabilities.canonical.json` holds **2 tools behind 2 capabilities**, both the project's own; the release's 6 live in a different file, and the two are joined only at deploy time, into the lock. Rendering the JOINT contract instead would contradict D1324, which fixed the table at two rows. | The marking moves from the rows to the **header**, and every part of it is derived at render time: the project's slug and contract path; a sentence saying these are *not the whole of what its deployment serves* with a relative link to the release's catalog; and the tool names **both contracts carry**, read out of the release contract rather than listed -- `query_resource` today. That last one is the reason the sentence is worth generating: one name, two authorizations, and a scope granted for one grants nothing for the other. | A catalog of a project's two tools under the heading the release's six use would tell an adopter their deployment serves two. That is the exact failure `render-mcp-catalog.py`'s docstring says the whole file exists to prevent, one level down. | -- |
| **D1339** | Nothing in the plan: found by running the suite after the renderer gained its first `@dataclass`. | **Three green proofs went red at IMPORT, before any assertion.** `test_mcp_catalog.py` loads `bin/render-mcp-catalog.py` by path in three places and never registered the module in `sys.modules`; `@dataclass` resolves its own class's module through `sys.modules[cls.__module__]` while the class is being created, so `dataclasses` read `None.__dict__` and raised `AttributeError`. Green since Session 8 because the file had no dataclass in it. | Fixed in the **harness**, not by avoiding the dataclass: one `_renderer_module(name)` helper that registers the module under a caller-chosen name, and the three call sites reduced to calls of it. importlib's own documentation prescribes the registration; the three sites already used three distinct names, so nothing collides. | §7 question 4 -- when a defect class appears, which side gets the fix. The product's dataclass is the tree's idiom (`render-evaluation-report.py` has one); the broken half was an import idiom that had never been exercised against a module that needed it. | -- |
| **D1340** | This plan's Run 4 step 2: the guide's *If something fails* table gains *"the three refusals an adopter meets on this path ... each with the exit code and the sentence the product prints, read from the tree (D267)"*. | **Two of the three could not be quoted, because the product printed a Python traceback.** Measured (rig 25i, on a throwaway copy of the example set): a project migration stamped below the version its set was frozen against, and a table in `app` without FORCE row level security, both make `./deploy.sh --render-only` exit **1** with a stack ending in `ProjectSetError` / `MigrationError`. Neither is a `RenderError`, and `bin/render-config.py` caught only `RenderError`, `ManifestError` and `CapabilityContractError`. `bin/migrate.py` and `bin/dev.py` have handled this class since Session 20; the render is its third caller and never got the decision. Exit 1 is not in the exit-code convention. | Repaired in `bin/render-config.py`, one `except migrations.MigrationError` clause: the refusal is printed with *the previous valid render, if any, is unchanged* and the exit is **5**. Re-measured on both arms after the repair, and the control still renders 0. A guard in `test_project_migration_sets.py` with three arms -- the project error reported, the `RenderError` branch unchanged, and an unrelated exception still propagating rather than being swallowed into a contract exit code. The guide's table quotes the repaired sentence. | §7 question 5: when a decision is implemented, which of its callers got it. The row asked for a quotation and the quotation turned out to be a stack, which is D1325's class in the command an adopter's FIRST bad migration meets. Found by running the refusal rather than reading the message out of `migrations.py`. | -- |
| **D1341** | ADR 0207 §4 (this session's own): *"a launch against the rendered document is refused with a message naming the deploy as what is missing"*, which the walker records verbatim. | **The message named what the file IS, not what was missing.** Measured (rig 25j): `bin/apg.sh studio --project project.example.yaml --outputs .generated/fixture-alpha-dev/outputs.json` exits **2** with *that is a rendered document; the REST route is an observation* -- inherited from `bin/api.py`'s Session 5 wording, correct, and unreadable as an instruction by somebody who has never deployed anything. `studio --help` does exit 0, so the other half of §4 held. | The sentence gains its remedy and keeps its exit code and its condition: *... and a render has made none. Deploy the project, then pass --outputs the outputs.json that deploy published (ADR 0158). Until then `studio --help` is what there is to read.* `bin/api.py`'s copy is left alone -- it is a different verb with a different reader, and widening the change would be a rewrite nobody measured. The existing proof in `test_studio_command.py` is strengthened to assert both new clauses. | ADR 0195 says the third answer is REPORTED, and a report a reader cannot act on is reported only in form. The walk is the one reader guaranteed to arrive here with no deployment and nobody to ask. | 0207 |
| **D1342** | This plan's Run 4 step 3: the task statement instructs the walker to write *"`reached_success_criterion` honestly and `blocked_by` when it is false"*. | **`blocked_by` had no reader.** It is not in `REQUIRED_FIELDS`, no function in `dx_record` touched it, and `bin/dx-record.py` did not print it. A field a task statement asks for and nothing reads is an unverified field -- D816, D929 and D1247 are the three times this repository has already paid for that shape. | `dx_record.blocked_by_problems` reads it in **both** directions: a record whose criterion is not true and which does not say where it stopped is a finding, and so is a record that reached the criterion while carrying a blocker, because its two halves disagree and it does not say which is wrong. The command prints it as a fifth reading. Proofs in both modules, five arms and three, the clean record the control in each. | A failed walk that does not name the step it stopped at spends the second walk §9 allows on a search instead of a repair, and there is never a third. The field was going to be asked for on the day this page was written; it has a reader from the same day. | -- |
| **D1343** | `tests/contract/test_session12_documented_path.py`'s `CURRENT_PATH_DOCUMENTS` -- eight documents, unchanged since Session 12 -- against `dx_record.DOCUMENT_ROOTS`, which Run 3 wrote. | **The two halves of one claim scanned different documents.** The OFFLINE half did not read `docs/new-team-member.md`; the LIVE half does, and has since Run 3 (D1323's other side). So the page `docs/README.md` sends a new reader to FIRST was the one page whose commands, flags and session numbers nothing checked -- which is how it kept a *future session (10)* label through fifteen sessions in which that session shipped. | Both `docs/new-team-member.md` and the new `docs/second-walk.md` join `CURRENT_PATH_DOCUMENTS`. Widened rather than narrowed on the other side: the guide is the page most likely to name a command that has moved, and the statement is a page whose typo would fail a walk on the builder's typing. Both pass the existing four proofs unchanged. | A scan is only as wide as its list, and a list written in Session 12 describes Session 12's documentation. The guide gained two steps in Sessions 22 and 23 and was never added. | -- |
| **D1344** | This plan's §2: `DX-DOC-001`'s node ids name `test_render_mcp_catalog.py::test_a_project_catalog_is_rendered_from_the_projects_contract` and `::test_check_project_refuses_a_stale_catalog`. | **A module for the catalog renderer already exists**: `tests/contract/test_mcp_catalog.py`, 457 lines, 15 proofs, and the plan's own step 5 says to extend it rather than add a module if one is found. It was. | The two proofs keep their names and land in `test_mcp_catalog.py`, so Run 5's registry entry reads `test_mcp_catalog.py::...`. The module's `pytestmark` is unchanged, which is what D1240 asks either way. | Recorded rather than silently reconciled, because §2's node ids are what Run 5 writes into the registry and a node id that names no test is a claim that collects nothing (D1236's shape). | -- |
| **D1345** | This plan's Run 4 step 2: the guide's *done* section is *"verbatim what the record's `reached_success_criterion` means: steps 1-14 complete, `bin/apg.sh dx-record check` reports none/none/none, and the gate exits 0"*. | **`check` prints five readings and two lines out of the record, not three** (measured, rig 25k): source edits, commands the documentation does not name, documents that moved after the walk, `followed_by`, `blocked_by`, then the documents the walk did not record reading and the undocumented steps it recorded. And the guide is read by every new team member, of whom only a WALKER owes a record at all. | The done section states the criterion in the plan's words -- *steps 1-14 complete, `dx-record check` reports none, none and none, and the gate exits 0 on your clean tree with your project directory tracked* -- and a paragraph under it says who owes a record and which two clauses apply to a reader who does not. `docs/second-walk.md` §3 carries the exact count of readings. | A guide that told every reader to produce a walk record would be documenting the measurement instead of the product; a criterion that quietly dropped the record would stop being the walk's. Both halves are said, in the place each is read. | -- |
| **D1346** | This plan's §2, `REL-STAGE-001`: *"`VERSION` (the file) equals `agentic_postgres.VERSION`"*, with a proof named `test_the_version_file_and_the_constant_agree`. | **There is no `agentic_postgres.VERSION` constant** (measured, rig 25j: `hasattr` is False, and the only public name carrying the word is `template_version`). There deliberately is not one: `VERSION` the file is the single authority and `template_version()` reads it, which is ADR 0002's rule applied to the release identity. So the proposed comparison is a tautology -- a function that reads a file, compared against the file it read -- and a tautology is the least useful thing a proof can be. | The proof keeps its name and changes its subject to the pair that CAN drift: the paragraph above `CURRENT_SESSION`, which is where the release says which session it moves to and which version `VERSION` moves to with it. Rig 25j also measured that **nothing in the tree reads that paragraph** -- a declared field with no reader (D816, D929, D1247), and the release's own account of itself is exactly that kind of field. `tests/contract/test_release_contract.py` is the reader; the live half reads the class out of the same paragraph rather than carrying its own copy of it. | A second authority on the version would be the defect ADR 0002 exists to prevent, and adding a constant to satisfy a proof would have created one to measure it with. The drift that is real is prose against code, and Session 19 is the precedent: `VERSION` moved to `1.0.1` while `CURRENT_SESSION` stayed at 18, and the comment is the only place a reader is told why. | 0002 |
| **D1347** | This plan's Run 5 step 2 and CLAUDE.md §2: *"the seven requirements of §2"*. | **§2's table has SIX rows**: `DX-CTX-001`, `DX-COMPLETE-001`, `DX-WALK-001`, `DX-DOC-001`, `SEC-DX-001`, `REL-STAGE-001`. §7's own arithmetic agrees with the six -- it predicts 126 claims as *122 + 4*, and four claims over seven requirements would leave one unclaimed, which D697 forbids. | Six requirements registered; the registry goes 213 -> 219. Every count this run writes down is COUNTED from the file rather than recalled (D1194). | A miscount in the plan's own prose is cheap here and expensive at the close, where the Stage 4 report's tables are filled from the same numbers. §7's arithmetic is the second reading that settles it. | -- |
| **D1348** | This plan's Run 5 step 3: `upgrade plan --json` -> *"`proposed` equals the constant's comment class (`minor`), `required` <= proposed, and the plan *may proceed*"*. | **`bin/upgrade.py::as_json` emits none of those three keys.** It emits `bump` (the class the version delta proposes), `requires` (the class the changes require), `verdict` (`ok`/`blocked`/`undetermined`) and `reasons` (empty when nothing blocks), alongside `project`, `installed_version`, `candidate_version`, `changes`, `operator_digests_moved` and `differences`. | The live proof reads the payload's own names, taken from `as_json` rather than from the plan. Read against the emitter before the proof was written, because this one could only have failed on the host: a `KeyError` at the end of a fifteen-minute sweep, in the one proof the trip is being made for. | §7 question 3 in its cheapest form -- the plan names a surface's fields and the surface is the authority on what it emits. The same shape as D1316 (`dr-kit verify` refusing the flag its own help names): a document describing a command, believed instead of the command. | -- |
| **D1349** | `bin/dx-record.sh`'s usage: *"Print the four readings -- source edits, commands the documentation does not name, documents that moved since the walk, and the shape of `followed_by`"*, and `bin/dx-record.py`'s exit-code block: *"one of the four readings has findings"*. | **The command prints FIVE.** Run 3 wrote four; Run 4 added `blocked_by` as the fifth (D1342) and moved no usage text, so the command's own `--help` undercounted what it did -- in four places, across both files, plus a stale comment in `test_cli_contract.py`. Measured by counting `report()` calls: seven in the happy path, five of them findings and two of them lines that are not. | All four texts moved to five, and the count given the reader it never had: `test_check_is_a_verb_and_reports_every_reading_it_names` (renamed from `..._the_three_readings`, which was itself a stale count in a node id) asserts every reading is printed AND that the usage's spelled number matches how many there are. Mutation `f` replants the defect and the proof kills it. | **A number in a usage text with nothing reading it is a declared field with no reader** (D816, D929, D1247) -- and this one was created by the repair for another instance of the same class, one run earlier. §7 question 5 in the documentation: when `blocked_by` was implemented, the reader that got it was the checker and not the help. | -- |
| **D1350** | `test_session_twenty_{two,three,four}_gate_modes.py::test_the_gate_resolves_claims_for_its_own_session`: *"One session literal, and no filename from the gate it was derived from"*, asserting `f"readonly SESSION={SESSION}" in source`. | **It reads the gate's PROSE, and the prose always agrees.** Every gate since 22 opens with *`readonly SESSION=NN` is the only session literal*, so the raw text carries the string whatever the assignment says. Found by battery mutation `h`: `readonly SESSION=24` planted in the Session 25 gate left the guard GREEN. The same module carries a `code()` helper written for exactly this class (D277, D1197) three assertions further down, and this line did not use it. Present in all four modules -- 22, 23, 24 and, by derivation, 25. | The assertion reads `code(source)` in all four, with the reason beside it and a message that says the header may still SAY so. Mutation `m` replants the defect against the repaired guard and it dies. Repaired in every module rather than in the one the mutation found, which is §7 question 5. | **A scan that reads prose measures prose**, and this one measures a sentence written to explain the thing it is checking. The near miss is what makes it worth a row: the helper that fixes it was already in the file, added by an earlier run for the same reason, and the line three above it never got it -- D1197's repair reaching one caller and not the next. | -- |
| **D1351** | This plan's Run 6 step 3: copy the task statement out *"with the SHA substituted where the statement says `<release>`"*. | **The statement carries no such placeholder.** Its `release` member is documented as `<the full commit sha of the clone>` -- a placeholder the WALKER fills by reading `git rev-parse HEAD` in the clone they are standing in. | Nothing substituted, nothing added: `walk-prompt.txt` is the 4204 bytes between the markers, byte for byte, and its sha256 recorded in this run's Done. | Handing the walker the SHA would give them a fact the walk is meant to make them derive, and deriving it is what proves they are in the clone they think they are in. The plan's instruction would have made the record's `release` member the executor's claim rather than the walker's measurement. | -- |
| **D1352** | **The first walk's finding, re-measured.** `bin/migrate.py` catches `migrations.MigrationError` and nothing else. | **`config.ManifestError` escapes `main()`**: `freeze-lock --project` on a manifest naming a directory that is not there prints a full Python traceback and exits **1**, which is not a code the runbook's convention defines. Rig 25m grepped the class properly -- of the twenty-five `bin/` commands that load a project manifest, this was **the only one that raised**; every other answers 2 or 5 with a sentence. **This is D1340 one caller over**: that row repaired `bin/render-config.py` for `MigrationError` in Run 4, having found two callers already had the decision, and nobody grepped for the fourth. | `except config.ManifestError` above the existing clause, printing the message and returning **2** -- invalid operator input, which is what `deploy.sh --render-only`, `render-mcp-catalog.py` and `render-evaluation-report.py` already answer for the same file. Proved with three arms including the one that keeps it honest: a real `MigrationError` still exits 5 (rig 25p, planted digest, tree restored by `cmp`). | The adopter meets it at guide step 9, the first command they run against a manifest they wrote themselves. §7 question 5 twice in two runs: when a decision is implemented, grep every reader -- and D1340's own repair did not. | -- |
| **D1353** | **The first walk's finding, re-measured.** `docs/dev-environment.md` gives `bin/apg.sh dev psql --project project.yaml -- -c '\dt api.*'` and says arguments after `--` are psql's own, forwarded unread. `test_dev_command.py::test_the_wrapper_forwards_psql_arguments_after_a_double_dash_unread` asserts it. | **The documented line has never worked.** `bin/dev.sh` line 145 `shift`s the separator away before forwarding, so `bin/dev.py` receives `-c '\dt api.*'` with nothing to say those are psql's, and its argparse refuses `-c` as an unrecognised argument -- exit 2. **The proof passed because it read the wrapper's SOURCE**, and the wrapper's half was correct; it never looked at the parser on the other side. Its docstring names the reason it did not run the command: *running it needs a terminal this test does not have* -- which is D1354. Two defects held each other up. | The wrapper forwards the separator; `bin/dev.py` splits `argv` on the first `--` before argparse sees anything. The proof now RUNS the command (D1114) and asserts it gets past the parser, with two controls -- an unknown flag before the separator still refused, a bare positional still reaching `rest` -- and keeps the wrapper's source assertions, on the COMMENT-STRIPPED text (D277, D1197, which this assertion needed within a minute of being written, because the repair's own comment says the word `shifted`). | **A proof that measures one of two layers reports on the layer it measured.** The scan was not wrong about `bin/dev.sh`; it was silent about `bin/dev.py`, and silence read as green for two sessions. The untestability was not an obstacle to catching this -- it was the mechanism. | -- |
| **D1354** | `dev_environment.psql_arguments`: *"`-it` so the session is a terminal"*, unconditional. | **`docker exec -t` refuses outright when stdin is not a terminal** -- *cannot attach stdin to a TTY-enabled container* -- so the documented non-interactive form could not run from a script, from CI, or from any test. The walker got past it under `script(1)`, which no page mentions. | `psql_arguments` takes `tty: bool = True`; `bin/dev.py` passes `sys.stdin.isatty()`. The pure module does no I/O and the command makes the decision. The default is unchanged, so the interactive assertion that already existed still passes and the headless arm is added beside it. | It is the reason D1353 survived. A command that cannot be run by a test gets proved by reading it, and reading it is how a two-layer defect hides in the layer nobody read. | -- |
| **D1355** | **The first walk's finding.** `docs/new-team-member.md` step 5: `sha256sum -c docs/source-specification.sha256`, on a page whose every other command runs from the repository root. | **It fails from the root.** The checksum file names `source-specification.md` with no directory -- what `sha256sum` wrote beside the file it digested -- so the check looks for `./source-specification.md` and reports *No such file or directory*, exit 1. No page says to change directory. | `(cd docs && sha256sum -c source-specification.sha256)`, with the sentence saying the `cd` is load-bearing and why, and a subshell so the reader's own directory does not move. | The cheapest possible finding and it survived to the second walk, on the page written for people who have never seen this repository. Nobody had run step 5 from the directory the page runs everything else from. | -- |
...
| **D1370** | This plan's Run 1 step 6, carried into §7, §10 and `CLAUDE.md`: *"where is the outsider's `outputs.json` from the 2026-09-08 bring-up?"* — unanswered through six runs, with `fresh_host` to stay `not_run` if it were never produced. | **It exists, it is a DEPLOYED document, and this release reads it.** It is not in any repository and never could be: `/*-outputs.json` and `.generated/*` are both gitignored and `git check-ignore` confirms it, so neither GitHub remote was ever a place to look. It is on the outsider's own appliance — `2.28.74.158`, `ubuntu-4gb-nbg1-1`, a Hetzner box that started empty — inside the DR kit the bring-up exported: `/home/op/dr-kit-20260908T132930Z/projects/snippets-dev/outputs.json`. Read 2026-09-15: `document_kind: deployed`, `schema_version` **16**, `template_version` 1.0.0, `deployed_through_session` 18, `observed_at 2026-09-08T12:31:42Z`, `host.id` **`apg-snippets-01`**. An `op` account exists there with passwordless sudo, contrary to the operator's recollection. | **Fetched to `~/dr-kits/snippets-2026-09-08/outputs.json`** on ext4 at mode 0600 (the document alone — never the kit's `secrets.txt`, `host.yaml`, `capabilities.yaml` or `bootstrap-state.json`), and put through the product's own reader rather than a hand-rolled check (D1114): `dr_kit.verify_deployed_document` returns **no problems**, with D1326's own two controls reporting — a v3 document *below the facility's first* and a v99 *above this release* are each named rather than accepted. **`APG_FRESH_HOST_OUTPUTS` has a document for Run 7**, and `fresh_host` is attempted rather than carried. | v16 is exactly `KIT_FIRST_OUTPUTS_VERSION`, the bottom of the readable band — so before D1326 this document would have been refused by version and the claim would have stayed `not_run` with a file sitting on a host nobody had looked at. D1326 was written in Run 1 for a different reason and is what makes this answerable. And `host.id` is `apg-snippets-01`, not `apg-vps-01`: the independence `DEP-001` requires is a property of this document rather than an argument about it. | 0197 |
| **D1371** | This session's own `tests/deployment/test_session25_release.py`, written in Run 5: the candidate is rendered by Session 13's `candidate` fixture, which runs `deploy.sh --render-only --capabilities capabilities.example.yaml`. | **The deployment is not rendered from that file, and on this host the difference is total.** `capabilities.example.yaml` is the REVIEWED SET the release compiles its contract from; the operator's own `capabilities.yaml` is what a deploy is run with, and on this host it enables **nothing** -- correctly, and that is D930: the host's manifest does not decide the deployed lock, the committed canonical contract does. So `upgrade verify` compared a candidate carrying eight enabled capabilities against an installed document carrying none and reported **eight differing leaves**, truthfully, about a comparison that should never have been made. Session 13's own proofs survive the same fixture because none of their readings depends on the capability manifest agreeing; mine all did. | A `release_candidate` fixture of this module's own, rendering from the two manifests the deployment records -- the installed `manifest.yaml` and `OPERATOR_CAPABILITIES` -- and the inline render in the `verify` proof moved to the same file. **No fallback to the example**: if the operator manifest is absent the proof fails saying so, because the fallback IS the defect. Session 13's fixture is left alone; four passing proofs depend on it and changing a shared fixture to fix a borrower is how a repair becomes two. | Reusing a fixture without reading what its contract actually guarantees. Its docstring says *what this checkout would render for the deployed project* and it means *from the example capability manifest*, which is true for the readings it was written for and false for every reading layered on top. | 0930 |
| **D1372** | The same module's `test_the_plan_priced_this_release_as_the_class_it_proposes`, also Run 5's: `upgrade plan --json` must report `bump: minor`, `verdict: ok`, `reasons: []`. | **The reading is only true BEFORE the release is deployed, and the sweep runs after.** `plan` prices a candidate against what is INSTALLED; once the deploy has landed, installed and candidate are both `1.6.0` and the planner answers *the candidate is not ahead of what is installed* and blocks with exit 6. Correctly. The proof could not have passed in any sweep of any session -- written in Run 5, first executed on the host 2026-09-15, red on first execution. §7's second question, which asks whether a proof has ever run in the environment it is about. | **The pre-deploy reading stays what it always was: an operator observation**, made at the sheet's `upgrade plan` step and recorded in this run's Done (`bump minor`, `requires patch`, `verdict ok`, `reasons []`, one leaf differing, on both projects). The proof now measures what a sweep CAN measure, and it is stronger than the class: `differences == []` says the installed rendered document and the one this checkout renders agree at **every leaf**, `operator_digests_moved == []` says the candidate came from the deployment's own inputs, and the verdict must be `blocked` for the *not ahead* reason and no other. A release stamped but half-applied fails all three. | A proof written from the plan's narrative -- *the trip reads `minor` on the terminal, so make that a claim* -- without asking at which moment in the trip the sweep runs. The narrative was right about the reading and wrong about who can make it. | 0162 |
| **D1373** | `bin/session-25-check.sh`'s header: exit 5 is *"the evidence was WRITTEN and some claim in it is not `passed`"*. The comment three lines above the suite call: *"the evidence is written whether or not the suite passed, and a failing gate is the run whose output an operator most needs to read."* ADR 0163: three statuses, `failed` among them. D1369, written the day before: `documented_path` is reported **`failed`**, never `not_run`. | **The gate cannot emit a `failed` claim, and never could.** It runs under `set -euo pipefail` and `run_suite` calls pytest directly, so any failing proof ends the run at step 5 -- before step 6 computes the claims and step 7 writes the document. Measured 2026-09-15: 318 proofs passed, the JUnit was written, `documented_path` was genuinely false for the first time in this project's history, and **no `session-25-host.json` existed to record any of it**. `GATE_25_HOST_EXIT=1`, a code the convention does not define. No evidence document in twenty-five sessions has ever carried a `failed` claim, which reads as *nothing has ever been wrong* and means *this path had never executed*. | Both live modes capture the suite's status instead of dying on it, write the evidence, and then exit: the writer's own code when it is non-zero (5, unchanged), else 6 when the suite failed and every claim nonetheless passed. **Both modes, not the one that failed** -- §7's fifth question, and the habit that let D1302 survive three sessions. Proved by a comment-stripped scan in `test_session_twenty_five_gate_modes.py`; battery 5/5, the informative mutation being the one that removes the capture and leaves the words in a comment, which is how D1350 hid for four sessions. | **The most valuable thing this trip found, and it was found by the first claim that was ever genuinely false.** A status the model computes, the gate documents, an ADR defines and a divergence row settled the day before -- and no run could produce it. Everything that made it invisible is the same thing: nothing had ever been wrong enough to try. | 0163 |
| **D1374** | `bin/render-jwks.py`, on both deploys: *"the key set CHANGED: every verifier must be RECREATED, not restarted, before it holds this set."* | **Nothing had changed.** The condition is `changed = write(document, destination)` -- whether the FILE's bytes moved, not whether the key material did. Measured on both projects against their 2026-09-13 documents: alpha `active_kid` `w4OqVzyJ…` and `public_jwks_sha256` `f6608021…` before and after; beta `IlFWmP6x…` and `ecc27cc9…` before and after. **Same kid, same key-set digest, both projects.** The file's bytes move because each deploy materialises a new secret generation (alpha `26c66f0a…` → `785db1d1…`), and the key is carried into it unchanged. | **Not repaired on the day** -- it is a message, the advice it gives is conservative, and every verifier was recreated anyway. Recorded here and in the ledger: the sentence should distinguish *this file was rewritten* from *the key set changed*, and only the second warrants recreating a verifier. A row rather than a repair because a release trip is not where a message on the deploy path gets edited. | An operator reading it at the end of a deploy has to decide whether a production signing key just rotated, and the sentence answers a question about bytes while appearing to answer a question about keys. It is ADR 0195's family seen from the other side: not an unknown reported as an answer, but a *file event* reported as a *domain event*. | 0195 |
| **D1375** | The plan's Run 7 step 7: `bin/session-25-check.sh --mode offline` on the host as `op`, *"as a control that the checkout there is the one measured here."* | **`op` cannot reach the Docker socket, and Session 25 is the first release whose OFFLINE mode needs it.** *permission denied ... unix:///var/run/docker.sock*. Gate 01 came back **1 failed, 5429 passed, 278 skipped**, and the single failure is `SEC-DX-001`'s artefact scan -- which this session wrote to FAIL rather than skip when it cannot scan, because a declared offline claim that passes without scanning is worse than a red (ADR 0202). Gate 25 offline then refused at step 2b with exit **3** and a sentence naming all three places it needs a daemon. Both behaved exactly as designed; the environment is what is missing. `op` never needed Docker before -- deploys run as root, and `apg dev` is a workstation surface. | **`op` was NOT added to the `docker` group.** That is root-equivalent privilege on a production host, no claim requires it, and a release trip is the wrong moment. The host's offline half is a CONTROL and not an input -- the plan's own word is *"too"* -- so the merge takes the workstation's, where Docker works. Named on Stage 4's bill as a provisioning decision. | The proof that surfaced it is the one designed not to skip, and it was the only one of 278 skips-worth of Docker-gated tests that said anything. That is the whole argument for ADR 0202's declared-offline rule, demonstrated on the first host that could not satisfy it. | 0202 |
| **D1376** | This run's own sheet, step 9: `sudo setsid nohup bash /home/op/g25-host.sh … &`. | **It SIGTTIN-stopped.** `[1]+ Stopped` -- a backgrounded `sudo` whose timestamp has expired cannot read a password, so it is stopped rather than run. The sweep never started and no summary file existed. **`g25-host.sh`'s own header documents this exact failure**, inherited from `g24-host.sh`: *"a backgrounded `sudo` that cannot read a password is SIGTTIN-stopped, state T … which cost twenty-five minutes once already."* The sheet that handed the operator the line did not carry the precaution the script it launches describes. | `sudo -v` in the foreground immediately before the launch, in the sheet and in this row. The operator ran `kill %1`, `sudo -v`, then the same line, and it ran. | A trap documented in the artefact and absent from the instruction that uses it. Writing the warning where the reader of the SCRIPT will see it, rather than where the person TYPING the command will, is the same class as D1364 -- a required step that lives only in the failure table. | -- |
| **D1377** | This run's own verification of the D1373 repair: *"shellcheck clean"*, from `shellcheck -S warning bin/session-25-check.sh`. | **The gate runs shellcheck at DEFAULT severity, which includes `info`.** `-S warning` filters `info` out, so a new `printf` carrying backticks inside single quotes passed here and failed CI at step 2 with SC2016. The repair was correct; the check of it was looser than the check that matters, and the difference is one flag. | The backticks removed, and every shell file this run touched re-checked at **default severity** -- the gate's -- rather than at mine. | A local check weaker than the gate's reports green for a tree the gate will refuse, which is the most expensive kind of green: it costs a CI cycle and, on a day with a host trip in it, nearly cost a transport. The same shape as D1376 an hour earlier -- an instruction of mine that did not match the artefact it was about. | -- |
| **D1378** | D1311, this plan's own row: *"One tag, `1.6.0`, on the `main` commit both Run 7 gates measured"*. | **No commit was measured by both gates.** The trip took two sweeps because the first found three defects, and the repairs moved the tree between them: the HOST gate measured `de2aabf` (the release deployed on both projects, and what `source_commit` says in every deployed document), while the OFFLINE and EXTERNAL halves were measured at `13c4b39`. D1311's sentence assumed one sweep at one commit, which is what every previous trip happened to be. | **The tag goes on `main`'s head after the fast-forward, and this row says what each gate measured.** The difference between the two trees is `bin/session-25-check.sh`, two test modules and a plan document -- no render input, and that is measured rather than asserted: beta's rendered document is BYTE-IDENTICAL across the two commits, and alpha's is once the first sweep's own pollution is cleaned (D1371's fixture). CI is green on the tagged commit, which is this repository's full check. The evidence document carries both: `source_commit de2aabf` and `offline_checkout_commit 13c4b39`, printed by the merge rather than folded (ADR 0202). | A rule written for the shape a trip usually has, met by a trip that needed a second sweep. Tagging `de2aabf` would name a tree whose gate cannot write a `failed` claim; tagging nothing would leave the stage's only tag missing. The honest third option is to tag the head and say precisely what was measured where, which is what a reader of a tag wants to know anyway. | 0162 |
...
**THE QUESTION FOR THE OPERATOR — ANSWERED 2026-09-15, see D1370.** It was on
the outsider's own appliance all along, inside the DR kit that bring-up
exported, and this release verifies it clean. What follows is the question as
Run 1 left it, kept because the reasoning is what made the answer findable:
*where is the outsider's `outputs.json` from the 2026-09-08 bring-up?* No path was guessed
