...
| **D902** | Run 5's text: *"**ADR 0176's sibling ADR** records what makes it different."* | **0176 is Run 1's lock check.** Run 5's ADR is **0180**, after 0177 (capability version), 0178 (denial taxonomy) and 0179 (per-capability narrowing). | Stated so the next reader following §5 to ADR 0176 does not find a decision about `uv pip compile`. | D879's shape, second instance. The plan named ADR numbers before three runs had consumed them, which is not a mistake so much as an unavoidable property of planning — and it is why each run checks its own text before costing it. | 0180 |
| **D903** | Run 5's text: *"the quota table is read on **every** request, so its growth is a latency problem before it is a disk one — unlike `agent_audit`."* | **Measured, and it is the reverse.** Every access is a primary-key hit on `(agent_id, window_start)`: the upsert costs **0.0093 ms at 100 rows and 0.0084 ms at 200,000** — flat across a 2000× growth, ×0.91 — while the table goes from 64 kB to **22 MB**. The CONTROL is the same lookup with no index at the same size: **5.56 ms, 659× the indexed upsert**, so the rig can plainly see a slow lookup when there is one. | **Retention is lazy and off the request path** (ADR 0180). Pruning does not belong in the upsert, because it would cost latency on the one path that pays it, to solve a problem that does not exist. | **The plan drew the wrong lesson from the right contrast.** `agent_audit` is unbounded *and nothing prunes it*; the quota table is bounded in SHAPE — one row per agent per live window, 24 a day at hourly windows — and nothing on the request path reads more than one row of it. Had the claim been inherited, the design would have put a `DELETE` in the hot path to protect a lookup that does not degrade. **An assertion about an index is a measurement, and this one took four minutes.** | 0180 |
| **D904** | The fifth budget needs its own check, wherever that lands. | **It cannot afford its own request.** The agent plane holds no database credential (D407), so every database interaction is an upstream PostgREST call as the caller — and a write is already **four** of them, a read three, each holding a connection from a pool shared with human callers. **None of it has ever been timed** (§9, standing since Session 8). A separate quota RPC is a 25–33% increase in round trips on a path whose latency nobody knows. | **The count is taken inside `api.agent_audit_begin`**, which already runs on every audited call. Zero extra round trips, and the refusal is recorded by the path Run 3 built. | **ADR 0141 had already put `begin` before the scope check so that a denial is audited**, and a quota refusal is a denial — so the place that costs nothing is also the only place the refusal is recorded. That is a coincidence worth naming: the constraint (no credential) and the existing decision (audit the denial) point at the same function. | 0180 |
| **D905** | A counter under concurrency needs care about isolation. | **Measured at `read committed`, the level the deployment runs, with ADR 0171's pattern.** Two overlapping `INSERT … ON CONFLICT DO UPDATE … RETURNING`: final count **2**, both exit 0, and the loser **blocks** until the winner commits — 0.94 s against a 0.75 s hold. The CONTROL, the same two as a plain `INSERT`, gives the loser `23505`, so the transactions genuinely overlapped. | **No lost update, and the verdict is the returned count** rather than an error code — the same shape ADR 0171 fixed for the refresh plane. `repeatable read` is rejected: its loser gets `40001`, and retrying a quota increment is how a caller exceeds its quota. | The first control attempt read for the string `23505` and found nothing, reporting the control as failed while the loser HAD raised: **`psql` does not print the SQLSTATE unless asked**. A rig reading for a string its instrument does not emit — the apparatus family again, caught because a failing control is not a result. | 0180 |
| **D906** | ADR 0180's own first draft: *"the quota bound itself is a capability-manifest field, so it is a narrowing under ADR 0179's rule and a `schema_version` question."* | **`AGT-QUOTA-001` bounds an AGENT, not a capability**: *"a windowed quota bounds an agent across requests, survives a process restart, and its refusal is audited."* A per-capability bound is ADR 0179's shape and is decided by the manifest; this one is decided by whoever issued the agent. `app_private.agents` already carries the scopes, the status and the `authz_version` that same person decided. | **Two nullable columns on `app_private.agents`**, NULL meaning unbounded. **It is not a `schema_version` question at all**, which is what keeps D892's two-formats arithmetic true rather than forcing a v4. | **Caught in the run's own ADR, one step before building on it** — §5 puts measurement before implementation for exactly this, and the cost of finding it here was a paragraph. Had the draft been followed, Run 5 would have opened with a schema bump it did not need, and D892's argument against three manifest formats in one session would have been broken by the run that cited it. **An ADR is not evidence; it is a decision, and a decision can be wrong in the direction its author was already leaning.** | 0180 |
...
| **D940** | Migration 0027 *"adds three nullable columns … no existing row is rewritten, and a null `denial_reason` on a historical row is correct rather than missing"* (§4). | **Its CHECK said otherwise, and PostgreSQL agreed with the CHECK.** `agent_audit_reason_iff_refused` was added VALIDATED, so `ADD CONSTRAINT` scanned the table — and the deployment's `agent_audit` held **48 `refused` rows** written by Session 9's and Session 15's live proofs, every one with the NULL reason §4 called correct. *"check constraint … is violated by some row (23514)"*, the whole migration rolled back, the deploy stopped at step 6 with alpha's old release still serving. **No fixture cluster ever had such a row**: `test_agent_audit_plane`'s cluster is built empty and migrated in one pass, so the migration was proved thirty times against a state no deployment is in. | **0027 re-frozen with `NOT VALID`** — legitimate because it is applied nowhere (the cluster carries none of its columns, no constraint, no enum type; the transaction rolled back whole) and D912's window is open until this trip closes it. Recorded here, second instance this session. The fixture now **seeds a historical refused row before applying 0027**, which reproduced the deployment's failure offline in sixteen seconds against the old file, and a proof asserts the constraint is `convalidated = false`, still refuses a new reasonless row, and is refused by `VALIDATE CONSTRAINT` — so the exemption is a real row, not an absent one. | **Question 6, on a migration rather than a proof**: the migration and its fixture agreed because both were written for an empty cluster. §4's sentence was true of the columns and false of the constraint, and nobody asked what `ADD CONSTRAINT` does to rows that already exist. **The generalisable form**: a migration over a table with history has to be proved against a cluster WITH history, and *"nullable, no row rewritten"* is a statement about columns that says nothing about a CHECK. The trip found it in the first minute of the first deploy, which is what the trip is for. | 0178 |
| **D941** | dbmate reports what it applied: *"Applying: … Applied: … in 32ms"* is a migration in `schema_migrations`. | **dbmate printed `Applied: 20260903120027_… in 32.84948ms` for a migration the cluster does not have.** Read after the fact: `app_private.schema_migrations` — the product's table, not `public.` — does not record it, none of 0027's objects exist, and the `Error: pq: check constraint …` sits in the same run's stderr. The line is a false success report from a third party, and it was the line the deploy's stdout showed while the truth was in stderr. | **Recorded, not explained.** The cluster is the authority and it was read; no rig reproduced dbmate's ordering and none is claimed (D267). The deploy's own verdict was right — exit 5, `migrations did not apply` — and that is the line to read. | **D145 and D548's family, in the migration plane**: a third party's success line that does not mean what it says. `postgrest --ready` returned 0 while every request 404'd; `pgbackrest info` exits 0 for a stanza that does not exist; dbmate says Applied. Read the state, never the line. | — |
| **D942** | Runs 6 and 7 changed what an agent write is — two reserved parameters, required — and Run 3 widened `api.agent_audit_begin` to five arguments; ADR 0175's arity guard and the contract suite caught every caller. | **Ten Session 8 and 9 live proofs were callers nobody caught, and the first host gate found all ten at once**: 264 passed, 10 failed, and none of the ten was Session 16's. Every `create_note` / `update_task_status` call in `test_session9_agent_writes.py` and the hidden-write call in `test_session8_agent_plane.py` sent the reviewed function's arguments and nothing else, so the framework refused each for its SHAPE — *"2 validation errors … idempotency_key … dry_run: Missing required argument"* — before the plane saw it. Two more named the audit function at its old arity where the guard cannot look: a `POST /rpc/agent_audit_begin` body (PGRST202) and a `REVOKE EXECUTE ON FUNCTION … (text, uuid, jsonb)` signature (*"function does not exist"*). The other six were downstream: no write happened, so no record agreed with anything. | Every write site sends the two reserved parameters with a FRESH key per call — a reused key would turn the second create into a replay, which is Session 16's guarantee and not Session 9's — and the two direct sites use the five-argument shape. Test-only; the deployment did not move and the release stays `8237c77`. | **D938's other half.** Session 16 wrote no live proof for its own planes, and it also re-ran none of the live proofs its planes changed — the offline twins in `test_mcp_tools.py` were updated in the same runs and went green, and nothing offline reaches a `tests/deployment/` module's request body. The arity guard has **two blind spots the trip measured**: an HTTP body naming an RPC's parameters, and a GRANT/REVOKE signature. Both are §9 entries, not repairs made tonight. And one proof asserted `"error" not in result` while the tool answered `isError: true`, so its own control had already passed on a refused write; the assertion that failed was the JSON parse two lines later. | 0175 |
| **D943** | The claim proofs a gate runs explicitly (step 6) are the same proofs the contract suite runs, so a module green in the suite is green in the gate. | **The gate runs them in a different ORDER, and one Session 9 proof depended on the order.** `static_nodeids_for_mode` returns `sorted(nodeids)`, so `test_a_quota_refuses…` runs before `test_the_audit_identity_comes_from_the_guc…` in a gate and after it in the suite. The identity proof selected every `query_resource` row in the module-scoped cluster and asserted the one it wrote was the only one; Session 16's quota proofs write `query_resource` rows for agents of their own, and in the gate they had already written them. **Second host gate: live suite 100% green, then 396 passed / 1 failed in step 6.** The same failure appeared on the workstation the morning before, when the D940 seed used the same tool name, and was read as the seed's fault. | The proof is scoped to its own agent — the identity the GUC set is what it asserts, so the filter narrows nothing it claims. Reproduced in the gate's order locally (the two quota proofs, then this one) before and after. Test-only; the release stays `8237c77`. | **A shared fixture is a shared table**, and a proof that reads it unscoped is asserting something about every other proof in the module. It held for seven sessions because nothing else wrote that tool name; Session 16 did, and the two orders diverged. The lesson generalises to every module-scoped cluster: scope every read to what the proof itself wrote. And the morning's repair — renaming the seed — treated an instance and left the class, which the gate then found. | — |
...
**Done.** ADR **0180**, migration **0028**, D902–D914 — **thirteen rows, four of
which correct the run's own text**, the highest ratio of the session. **CI green
on `4868d65`, run `33835105484`**, read from that commit's own verdict rather
than inherited from an earlier one (D913, D914).
...
**Done.** ADR **0181**, migration **0029**, D915–D922. **CI green on `30931ab`,
run `33843556276`.** **Every arm of the
