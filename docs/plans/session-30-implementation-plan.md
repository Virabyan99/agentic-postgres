...
**D1559–D1566** and Run 7 **D1567–D1569**; **next free is D1570**, and the
...
D1557–D1558, Run 6 D1559–D1566, Run 7 D1567–D1569. Next free number after
this table is D1570.**
...
`evidence/session-30.json` is expected at **135 claims** (129 + 6), `passed`
**129**, `not_run` **5**, `failed` **1**. The sixth claim is `mirror_retry`,
added after Run 6 (D1566); it is offline, so it is `passed` before the trip
begins. **The fifth `not_run` is `port_allocation`** and it is Run 7's own
choice, not a regression: `--after-reboot` is a declaration that a reboot
happened, this trip performs none, and the claim is reported `not_run`
rather than kept green by a declaration that is no longer true (D1568). A fifth `not_run` is a finding, not
