...
**D1559–D1566**; **next free is D1567**, and the runs add theirs below.
...
D1557–D1558, Run 6 D1559–D1566. Next free number after this table is
D1567.**
...
**130**, `not_run` **4**, `failed` **1**. The sixth is `mirror_retry`, added
after Run 6 (D1566); it is offline, so it is `passed` before the trip begins. A fifth `not_run` is a finding, not
