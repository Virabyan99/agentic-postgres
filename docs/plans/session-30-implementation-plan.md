...
| **D1542** | Stage plan §11: *"Every proof registered to a claim, checked by the registry test, so an orphan cannot read as `passed`"* — stated as an existing property. | **It does not exist.** `test_acceptance_registry.py:190` checks registry → tree (every node id is collectible); `:594` checks every module carries `pytestmark` (D1240); **nothing checks tree → registry.** The orphan carries the right module marks (`p0`, `security`, `live_host`) so D1240's guard is green, and no registry entry names it so `:190` never looks. **Measure (Run 2): how many `def test_` under `tests/deployment/` are node ids of no registry entry?** — the guard's exemption set, if any, is that measured list and nothing else. | **`tests/contract/test_deployment_suite_shape.py` gains `test_every_deployment_proof_is_a_node_id_of_some_requirement`**: every `def test_`/`async def test_` under `tests/deployment/` (by AST, so a decorator or a parametrize does not hide one) is `tests/deployment/<file>::<name>` in some entry's `test_nodeids` (parameters stripped, `strip_parameters`'s rule). Control: a synthetic module written to `tmp_path` with one unregistered test is found by the same function (D374). If Run 2 counts more orphans than the one, they are listed in a frozen `KNOWN_UNREGISTERED` tuple the test compares **for equality** (an orphan removed from the tree must be removed from the tuple), each with the session that owns it, and this row is rewritten with the count. | The `studio_*` lesson (D1236) is that three claims read `passed` over a proof that never ran; the reverse check is the only thing that makes that impossible rather than merely unlikely. An exemption tuple compared for equality is a list something checks. | — |
...
**Done.** _(rig 30a's six arms with `ps` states; rig 30b's three; 30c's
count and names; 30d's count and names; the D1540 grep's answer; the four
ADR numbers as committed.)_
