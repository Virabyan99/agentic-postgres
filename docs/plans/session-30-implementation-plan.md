...
D1557–D1558, Run 6 D1559–D1566, Run 7 D1567–D1572. **Run 8's four PRE-WINDOW rows are
D1573–D1576: every one of them is a line of Sheet B that could not have run,
and all four were found by reading the tree before the window opened rather
than inside it. D1577 is the fifth, and the only one this session could not
determine the cause of. D1578 is Run 8's first row from INSIDE the run: the
prepared key alpha's step 2 refused, and **D1579–D1581 are what alpha's
window measured: the rotation ends at step 6, the two-key overlap the deadline
protects cannot exist, and the mount digest cannot see a secret mount.** Next
free number after this table is D1582.**
...
| **D1564** | Run 3's *Done*: the D972 refusal moved into `bin/lib/tty-guard.sh`, sourced by `deploy.sh:27`, with the message bytes asserted identical on both sides; `bin/session-01-check.sh` clean at that run's close. | **`bin/session-30-check.sh --mode offline` exited 1 at STEP 1**, before any claim was computed: `shellcheck deploy.sh bin/*.sh libexec/*` does not hold the sourced library among its inputs, so it emits `SC1091 (info): Not following: bin/lib/tty-guard.sh was not specified as input` and **exits 1** — an info-level finding is still a non-zero exit, and every gate runs under `set -e`. Measured both ways in one rig: the unrepaired form exits **1** with that message, the form carrying `bin/lib/*.sh` exits **0** with **0 bytes** of output. **Run 3 repaired ONE caller of twenty-six.** `grep -n 'shellcheck deploy.sh' bin/session-*-check.sh` returns 26 lines: `session-01-check.sh:118` carries `bin/lib/*.sh`, and **sessions 02–28 — all twenty-four of the remaining gates — do not**. Every one of them fails at step 1 today, on a checkout where nothing is wrong. | **Repaired in this session's gate first**, with the reason in a comment beside it, and the other twenty-four left named and unedited for the operator to decide — silently rewriting twenty-four released gates to make a number look better is the shape this project refuses. **The operator took the sweep**, so all twenty-six now carry `bin/lib/*.sh`. **It is a WIDENING, not a weakening** (CLAUDE.md §6): the lint covers one more real file, and the form was measured at **exit 0, 0 bytes** — in the `-x`-less spelling `session-01-check.sh` step 2 actually uses — **before** a gate was edited. Every anchor pre-flighted to match exactly once, a miss fatal; all 24 snapshotted and `diff`ed afterwards, each **one line out and eight in** and no other line touched. Measured after: `bash -n` on 26/26, `shellcheck -x` over all 26 at exit 0, and **25 of 25** gates that take a mode still answering an unknown one with exit 2. Targeted: **1506 passed**, the list derived by `grep -rln 'session-.*-check' tests/contract/*.py` — 25 modules. | **§7's question 5, and the largest instance this project has recorded**: *when a decision is implemented, which of its callers got it?* Run 3 greped the readers of the moved NAME and the moved TEXT, as D979 and D1187 require, and the caller it missed reads neither — it is a **glob** that silently stopped covering a directory the release gained. `bin/session-01-check.sh` was repaired because Run 3 ran it; the other twenty-five were not run, so nothing said anything. A gate nobody runs is a caller nobody greps. | — |
...
| **D1566** | §10, carried in from the stage plan: *"The mirror's per-pass transport flake is upstream and no repair here removes it … **D1549's repair** makes the verb report it correctly; it does not stop it."* ADR 0220's header: *"implemented in Run 3"*. | **ADR 0220 was written in Run 2, accepted, and implemented by no run.** Run 3 built the exec discipline and not this; §5 assigns it to nothing; the header's *implemented in Run 3* was a plan the plan did not keep. It sat decided-and-unbuilt through Runs 3–6 while the defect it repairs is the reason both host units spent days `failed` with a mirror bucket that was materially current (D1546). | **Built after Run 6 on the operator's instruction**, with the host's next timed pass ~7 h away. `verb_mirror` runs the copy once more on a non-zero exit before judging it; a second failure is still exit 5, no record, and a failed unit. `mirror_record` gains `retried`, the parser carries it through with absent meaning `False` and a non-boolean meaning *this is not a record*, and **the deployed document gains nothing** — no schema moves. Battery **6/6 killed** (the retry deleted; the retry unbounded; the record forgetting; the parser dropping the field; the parser accepting a non-boolean; the READING gaining the field), each beside a control the mutations cannot reach. Registered as **`OPS-MIRROR-001` → `mirror_retry`, a SIXTH requirement and a sixth claim**, declared offline: five proofs guarding a path a production timer takes nightly, belonging to no claim, is the exact defect this session registered `STU-QUERY-002` to fix (D386, D1236). §2's five become six and §7's 134 becomes 135. **It does not reach the host by itself** — the host runs the launcher's own checkout and the deployed commit is `8c61309`, so Run 7's transport and checkout is what carries it, and tonight's pass runs the old code. | Two readings, both worth keeping. A decision with an **Accepted** status and no run is invisible to every guard this project has: nothing scans an ADR against the tree, and the header said Run 3 with such confidence that four runs read past it. And renaming a proof reached back into a Session 18 requirement — `REC-REPO-001` named the test ADR 0220 replaced, and its description said a copy's *failure* is one failed pass, which is no longer what a failure is. Both moved in this commit (D1119), the description included, because a registry describing behaviour the product no longer has is the silent reconciliation §6 refuses. | 0220 |
| **D1567** | §5 Run 7 step 2 and Appendix Sheet A1: `bin/doctor.sh` both projects → *"11 ok; **the mirror check reads what Run 1 left**"*, and A1's *"the mirror line reading today's copy"*. | **The host's own timer answered ADR 0220's question overnight, with a paired control, six minutes apart.** Read as `op` at 08:32 UTC on 2026-09-19: `agentic-postgres-backup-mirror@alpha-dev` ran 04:42:19→04:42:45 and exited **0**; `@beta-dev` ran 04:48:19→04:48:40 and exited **5**. Same host, same unit template, same 1.7.0 code, no retry. That is D1546's flake reproducing at roughly every other pass, and it is the closest thing to a controlled experiment this defect will ever get: the difference between the two runs is not the code, the host, the network or the hour. **Beta's `mc` line could not be determined** — `journalctl` for a unit is root's, and `op` gets *"No entries"* with a hint naming `adm`/`systemd-journal`. So the failure is reported as exit 5 and the TEXT is the third outcome (ADR 0195), not folded into *"the same flake as yesterday"*. | **A1 gains one line — beta's journal, as root — and A1's expected reading is corrected rather than the host.** Alpha's mirror line names **2026-09-19T04:42**; beta's still names **Run 1's hand copy on 2026-09-18**, because exit 5 writes no record. Both still read `ok`: `MIRROR_STALE_AFTER_DAYS` is 2 and beta's age is 0 days, measured in `diagnosis.mirror`. A sheet that told the operator to expect today's copy on both would have made a correct reading look like a finding. | The repair is already in the checkout and **already on the host as of this run's step 1**: the mirror unit runs `${ROOT_DIR}/bin/backup.sh` from the launcher's own checkout, and that checkout moved to `be987cf` at 08:40 UTC. **Tonight's 04:42/04:48 pass therefore runs ADR 0220's retry whether or not the deploy has happened by then** — which is worth knowing rather than discovering. | 0220 |
| **D1568** | §5 Run 7 step 6: the sweep declares *"every declaration `--help` lists **except** the `--rotated-*` three … and `--dx-record-file` (no person walked) and `--after-reboot` (no reboot)"*; §7 expects **134 claims**, `passed` 130, `not_run` 4, `failed` 1. | **Three of that sentence's four clauses are wrong, and each costs a claim in a different direction.** Measured against `evidence_claims.py`, whose rule is `missing or skipped → not_run`, so every undeclared gate silently downgrades whatever it admits. **(a) `--dx-record-file` must STAY.** It admits `DX-001`'s deployment node id — the proof whose whole job is to *refuse a false record*, which is why `documented_path` reads `failed`. Dropping it turns a deliberate `failed` into a `not_run`: the system would stop being reported as wrong and the evidence would start being reported as wrong instead — ADR 0163's two words swapped. The plan's *"(no person walked)"* reads the flag as a claim that somebody walked; it is not, and Sessions 28 and 29 both declared it and both read `failed`. **(b) `--replacement-host-outputs` cannot be declared at all.** Step 6's *"every declaration `--help` lists"* sweeps it in; there is **no such file on this host** and there never will be, because D1028 ends a rehearsal at the restore and the replacement node is gone. Measured 2026-09-19: absent. Declaring it would fail the preflight; `replacement_host_restore` stays `not_run` by decision either way. **(c) `--induced-alert-file` admits nothing** — `test_session14_observability.py:338` reads it with `os.environ.get`, not a `requires_environment` mark, so it only names which alert an operator induced, and nobody induced one. | **The sweep is Session 29's set verbatim, minus `--after-reboot`, plus `--redeploy-before-file`. One out, one in.** `/home/op/s30-r7-gate.sh` carries the diff and its reasons in the header. **`--after-reboot` stays dropped and the cost is stated rather than absorbed**: it is a declaration that something *happened* (`tests/conftest.py`'s own words), this trip performs no reboot, and declaring it would be §7's own defect pattern — the value that looked measured and was not. `DBX-PORT-001` carries `test_the_reboot_restored_the_projects_from_their_documents`, so **`port_allocation` goes `not_run`**, and `not_run` is the honest word for it. | **§7's arithmetic moves: 135 claims, `passed` **129**, `not_run` **5**, `failed` **1**.** The fifth `not_run` is `port_allocation`, joining `api_authorization`, `bootstrap_identity`, `credential_rotation_planes` and `replacement_host_restore`; `deployment_convergence` still leaves that list if DEP-002 passes. Exit 5 for those reasons and no other. **If the operator would rather keep `port_allocation` green than keep the declaration true, that is a decision and not a correction** — it is the one thing on this trip that a sheet cannot settle, and it is reversible by re-running the sweep with the flag. | 0163, 0195 |
| **D1569** | Appendix Sheet A1, first line: `bin/upgrade.sh check --project alpha-dev` → *"both versions the installed one, `verdict OK`"*. | **That is a PRE-transport reading, and A1 is issued AFTER step 1.** §5 Run 7 orders transport-then-A1, and step 1 moved the host checkout from `8c61309` to `be987cf` at 08:40 UTC. `bin/upgrade.py:271-330` reads `template_version` from the DEPLOYED document and `release_version` from the CHECKOUT, and since F-020/D1423 it also reads both commits. With the checkout moved, the two versions are no longer the same value and `commits_agree` is `False`. | **A1's expected reading is rewritten to what the command will actually print**: `installed 1.7.0`, `this release 1.8.0`, `deployed from 8c61309…`, `this checkout be987cf…`, and `commits DIFFERENT`. `verdict` is still **`OK`** — the verb's verdict answers *can a comparison be made*, not *do the two agree*, and it is `UNDETERMINED` only when nothing is installed. | **`DIFFERENT` here is the reason to deploy, not a finding.** The trap the sheet would otherwise set is precisely the one D1423 was written about: a host a release behind in everything but its version file read identically to a host that agreed, so the verb learned to print the commits. A sheet that calls the new, informative reading an anomaly would teach the operator to distrust it. | 0195 |
| **D1570** | Appendix Sheet A2 step 2, written in Run 6: the sentinel row is planted with `sudo bin/dev-token.sh … --role authenticated -- bin/api.sh … create-note --title …`, *"through the product's own surface (D1114)"*, because *"`api.create_note` derives ownership from the request identity"*. | **It cannot work, and it failed on its first execution at 09:0x UTC on 2026-09-19**: `{"code":"PT401", "message":"AP401: no request identity for this transaction"}`, `api: the service answered 401`. `bin/dev-token.py`'s own module docstring is the whole explanation: *"Nothing about the token is chosen by the caller, and **it carries no subject at all**. … Migration 0013 compares a subject against `app_private.users` inside the request transaction, so a derived subject names nobody and is refused. **This issuer names a role; the auth service names a subject** (ADR 0095)."* `owner_session`'s docstring prices it exactly: `auth_claims_are_current` is an EXISTS over five equalities including `credential_version`, `authz_version` and an exact scope array, and a bootstrap-issued token carries none of the three. **This is D298's eleventh occurrence** — *"the ten proofs that used one all returned `AP401` the first time a host gate ran after 0013"*. | **The row is planted the way the product's own isolation proof plants one** (`test_session3_isolation.py:167-172`): `SET ROLE "apg_alpha_dev_authenticated"; SET app.user_id = '<uuid>'; SELECT (api.create_note('<title>')).owner_id;` through the superuser's `psql`, with `docker exec` and **no `-i`** (the statement is in argv). It is still `api.create_note` — the product's own RPC, running as the product's own role — and the identity is SET rather than carried by a token. It is symmetric with Sheet A4's delete, which this plan already concedes to the superuser because **no product surface deletes a note**. | **Why Run 6 could not see it, which is the reusable half.** The plan says that recipe was derived *"from the proof that reads the file and the two `--help` pages it needs, never from memory"* — and it was. **Neither `--help` page mentions the missing subject.** `dev-token.sh --help` says the role is *"one of: anon, authenticated, docs"* and says only that the `docs` role is subjectless *on purpose*, which reads as though the other two are not. The decision that `authenticated` is subjectless too lives in `dev-token.py`'s module prose and in ADR 0095, and an operator surface that does not restate it cannot be derived from safely. **A recipe derived from a command's `--help` inherits whatever that `--help` leaves out** — which is a sharper form of §7 question 2 than *has it ever run*: it had never run, and the thing that would have caught it was reading the implementation of a command whose operator surface looked complete. | 0095 |
| **D1571** | §5 Run 7 step 3: after alpha's deploy, *"auth/storage/mcp **NOT** recreated (their mounts did not move — read `docker ps` ages)"*; Sheet A2's read: `sudo docker ps --filter label=apg.project.key=alpha-dev` → *"**no container younger than the deploy** (ADR 0155: no mount moved)"*. | **Both readings are wrong, in opposite directions, and the first is self-contradictory.** Measured after the 09:29 UTC deploy: `auth` **46s**, `storage` **46s**, `mcp` **47s** — the three the plan named as the ones that would NOT move — while `postgrest`, `docs`, `store`, `edge-probe` and `metrics` all read **25 hours**. `compose.yaml:110-113` is the reason, in the model's own words: *"the source path is an immutable generation directory **whose identifier does not exist until materialization runs**"*, and `runtime_override.mounted_digest` hashes *"**Path as well as bytes**, so that moving a mount to a different destination with identical content still counts as a change"*. `materialize-secrets` writes a NEW generation on every deploy — which is Sheet A2 step 3's own premise and DEP-002's entire control — so every secret-holding service's mount PATH moves on every deploy and ADR 0155 recreates it, necessarily. **The plan asserted that the generation moves AND that the three services mounting it do not; those cannot both be true.** Confirmed here: `59ce4a78cafc3be6` → `c18ea3c95b89521f`. `postgrest` is the control — it takes its credential through the environment rather than a mount, and it did not move. | **The expectation is inverted for every future trip: the secret-holding services are recreated on every deploy, and nothing else is.** That is the reading that distinguishes a converging deploy from a no-op, so it is worth MORE than the one the plan asked for. Sheet A3 was issued with it corrected before beta ran. | **And the filter returns EIGHT, not ten** (D587, second order). `apg.project.key` is a first-party label the model applies to edge-facing services and to **none of `postgres`, `pgbouncer` or `dbmate`** — `runtime_override.py:41-48` measured that on this host when `bin/backup.py` selected the cluster with it and matched zero containers on a healthy deployment. The doctor's `10/10` counts the two the label deliberately excludes, so `8` and `10/10` are the same fact read through two selectors. **D587's own comment now undercounts**: it says *six* edge-facing services, and the deployment shows eight — `metrics` and `store` joined in Session 14 and the prose did not follow. Nothing reads that number, so nothing went red; it is D1488's fifth occurrence and Session 31's to repair. | 0155 |
| **D1572** | §5 Run 7 step 6: the sweep expects `studio_tenant_read` **passed** — *"first execution of the orphan"* — and §7 expects `failed` **1**. | **It errored at setup again, and not for the reason Run 5 repaired.** Run 5 fixed the stranger's `ARRAY[]::text[]`, which migration 0011:116 refuses (`CHECK (array_length(scopes, 1) IS NOT NULL)`), and that was genuinely the first thing that broke the fixture. The host revealed the next one behind it: seeding the **auditor's own** row returned **403 `{"code":"42501", "message":"permission denied for schema api"}`**. `two_owners_one_relation` and `auditor` both create their subject with `role_name = project_a["database"]["roles"]["project_admin"]` and then write both rows through `POST /rpc/create_note`. **`project_admin` holds no `USAGE` on schema `api`, correctly** — a `project_admin` that could write application rows through `api` would be the defect, and `owner_session` uses `authenticated` for exactly this reason. **The product is right and the instrument is wrong.** The fixture's own docstring says the rows are written *"as their owners, with their own tokens"*; the owners it creates cannot write. | **Recorded and left**, which is §5 step 6's own rule for a defect found in the window. Repairing it is a one-line change to a fixture, but the sweep that records the evidence would then measure a tree that is not the deployed release, and §9 forbids taking a deploy to make a claim move. **Session 31 repairs it**: create both subjects with the `authenticated` role, the one `owner_session` uses, and re-run. | **The registration did exactly what it was for, and that is the finding worth keeping.** Before this session the same proof errored at setup and belonged to NO claim, so three studio claims read `passed` while it had never run (D386 + D1236) — which is why `STU-QUERY-002` was registered at all. It still errors, and now `studio_tenant_read` reads **`failed`**. The defect moved from invisible to reported without anything about the defect changing. **A claim reading `failed` here is the mechanism working, not failing.** Two second-order notes: this is the **sixteenth** never-executed proof to fail on its first execution, and it is the **second failure in one fixture** — Run 5 repaired what an offline run could see and could not see this one, because no offline run reaches a deployed `api` schema's grants. §7's arithmetic, now MEASURED rather than predicted: **135 claims, `passed` 128, `not_run` 5, `failed` 2.** | 0163 |
| **D1579** | Appendix R step 7 and Sheet B steps 7-8: *"Wait for the deadline, then `retire`, then redeploy and recreate"*, and *"`status` → `phase steady` and the D1475 line"*; §5 Run 8: *"the deadline is 930 s from `promote`; `retire` after it; then the last redeploy and recreate."* | **The rotation was already over when step 6 finished, and `retire` refuses.** Measured on alpha, 2026-09-19: after step 6's redeploy + `down` + redeploy, `status` read **`phase steady -- one key, nothing in flight`**, `active snuFu_ZGOVAe…`, one kid listed, `acknowledged nothing has been asked`. Not `PROMOTED`. Step 7 was then run anyway so the refusal would be on the record rather than assumed: **`rotate-signing-key: refused: no rotation is in flight; there is nothing to retire`**, exit 6 — `jwt_keys.retire_rotation:476` raises when `retire_after is None`. The deploy had rewritten the whole `jwt` member from the rendered key set, discarding `retire_after`, `verifier_acknowledgements` and the second `verification_kid`. **The state the deploy produced is `retire`'s own end state** — `retire_rotation` returns `verification_kids: [active_kid]`, `retire_after: None` — differing in exactly one member: `verifier_acknowledgements` is `None` where `retire` writes `{}`, which is **D1475's literal, from the other side**. | **Alpha's window closed at step 6 and steps 7-8 were reduced to measurements.** Step 8 was already satisfied: `GET https://alpha-db.agenticpostgresql.com/api/app/auth/jwks.json` → 200, **one key, `snuFu_ZGOVAe…`, the retired `w4OqVzyJ…` absent** — taken from the workstation with no root, and that document is derived from the **auth service's in-process signing key** (`services/auth-api/app/keys.py:89`), so it proves the issuer really moved, which is the one state `promote` says it cannot detect. `acknowledge` then read all three verifiers holding `f693eded…` through `/proc/<pid>/root`, and `doctor` 11 ok. Sheet C keeps steps 1-6 and replaces 7-8 with those three readings. | **The runbook describes a four-phase cutover and the product performs a three-phase one.** Rig 28d rehearsed `retire` *"both ways: refused before the deadline, accepted after"* — against a rig whose document it edited directly, never against a deploy. **Step 6 is one of the two steps the rehearsal could not cover** (*"not rehearsed — a provider edit"*), and it is the step that ends the rotation. §7's second question, answered the expensive way: `retire` has executed successfully exactly once, in a rig, on a path no operator can reach. | 0088 |
| **D1580** | Appendix R: *"the signing-key proof runs after step 7, not after step 5 (D1470). It asserts the retired `kid` is **absent** from `verification_kids`, which is exactly what `retire` does and what `promote` deliberately does not: **between them both keys are published on purpose**."* `jwt_keys.retire_rotation`'s docstring: *"Refusing early is the half that matters. Retiring before the window closes invalidates tokens that are still inside their own lifetime."* | **Nothing publishes the retiring key after step 6, so the overlap the deadline protects cannot exist, and the refusal that protects it is bypassed rather than triggered.** `bin/render-jwks.py:203-245` builds the published set from **three files on disk** — auth's signing key, the bootstrap key (retired in Session 15), the prepared key — and **never reads the document's `verification_kids`**. Step 6 requires clearing `APG_AUTH_JWT_PREPARED_KEY`, so the next deploy has one key to publish and publishes one. Measured as the digest moved `f6608021…` (one key) → `ccb3c58b…` (two, the PREPARED phase) → **`f693eded…` (one again)**, the last at step 6 and roughly fifteen minutes before `retire_after`. **The 930 seconds guard a record, not a set**: `retire_rotation` refuses early, but by the time it could be called the deploy has set `retire_after` to `None` and it refuses for the other reason. A token signed with the retired key inside its own 900-second lifetime is refused from the moment step 6's deploy lands — the exact harm the docstring names, arriving by the path the runbook prescribes. | **Recorded, not repaired, and not repaired inside the window** (§9). The end state is correct and the rotation is sound; what is wrong is the claim that the overlap spans `promote`→`retire`. A repair is a design question — the product would have to retain the retiring key's public half to keep publishing it, and `verification_kids` holds kids rather than JWKs — so it needs an ADR of its own and Session 31 owns it. **The operational reading for Sheet C is the inverse of the runbook's**: do step 6 PROMPTLY rather than carefully, because every second between `promote` and step 6 mints more tokens with the key step 6 will invalidate. | **This is the deepest form of §7's pattern in the project so far: a guard that cannot fail for the reason it is watching for** (D509). `retire_rotation`'s early-refusal branch is real, tested, and rehearsed both ways — and on the only path an operator can walk, the condition it guards has already been destroyed by the time it is consulted. It is not dead code and it is not wrong; it is unreachable. Nothing offline could have found it: the rig edited the document, and the deploy is what overwrites it. | 0088 |
| **D1581** | **D1571**, this session's own row, written seven hours earlier: *"the secret-holding services are recreated on every deploy, and nothing else is … `materialize-secrets` writes a NEW generation on every deploy … so every secret-holding service's mount PATH moves on every deploy and ADR 0155 recreates it, necessarily."* | **The mount-digest label cannot see a secret mount at all, and `auth` was running four generations stale.** Measured at 11:29 UTC: `auth` started **09:50**, mounting `…/generations/0f3b0bc398ee5481/auth/auth_jwt_signing_key.pem`, while the active generation was `c1c212f8bbebed11` — four materializations later. Its label read `7a203ce4…`, **byte-identical to what the deploy had just written for it** at 11:16, so Compose was right not to recreate it. After the `down` + redeploy put it on `e8cc5a95fbe9bf77`, the label was **still `7a203ce4…`**. The cause is one word in `runtime_override.mounted_paths_by_service:960`: *"every bind-mount source, per service, **out of the rendered override**"* — it parses **one** payload, and the secret mounts are written to a **different** file, `secrets-compose.override.yaml`. So a generation move is invisible to the digest by construction. Corroborated by a coincidence that is not one: `storage` and `postgrest` carry the **same** digest, `da33ccab…`, which is only possible if the one source each hashes is the same file — `…/rendered/alpha-dev/jwks.json`; `mcp` differs because it mounts that plus its capability lock (`:187`). | **D1571's OBSERVATION stands and its MECHANISM is withdrawn.** What recreates a service on a deploy is a change to its *rendered* mounts — which is why step 2's deploy recreated exactly `postgrest`, `storage` and `mcp`, the three that mount `jwks.json`, and why step 2b came back clean (D1473 answered). What does **not** recreate a service is a new secret generation. **Why `auth` WAS recreated at Run 7's deploy and not at this one is UNDETERMINED** and is reported as such (ADR 0195) rather than folded into the mechanism above. §10 carries it. | **The function derives its inventory from a document precisely so it cannot go stale — and derives it from one document of two.** Its own docstring names the defect it would become: *"a stale inventory here means a service whose content changed is silently not recreated — which is the defect this is for."* That is now measured, on the one service where it matters most: **the issuer.** Step 6 moves the signing key, and had we followed *"redeploy"* without Appendix R's *"and recreate"*, `auth` would have kept signing with the retired key while the document said otherwise — D1579's end state reached with the wrong key. **The recreate is not belt-and-braces; it is the only thing that moves the issuer**, and this run is the first evidence for it. | 0155 |
| **D1578** | Sheet B step 1 and `docs/operator-guide.md` §15.1: generate the key with `openssl genpkey …` and paste it at `APG_AUTH_JWT_PREPARED_KEY`. Step 2 is a redeploy, and the sheet's only failure mode for it is *"`render-jwks` prints … the key set CHANGED — or cannot be told from here (both normal)"*. | **An operator-supplied PEM is never checked until `openssl` refuses it deep inside a deploy, and the refusal is designed to say nothing.** Alpha's step 2 failed at 11:01 UTC on 2026-09-19: `deploy: the verification JWKS could not be derived: render-jwks: openssl failed: openssl rsa -in …/auth_jwt_prepared_key.pem`, and that is the **whole** message — `render-jwks.py:141-144` suppresses `stderr` deliberately and correctly, *"openssl names the key's path and, on some failures, prints material from it."* Diagnosed from the file's SHAPE with a purpose-written script and root: 1649 bytes, 25 newlines, longest line exactly 64, **`BEGIN` lines 0 and `END` lines 0**, first 27 characters `MIIEvQIBADANBgkqhkiG9w0BAQE`. Against the same generation's active signing key as a control — 1704 bytes, 28 lines, accepted by the same `openssl` in the same invocation — the difference is **exactly 55 bytes**: `-----BEGIN PRIVATE KEY-----\n` (28) plus `-----END PRIVATE KEY-----\n` (26) plus the absent trailing newline. **The body survived the paste byte for byte; both delimiter lines did not.** Meanwhile `bin/bootstrap-providers.py:161-163` validates exactly this — *"if `BEGIN PRIVATE KEY` not in pem or `END PRIVATE KEY` not in pem: raise"* — **but only for a key the product generates itself**, the one path that cannot be wrong. `materialize-secrets` writes an operator-supplied value to disk without reading it, and `value_kind: rsa_private_pem` has no validator anywhere in `src/`. | **The provider value is repaired by ADDING the two lines rather than re-pasting the body**, and verified before the next deploy by running `materialize-secrets` alone and re-running the shape script against the new generation — a check that costs seconds where the deploy costs minutes and leaves the project half-converged. The missing validation is **not built inside the window** (§9: nothing is repaired inside a window that can be repaired after it); §10 carries it. | **The failure was safe, fast and almost undiagnosable, and those are not in tension — they are the same decision seen from two sides.** Suppressing `stderr` is right; the cost is that the one class of value a human supplies by hand is the one class whose failure carries no reason. **`value_kind` is a declaration with no reader** (D816, D1247) in the direction that costs an operator a deploy: the contract states the kind, `bootstrap-providers` enforces it where it is guaranteed to hold, and nothing enforces it where it can fail. The check belongs at materialization, which has the bytes, runs as root, and can name the problem without printing the value — *"this is not a PKCS#8 PEM: no BEGIN line"* reveals nothing a shape script had to be written to discover. **And the deploy had already materialized a new generation and deferred four services before it looked at the key**, so a value that could have been rejected in the preflight cost a half-converged project instead. | — |
| **D1577** | Run 7's *Done*: after the trip, alpha's active secret generation is `c18ea3c95b89521f` and beta's moved likewise; the containers read *"`auth`, `storage`, `mcp` recreated on both projects; the other five untouched at 25 h"*. | **Both projects were redeployed AGAIN after the sweep started, and as `op` it cannot be determined by what.** Measured 2026-09-19 at 10:33 UTC: `apg-diag generation alpha-dev` reads **`0f3b0bc398ee5481`**, not `c18ea3c95b89521f`; beta reads `7dd329b19488363d`. All **ten** containers on each project report `Up 41-44 minutes` — started ~09:51 UTC — where Run 7 measured five of them at 25 h. The sweep launched 09:38 and ran ~15 minutes, so the restart falls inside its window, and `materialize-secrets` writing a new generation on every `up` (D1571) is what moves that pointer. The natural reading is that the host suite's own convergence proofs deploy. **It is a reading and not a measurement**: the deploy transcripts and `/var/lib/agentic-postgres/secrets/` are root-owned and `op` gets `Permission denied`, so no `op`-side command can name the caller. | **Reported as the third outcome, not folded into *"the sweep redeployed"*** (ADR 0195). Nothing is repaired and the rotation is not blocked by it: step 0.2 is the gate that says whether the deployment is steady, and it read `phase steady -- one key, nothing in flight` with `published f6608021…` — the same digest the 09:38 copy records, so the **key set did not move** even though the generation did. What it does settle is **D1575**: the `/home/op` copies went stale between Run 7's sweep and Run 8, which is why passing one to a command that writes would have produced a window that could never converge. | **Two things worth carrying.** A generation id is the control DEP-002 uses to prove a redeploy RAN (D509) — and it moves for reasons other than the redeploy under test, so a before/after comparison taken across a sweep measures the sweep as well. And *the deployment is what Run 7 left it* is a premise Run 8 would otherwise have started from: five hours and an unattended sweep separate the two, and the only thing that made the difference visible was reading container ages that had no reason to be interesting. | 0195 |
| **D1573** | §5 Run 8 and Appendix Sheet B step 0.1, and `docs/plans/session-28-implementation-plan.md` Appendix R 0.1: capture the retiring key set with `cat /opt/agentic-postgres/rendered/<key>/jwks.json`. | **There is no `rendered` directory under `/opt`, and the tree names a different root.** `deployed_output.RENDERED_ROOT` is `Path("/var/lib/agentic-postgres/rendered")` and `tests/contract/test_project_state_roots.py:60` asserts that exact string against the constant, `:73` the per-project path, `:98` the same value as a `readonly` in the shell. Measured on the host 2026-09-19 at 10:2x UTC: `/opt/agentic-postgres` **does exist** — it holds `releases/0833a7ffa6be…/`, an August release tree — and has no `rendered/` member at any depth. | **Step 0.1 is re-aimed, and then replaced outright** (D1574 is why). The row is kept separate from D1574 because fixing the path alone does not make the step runnable. | **The directory existing is what made the path read as correct.** A wrong path under a root that is absent fails the first time anybody types it; a wrong path under a root that is *present and plausible* survives two plans and a released operator guide, because every reader who glanced at `/opt/agentic-postgres` found something there. The host keeps two trees whose names both mean *what was rendered*, and the one the product writes is under `/var/lib`. | — |
| **D1574** | The same step 0.1, in both plans: *"The file is `0444` by design, so `op` can read it without root."* Appendix R marks 0.1 as the agent's line rather than a **[sudo]** one on the strength of it. | **The premise inverts what the code says, and `op` cannot read the file.** Measured: `/var/lib/agentic-postgres/rendered` is `drwx------ root root`, so the traversal fails before the mode is consulted — `Permission denied` as `op` for both projects. `rendering.py:122` is explicit about why the mode is safe: *"`0444` and not `0640 root:999`: the enclosing directory stays `0700` root-owned on the host, so widening this does not make the file readable by anyone who could not already traverse to it"*, and `:2319` repeats it for the migration modes. The mode is permissive **because** the directory is not; the plan read the mode as a grant. | **Step 0.1 is taken from the PUBLISHED endpoint instead, and it needs no root at all.** `GET <routes.app.url>/auth/jwks.json` (D1576) returned 200 for both projects; each published set holds exactly one key; the object was written out as a **single JWK** rather than a key set, because `test_session5_convergence.py:596-598` reads `retired["kid"]` and a `{"keys": […]}` document would raise. The control is the deployed document: `kid == jwt.active_kid` asserted true for both, and the object was refused if it carried any of `d p q dp dq qi k`. Kept at `/home/op/s30-retired-<key>-jwk.json` (0600 op) and on the workstation. **alpha `w4OqVzyJ0Z7wHxr9aZqNyid90Yx1ljQLI8-IVOM3x8o`, beta `IlFWmP6xqSJty8PQcAsWIbnueFoN14RRBCMcy3qJSzI`.** | **A file mode is not an access decision; the path to it is.** This is §7's *value that looked measured and was not*, in its quietest form: somebody read a real comment, in the right file, about the right constant, and took the second half of a sentence whose first half was the condition. The repair is better than the original rather than equal to it — the endpoint is what the plane actually publishes, where the rendered file is what the deploy wrote, and ADR 0215 is the decision that those are different readings. | 0215 |
| **D1575** | Sheet B's preamble: `OUT=/home/op/<key>-dev-outputs.json` (the current pair, **D1494**), used as `--outputs` for **every** step including `acknowledge`, `promote` and `retire`. `docs/operator-guide.md` §15 and Appendix R 0.2 say the same. | **`--outputs` is not a read. Every mutating step WRITES the file it names.** `bin/rotate-signing-key.py:505-507`: `write_back(arguments.outputs, document, updated)`, and `write_back` calls `deployed_output.write_deployed_document(document, path)` — which writes **the path it is given**, atomically, `0600 root` (`:877-911`). The canonical document is `PROJECT_STATE_ROOT/<key>/outputs.json` = `/etc/agentic-postgres/projects/<key>/outputs.json` (`deployed_output.py:48`), which is what Run 7's own sweep passed to `--project-a-outputs`. Pointing `--outputs` at `/home/op/<key>-dev-outputs.json` would have recorded the acknowledgements, the promotion and the retirement into an **op-owned copy**, left the deployment's own document untouched, and chowned that copy to `root:root` on the way out. And the copies are **already stale**: they were installed at 09:38 UTC and something redeployed both projects at ~09:51 (D1577's subject), so `acknowledge` would have compared each container's live digest against a superseded `public_jwks_sha256` and reported all three verifiers behind, permanently — a window that cannot converge, sending the operator round Appendix R's *"if acknowledge comes back dirty"* loop at one deploy per turn. | **Every `--outputs` on Sheets B and C is `/etc/agentic-postgres/projects/<key>/outputs.json`.** Confirmed at 0.2: `phase steady -- one key, nothing in flight`, `acknowledged nothing has been asked`, `published f6608021…`, issuer `https://alpha-db.agenticpostgresql.com/api/app/auth`. `docs/operator-guide.md` §15 carries the same error and is a released page; Run 9 repairs it. | **A rule about which copy to READ was applied to a command that WRITES.** D1494 is entirely about `--project-a-outputs` and the gate — its harm is *measuring the wrong release*, and its remedy is *name the `-dev-` file*. Nothing in it is about a mutation, and the sheet inherited it by filename rather than by the rule's subject. **The failure mode is the one this project fears most**: a `promote` is irreversible, it would have printed success, the copy would have read exactly right afterwards, and the deployment would have been untouched. It fails *loudly* only by the accident that the copy is stale; had the sweep not redeployed, the acknowledgements would have matched and the promotion would have been recorded nowhere that matters. | 0158 |
| **D1576** | Sheet B step 8: `curl -sS https://<the project's auth route>/auth/jwks.json` → *one `kid`, not the retired one*. | **The deployed document publishes no route named `auth`.** `routes` holds exactly eight members on both projects — `app`, `app_docs`, `docs`, `health`, `mcp`, `metrics`, `rest`, `storage` — measured 2026-09-19. The auth service sits behind the **`app`** route, which strips `{api.public_base_path}/app` before forwarding, *"because the auth service serves `/auth/login` and `/admin/users` at its root"* (`naming.py:395-401`); `services/auth-api/app/routes.py:635` registers `GET /auth/jwks.json` at that root. So the URL is `<routes.app.url>/auth/jwks.json` = `https://alpha-db.agenticpostgresql.com/api/app/auth/jwks.json`. | **Measured, not reasoned: both projects answered 200 and this is how D1574's repair captured the retiring keys.** Independently confirmed by 0.2, which prints `issuer https://alpha-db.agenticpostgresql.com/api/app/auth`. Step 8 on both sheets names the `app` route and derives the URL from the document rather than from a route name that does not exist. | A sheet's last step is its least rehearsed line (D1199's shape, one level up), and this one sits at the end of a sixteen-minute window with an irreversible step behind it. An operator who reached step 8 and found no `auth` route would be hunting for a missing route at the exact moment the correct reading was one prefix away. | — |

---

## 2. What the session adds to `tests/acceptance-registry.yaml`

**Five requirements, five claims, all `target_session: 30`, all P0** — and a
**sixth of each arrived after Run 6**, `OPS-MIRROR-001` → `mirror_retry`, when
the operator called for ADR 0220's repair to be built (D1566). Every
...
**Follow-on, after this run closed green: the operator took D1564's sweep.**
All twenty-four remaining gates now carry `bin/lib/*.sh`, so all twenty-six get
past step 1. It is a **widening** (CLAUDE.md §6) — the lint covers one more real
file — and the form was measured at **exit 0 with 0 bytes** in the `-x`-less
spelling `session-01-check.sh` actually uses, *before* a gate was edited. Every
anchor pre-flighted to match exactly once with a miss fatal; all twenty-four
snapshotted and `diff`ed afterwards, each **one line out and eight in**, no
other line touched. After: `bash -n` on 26/26, `shellcheck -x` over all 26 at
exit 0, **25 of 25** mode-taking gates still answering an unknown mode with exit
2, and **1506 passed** across the 25 modules `grep -rln 'session-.*-check'
tests/contract/*.py` names. One pre-existing oddity was read and left alone:
`bin/session-02-check.sh` holds no `readonly SESSION=` at all, because it
...
|---|---|---|---|
| `exec_discipline`, `release_reading_ref`, `contract_compile_output`, `suite_shape`, `mirror_retry` | offline | the gate's offline mode here (Run 6); CI | `passed` in `evidence/session-30-offline.json` and in `evidence/session-30.json` |
...
`evidence/session-30.json` is **135 claims** (129 + 6), `passed` **128**,
`not_run` **5**, `failed` **2** — MEASURED on 2026-09-19, not predicted.
The sixth claim is `mirror_retry`, added after Run 6 (D1566); it is offline.
**The fifth `not_run` is `port_allocation`** and it is Run 7's own choice,
not a regression: `--after-reboot` is a declaration that a reboot happened,
this trip performs none, and the claim is reported `not_run` rather than
kept green by a declaration that is no longer true (D1568). **The second
`failed` is `studio_tenant_read`**, whose proof errored at setup one layer
below the error Run 5 repaired (D1572) — the claim reporting it is the
registration working, and Session 31 repairs the fixture.

**`deployment_convergence` PASSED**, for the first time since Session 11:
`--redeploy-before-file` was declared on a trip for the first time, the
sentinel row survived the redeploy, and the secret generation moved
`59ce4a78cafc3be6` → `c18ea3c95b89521f`, so both halves of DEP-002 —
the row and its control — were answered by one deploy. A fifth `not_run` is a finding, not
