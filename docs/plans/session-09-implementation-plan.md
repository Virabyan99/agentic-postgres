...
**Next free number after this table is D512.**
...
| **D506** | `docs/session-09-operator-guide.md` §4 step 4 and §1: *"Apply migrations 0019 and 0020, on BOTH clusters, BEFORE the deploy"*, with §1 arguing the ordering is what stops a `42501` appearing where the boundary's own `AP401` belongs (D475). | **The step cannot be executed, and the deploy already does it — in the right order and more strictly.** `bin/migrate.sh status/up` run dbmate in a container, so they need the runtime override, which `_write_root_only` can only produce as root; and they read a rendered migration SET whose location depends on the flag. Without `--runtime` the set is the checkout's `.generated/<key>/`, which §4 step 2's `--render-only` has just atomically replaced (`shutil.rmtree` → `copytree` → `os.replace`, `bin/deploy-project.py:404-426`) with a staged copy that structurally cannot contain the override — `bin/compose.sh:451`, `die 3`. **With `--runtime` the set is `/var/lib/agentic-postgres/rendered/<key>/`, which is installed BY the deploy** and therefore still holds the previous release's migrations. Measured on alpha-dev: `/var/lib` carried 18, the checkout 20, and `--runtime status` answered **`Applied: 18, Pending: 0`, exit 0** — 0019 and 0020 not reported pending but absent, a green line for work that had not happened. Meanwhile `deploy-project.py:1553` is `step("6. Bootstrap and migrate the cluster")`: it bootstraps the roles, runs `migrate.sh --runtime up` against the set it installed four steps earlier, `fail`s the whole deploy if they do not apply, and only then reaches `step("6b. Start the deferred services")`. | **Step 4 is deleted; the deploy is the mechanism.** The guide now runs the deploy and then verifies with `--runtime status` that the ledger reads 20 — a check of the deploy's work by a second reader, not a substitute for it. `--render-runtime-only` is documented as the narrow repair for a missing override, and the `--runtime` flag is documented as reading the INSTALLED set, one release behind until the deploy publishes. | **D475's property is real and is enforced by the product rather than the runbook** — the agent plane starts in 6b, after 6 has migrated, so it never serves a write against an unmigrated cluster. The guide restated a guarantee the deploy already makes and, in restating it, asked for an ordering the tooling forbids. **CLAUDE.md §6's whole subject, arrived at from a new direction: the wrong answer coincided with the right one for as long as `/var/lib` and the checkout agreed, which is every moment except the one a release adds a migration.** And the failure was silent in the direction that matters: `Pending: 0` is what an operator wants to see. Confirmed green afterwards — alpha-dev step 6 applied 0019 in 43ms and 0020 in 38ms, and an independent `--runtime status` read `Applied: 20`. | — |
| **D507** | `docs/session-09-operator-guide.md` §4 step 5: `./deploy.sh --project … --capabilities … --through-session 9`. | **`deploy: --through-session requires --host.`** `docs/session-08-operator-guide.md` carries `--host host.yaml` on its deploy; the Session 9 guide, assembled from it in Run 8, dropped it — the third flag lost the same way, after `sudo` on six lines (D505). | **`--host host.yaml` restored on both deploy lines**, and the three lost flags are now stated together in §0 as the shape to check when this guide is next copied forward: privilege, manifest, and the flag that selects which rendered set a command reads. | **A guide assembled by retyping loses exactly what is invisible in the command and load-bearing in the script.** All three refusals were loud and none cost anything; the same omission on the flag that selects a rendered SET was D506, and that one was silent. **The pattern is worth more than the three fixes**: what survives retyping is the words, and what does not is the arguments — so the next guide is derived from the previous one by diff, not by rewriting. | — |
| **D508** | ADR 0118 and migration 0018: the agent plane's RPCs are reviewed and unpublished, `REVOKE ALL … FROM PUBLIC` and **granted to `agent_reader` alone**. 0019's own header states the rule it was written to satisfy — *"grep -c agent_writer on 0018 returns ZERO"*. | **`agent_writer` was never granted `EXECUTE` on `api.mcp_agent_context()`, so every write agent was refused before it did anything.** 0019 asked "what does a WRITE need?" and granted all of it — `USAGE ON SCHEMA app_private`, the pre-request hook, both comparison helpers, both audit functions — and never asked "what does an AGENT need?". ADR 0125 resolves the caller's context **once per request in `on_request`, before discovery and before execution**, and `api.mcp_agent_context()` is the statement it sends. Measured on alpha-dev at `f3004fd`, as the role and not as a superuser (ADR 0065/0066), with `agent_reader` as the control: as `agent_writer`, `ERROR: permission denied for function mcp_agent_context`; as `agent_reader`, 0 rows — reached it. **Every symptom follows**: the host gate answered `upstream refused with status 403` on all four write proofs; `app_private.agent_audit` held no row for any of them, because context resolution precedes `begin`; and PostgREST logged nothing, because a 403 is not a 5xx at its level. | **Migration 0021**, granting `agent_writer` `EXECUTE` on `api.mcp_agent_context()` **and on `api.owner_activity_report()`**. The second is the half a repair aimed at the observed failure would have missed: `owner_activity_report` backs the `run_report` capability, whose `required_scopes` are `[notes:read, tasks:read]`, both inside `agent_writer`'s ceiling — so a write agent may hold them, `discoverable_by` offers the tool, and the same missing grant waits one step further on. **No ADR**: 0021 decides nothing. ADR 0118 is unchanged and is now applied to both of its subjects rather than to the one that existed when it was written. | **CLAUDE.md §6 question 5, and this session has now answered it wrong three times** — D501 (0019 built two indexes for a reader it never created), D495 (`bounded()`'s `resource is None` carrying two ideas), and this. **The pattern is a decision that was complete when written and became incomplete when the world gained a case**, which is not a mistake anybody makes at the time and not one any offline test can see: nothing offline resolves a context. **And note which proof caught it.** `agent_credentials`, `agent_scopes`, `agent_surface` — every Session 8 claim — stayed green, because a read agent was never affected. It took a live write, on a cluster, by a role activated one session after the grant list was written. | — |
| **D509** | `test_a_read_survives_the_same_audit_failure`'s control arm, written in Run 8 to establish that the read works *before* the audit grant is withdrawn: `assert control is not None and "error" not in control`. | **The control was satisfied by the failure it exists to exclude.** A FastMCP tool failure comes back as `{"result": {"content": [{"text": …}], "isError": true}}` and carries **no JSON-RPC `error` member at all**, so `"error" not in control` is true of a refusal. On the host the control read was being refused `403` (D508) and the assertion passed anyway; the arm it guards, two lines later, checks `not result.get("result", {}).get("isError")` and caught the same refusal immediately. **The strict check and the weak one sit eight lines apart in the same function.** Found while explaining why the control appeared to succeed on a token that could not resolve a context — which for an hour made D508 look impossible. | **Both control arms now assert `isError` as well as `error`**, in the read test and in `test_a_write_whose_audit_record_cannot_be_opened_does_not_happen`. The write one was never wrong — `note_count(control_title) == 1` catches it a line later — but it is the same assertion and the next person to copy one should copy the strict one. | **A control that cannot fail for the reason it is watching for is not a control**, and CLAUDE.md's §6 list already carries this shape twice: D374, *"a test can check a string its target cannot contain … it passed for an unrelated reason"*, and *"a test comparing two constants is not testing the thing between them"*. **The cost here was not a missed defect — the paired arm caught it — but an hour of diagnosis spent on evidence that said two incompatible things.** A wrong control is worse than no control: no control leaves a gap, and a wrong one furnishes a fact. | — |
| **D510** | `test_a_denied_write_is_recorded_and_never_reaches_the_database`, AGT-AUDIT-001's denied arm, whose docstring says: *"this names a task id that does not exist, so the write is refused by the product itself **after reaching it**"*. | **It sent `p_expected_status: "todo"` and `p_new_status: "done"`, and neither is a label of `api.task_status`** — `('pending', 'in_progress', 'completed', 'cancelled')`, frozen by ADR 0003 and created in 0007. PostgreSQL raised `22P02` before `api.update_task_status`'s body ran; PostgREST returned that code; `write_refusal` maps it to nothing, so it stayed masked and the caller got the framework's opaque string instead of `row_not_found`. **The wrong labels did not merely fail an assertion — they measured the wrong boundary.** A write refused by the *type system before reaching the function* has nothing to say about ADR 0141's denied arm, whose whole content is that a write refused **by the product, after reaching it** leaves a `refused` agent-plane row and no `database` row beside it. The arm proved nothing about the requirement even in principle. | **Valid labels**: `pending` → `completed` against a task id that does not exist, so the refusal is `AP404`/`PT404` from the function's own `IF NOT FOUND`, which is what `UPSTREAM_WRITE_REFUSALS` translates to `row_not_found`. The docstring's claim and the request now describe the same thing. | **The third never-executed proof this trip**, after the `started_at` projection and D509's control. All three are live-host-only, all three were written in Runs 6-8, and all three were invisible to a green suite of 3,927 — because the suite never runs them and nothing tracks that. **CLAUDE.md's oldest open item is exactly this**: *"Nothing knows which proofs have never executed … five sessions, still unbuilt."* Run 9 is the fifth session to pay for it and the first to pay three times in one gate. **A run-age per node id would have caught all three before the trip.** | — |
| **D511** | `test_the_published_route_applies_the_input_bounds_before_the_service_allocates` (Session 6, API-AUTH-002's live half): a body of `MAX_BODY_BYTES * 4` must be answered **413 by the edge**, proving Traefik's buffering middleware is attached rather than the service reading the whole request and answering 400 (D273). | **The same test, against the same deployment, passed in one host run and failed in the next 40 minutes later** — `status=0`, `URLError: <urlopen error [Errno 32] Broken pipe>`. Nothing about the edge changed between them: the only intervening deploys applied migration 0021, which grants two functions to a database role. **The refusal is not in doubt; the client's ability to read it is.** Traefik answers and closes the connection as soon as the body passes the bound, and `urllib` is still writing 64 KiB — so whether the client reads a 413 or takes `EPIPE` is a race between the two, decided by socket buffer sizes and timing rather than by anything the product does. | **Recorded, not repaired, and the test is NOT weakened.** Accepting `EPIPE` as a pass would make the proof green whenever the connection breaks for any reason — including the failure it exists to detect, where the service reads the whole body and answers 400. That is D509's shape, and this session has already paid for one control that could not fail for its own reason. **The honest repair is a client that finishes writing before it reads**, or one that treats a mid-write refusal as a distinct third outcome and asserts against it; both are a change to a passing Session 6 proof and belong to whoever owns API-AUTH-002. | **A proof whose verdict depends on a race reports the deployment's state only some of the time**, and the failing direction is the one that reads as a defect. It is worth more than its cost here — this is the first proof in this repository observed to *flip on the same release* — and it is a fact about the RIG, not the product, which is the distinction CLAUDE.md §6 exists to protect. **Nothing in the evidence document will distinguish this from a genuine regression**, which is the reason it gets a row rather than a re-run and a shrug. | — |
...
`docs/session-09-operator-guide.md` is written. It carried the ordering this
trip was thought to turn on — **migrations before the deploy**, because 0019
grants the privileges and the bootstrap plane grants the membership, and the
wrong order produces a `42501` where the boundary's own `AP401` belongs (D475).
**Run 9 found that ordering is not the operator's to arrange and cannot be:
D506.** The deploy migrates in step 6 and starts the four services that could
serve a request in step 6b, so the window exists and is unreachable. The guide
