...
| **D296** | (`bin/render-jwks.py:write`, its docstring: *"Byte-compared rather than rewritten unconditionally: the file's mtime is the only signal a reader has that a rotation happened, and a deploy that rewrote an identical file on every run would destroy it."* And the message on the `changed` branch: *"the key set CHANGED: every verifier must be RECREATED, not restarted, before it holds this set"*.) A deploy that changes no key prints nothing, and the message means the set changed. | **The message fires on every deploy.** `install_rendered` replaces the rendered directory with a `copytree` of the checkout, and the checkout contains no `jwks.json` -- so the file is **absent** whenever `render-jwks` runs inside a deploy, and the byte comparison has nothing to compare against. Measured with a control that the function *can* return False: absent → `True`; present and identical → **`False`**; removed as `install_rendered` leaves it → `True`. Observed live on alpha-dev on 2026-08-15: the deploy printed `the key set CHANGED` and then listed **exactly the two kids that were already published** -- both read off the running deployment minutes earlier. The file changed; the set did not. | **Recorded, not fixed in Run 13.** The honest fix is to compare the *kids* against the previous deployed document's `verification_kids`, which the document already carries -- so the deploy would say what changed rather than that something did. That is a decision with alternatives (compare against the document, preserve the file across `install_rendered`, or key the message to a rotation phase) and it changes what a deploy tells an operator to do, which is not a thing to change in the run that is about to collect evidence. | **D186's rule, in the one message an operator acts on.** *Do not compile a diagnosis into an error message*: this one reports "the key set CHANGED" having observed only "I wrote the file". It has printed on every deploy since Session 5, so it cannot distinguish a routine redeploy from a rotation -- and the work it asks for is real (`down` then `up`, every verifier recreated). An operator who obeys it every time learns it is noise; one who learns it is noise will ignore the real one. **It also cost this run directly**: the redeploy was recommended without a `down` on the reasoning that no key changed, which was right, and the deploy then said the opposite -- the reason given for the recommendation was wrong even though the recommendation was not, which is D278's shape exactly. | no |
| **D297** | (`bin/session-01-check.sh` step 2, which prints `lock-dev-deps: requirements-dev.txt is current`.) The gate's environment check passed, so the environment is the one the lock describes. | **It checks the LOCK, not the environment.** `bin/lock-dev-deps.sh` verifies `requirements-dev.txt` is current with respect to `requirements-dev.in`; nothing verifies that the installed distributions satisfy it. Run 7 added nine packages to the lock (ADR 0083) and the host's venv predates them, so the first host gate afterwards printed the green lock line and then **died in collection** with four `ModuleNotFoundError`s -- `cryptography`, `argon2`, `fastapi`, `jwt` -- taking `session-01-check` and `session-06-check --mode host` with it. | **Unblocked by re-installing on the host** (`python -m pip install --require-hashes -r requirements-dev.txt`, 19 distributions), and the check itself is **recorded rather than built**: a gate step that compares installed distributions against the lock's pins and fails with the install command is a new authority over the environment, and adding one in the run that is about to collect evidence is how a gate change gets attributed to the evidence. | **A lock verifies what it can dereference, one layer out from D201.** `SCALAR_VERSION` named a release nobody published because a `packages:` entry is a string nothing dereferences; here the lock is correct and *nothing dereferences the environment*. The failure also names neither cause nor remedy -- `No module named 'cryptography'` sends a reader to the test, the release or the host's Python, and the answer is a venv nobody re-synced. Third instance of the carried-in `requirements-dev` item. | no |
| **D298** | (`bin/dev-token.py`: *"the subject is derived from the project, not supplied"*, and ten Session 5 proofs built on it.) A derived subject is safe because no caller chose it. | **A derived subject names nobody, and migration 0013 now checks.** The hook compares the subject against `app_private.users` inside the request transaction, and `auth_claims_are_current` is an EXISTS over five equalities including `credential_version`, `authz_version` and an exact scope array -- none of which a bootstrap token carries. **Ten of the twenty failures** in the first host gate ever run were `AP401: the request identity is no longer current`, and one of them was not a test: `bin/api.sh list-notes` answered **401** against a correct deployment, and had done since 0013 was applied. | **ADR 0095.** No role gets a subject; `development_subject` and `SUBJECT_NAMESPACE` are deleted. The proofs that need an owner take `owner_session` -- a subject created through the product's own `auth_create_user` and a token from `POST /auth/login`. A second registered subject carries the exclusion half of API-REST-001. | **The identity registry arrived and the tool that predates it was never asked whether it still worked.** The command's own docstring had the right argument -- *a caller who can name a subject can read any owner's rows* -- and stopped one step short: 0013 makes naming a subject a claim about the registry, so only the registry's issuer can make it. The proofs are stronger for it: a row-policy assertion about an identity the deployment has never heard of was always the weaker measurement. | [0095](../decisions/0095-a-subject-in-a-token-is-the-identity-registrys-to-assert.md) |
| **D299** | (`tests/deployment/conftest.py:service_container`, whose docstring defends the choice: *"Filtered on the Compose working directory rather than on the project name... ADR 0002 is explicit that a name is read from the deployed document rather than derived a second time."*) | **The identical selector D293 measured as matching nothing.** Three proofs reported `alpha-dev is running no container for the 'auth' service` about containers that were up and healthy. Run 12 found this in `bin/auth-admin.py`, measured it, wrote a first-party `apg.project.key` label into the fix and **left the copy in the test fixture untouched** -- because nothing had run these proofs on a host. | **The fixture selects on `apg.project.key`, read from `bin/auth-admin.py`'s `PROJECT_KEY_LABEL`** rather than restated, and its refusal message now names the filters it used. The old docstring's argument does not apply: `apg.project.key` is not derived in the fixture either -- it is read off the container. | **The mirror image of D294, in the same run.** ADR 0090 asked D276's question of the PROOF and left the PRODUCT reading the document; D293 fixed the PRODUCT and left the PROOF with the broken selector. **When a defect class is fixed, ask which side of the system got the fix** -- Run 12's question (*whose identity, and through which tool*) has a sibling, and neither is answered by the fix's own tests. | [0096](../decisions/0096-a-boundary-assertion-is-re-derived-not-relaxed.md) |
| **D300** | (Three Session 3/5 assertions: only `migration_user`, the access profiles and the authenticator may log in; exactly one function in `app_private` is executable by a request role; the only PEM in a secret generation is the bootstrap key.) | **Session 6 moved all three, deliberately, and each was reported as a violation.** `auth_service` is activated with LOGIN because a container authenticates as it; migration 0013 grants `auth_claims_are_current` to every role that runs the hook, and must, because `postgrest_pre_request` is `SECURITY INVOKER`; the auth service holds a signing key of its own because it **is** an issuer (ADR 0088). | **ADR 0096: re-derived, not relaxed.** LOGIN is keyed to the document publishing an application route -- the event, not the session number (ADR 0046, D280). The function check becomes a two-name allowlist, still exact, so a third function under open default privileges still fails (D57). The PEM scan excludes **one path**, built from `render-jwks.py`'s own constants, and `SEC-KEY-001` already asserts that file's mode and owner positively. | Each of the three had a reason recorded when it was made, and none of the three reasons reached the proof that would contradict it. The tempting fix in every case was a subset check -- `memberships >= activated`, `others ⊆ {...}` -- which turns *exactly what is granted* into *at least what is granted* and would pass the next accidental grant. | [0096](../decisions/0096-a-boundary-assertion-is-re-derived-not-relaxed.md) |
| **D301** | (`tests/deployment/test_session5_api_authorization.py`, whose docstring says *"a session that activates another role must move this assertion with it rather than leave the refusals below measuring nothing"*.) The authenticator is a member of exactly `anon`, `authenticated` and `api_documentation`. | **It is a member of `project_admin` too, since Run 9 (D266)** -- granted in the bootstrap plane, with the same `ADMIN FALSE, INHERIT FALSE, SET TRUE` options as the other three, beside a comment explaining why it is safe. The assertion was a literal set of three, so it could not move with anything. | **`AUTHENTICATOR_REQUEST_ROLES` is promoted to a module constant in `bin/postgres-bootstrap.py` and the proof reads it.** The bootstrap plane owns role membership (D102); the proof now reads the enumeration instead of keeping a copy. | The docstring described the very failure that occurred, in the test that then failed that way. **A test that says "a future session must update me" is a comment, and the update it asks for is one nobody is scheduled to make** -- the derivation is what turns it into a property. | [0096](../decisions/0096-a-boundary-assertion-is-re-derived-not-relaxed.md) |
| **D302** | (`tests/security/test_session2_secrets.py`: every declared consumer's file must be materialized.) | **`required: false` was not accounted for.** D283 made an optional secret's absence legal -- it had been failing whole deployments -- and the proof kept the older rule, reporting `auth_jwt_prepared_key.pem was not materialized` as a defect. That key exists only while a signing-key rotation is in flight, and none is; the operator guide says in as many words that it must **not** be created. | **The proof skips a consumer whose secret is optional and whose file is absent**, with `continue` rather than a relaxed assertion, so `assert checked` still counts only what was read and a **required** secret that is missing still fails on the next line. | D283's fix reached `materialize-secrets` and `bootstrap-providers` and not the proof that describes their output -- the same one-sided repair as D299, found in the same run. | no |
| **D303** | (`services/auth-api/app/errors.py`: `MALFORMED_REQUEST` is *"a request refused before any domain logic ran... one token for nine structural problems"* → 400; `INVALID_REQUEST` is for *"an authenticated administrator, so a reason is safe and useful"* → 422.) | **`_body` raised `InvalidRequest` for both of its failures**, so a duplicate JSON member in an **unauthenticated** login body was answered `422 {"error":"invalid_request","message":"duplicate JSON member: 'username'"}` -- the administrator-facing shape, naming the field, to a caller with no identity. **Two proofs disagreed and neither knew about the other**: the contract test asserted 422 and passed; the live proof asserted 400 and had never run. | **ADR 0097.** A new `MalformedRequest` carries its reason for the log and never for the caller; both of `_body`'s paths raise it; `_guard` maps it to `malformed()`. The three offline assertions move to 400 **and gain the assertion that was missing** -- that the body carries no `message` -- which is the property that was actually violated. The two genuine 422s (an administrator's scopes exceeding the ceiling) are untouched. | **D264's shape, and the mechanism that hid it is D211's.** Two authorities for one status inside one service, each internally consistent, and the disagreement was invisible because one of them lived in a proof nobody had executed. The live proof was right -- which is why it was the one that had never run. | [0097](../decisions/0097-a-structural-refusal-is-400-and-says-nothing.md) |
| **D304** | (`tests/deployment/test_session6_tokens.py` and `test_session6_isolation.py`, both asserting that `GET /auth/jwks.json` equals `jwt.verification_kids`.) | **They are different documents and the sets are not equal.** `keys.py` serves `{"keys": [public_jwk]}` -- the one key the service signs with -- while `verification_kids` is every live issuer's key, which since D276's fix is two. Measured on both projects: one key served, two recorded. The equality held only while there was one issuer, which is exactly when nothing needs checking. | **ADR 0098: containment, not equality.** The verifier's set is asserted equal across three readings including the bytes **inside the container** (D278); the issuer's published set must be a non-empty **subset** of it, which is D276 stated as a property rather than as a memory. The isolation proof asserts A's `kid` is absent from **both** of B's readings -- strictly more than the equality it replaces. | The proofs were written in Run 11 against a service written in Run 7, and the two were never run together. **Under the old assertion ADR 0088's own cutover would have turned both red at the moment they were most needed**: during an overlap the verifier holds two keys and the issuer publishes one. | [0098](../decisions/0098-the-issuers-published-set-is-not-the-verifiers-set.md) |
| **D305** | (`tests/deployment/test_session6_tokens.py`, reading the key set the verifier loaded: `docker exec <container> cat /etc/postgrest/jwks.json`.) | **The locked PostgREST image has no `cat`.** It is distroless -- no shell, no coreutils -- so the exec exits **127** with `executable file not found in $PATH`, which reads like a broken container rather than like a proof asking a distroless image for a program. It had never run: this is the line immediately after the container selector D299 fixed, so the first host run to get past the selector met it. | **`docker cp`, measured with a control.** A rig started the locked python image under Compose with one file-based secret and one plain bind of the same file: `docker inspect` reports both as `type=bind` with the source path, and `docker cp` reads the file out with nothing inside the container. It resolves through the container's own mount namespace, which is the view `cat` would have had. | **A proof can be blocked by the image it is proving things about**, and this one asked a hardened image for a convenience it is hardened against. The container being distroless is a property this repository chose; the proof was written as though it were not. Found only because D299's fix let execution reach the next line -- **one unrun proof hides the next**. | no |
| **D306** | (`tests/deployment/test_session6_tokens.py::test_no_verifier_holds_private_signing_material`, resolving the auth service's key through `project_a["secrets"]["generation_id"]`.) The deployed document names the generation a container is running on. | **It names the generation the DEPLOY wrote, and the live pointer moves without it.** `bin/project-runtime.sh`'s own header says `up: materialize secrets -> compose up`, so every unit restart materializes a NEW generation and recreates every container onto it. The gate's own restart-matrix proofs (`test_session4_convergence.py`, whose comment already says *a unit restart re-materializes secrets*) restart project A **inside the gate run** -- so by the time this proof ran, the auth container mounted `9418d7ae41b53ff2` while the document still recorded `9fefc82f32530001`, and the proof reported that the issuer *does not mount its own signing key and cannot sign anything*. Measured read-only on the host: the container's two bind mounts, its creation time, and the pointer, all naming the newer generation. | **An `active_generation` fixture reads the pointer**, the same file `bin/render-secret-override.py` reads, and the proof uses it. Which generation to read is decided by the question: *what a running container holds* is the pointer, *what a deployment recorded* is the document -- and `SEC-BOOT-001` and the materialization proof keep the document, because those are the second question. | **D76, met by a proof written four runs after it was recorded.** `bin/dev-token.py:signing_key_path` quotes D76 in its docstring -- *the document's generation is deploy-time history and the live pointer moves at the first restart* -- and the proof that needed it most was written without it. It also had an ordering dependency nobody could see: had it run before the restart proofs it would have passed, so the same suite could report either answer depending on collection order. **A green run would have been the luckier one.** | no |
...
### Run 14 — twenty failures, and what a first run is worth  ·  **Done offline.**

Run 13 deployed both projects and ran the Session 6 gate on the host for the
first time. **20 failed, 161 passed** — and **nineteen of the twenty were proofs
that had not executed since the thing they measure changed.** Not one of them
was hard once run. That is the whole finding, and it is D211–D214's, at a scale
that makes it hard to keep treating as an anecdote.

**Before any of them: the gate did not get that far the first time.** Four
modules failed to import — `cryptography`, `argon2`, `fastapi`, `jwt` — because
the host's venv predates the nine packages Run 7 added to the lock, and
`lock-dev-deps` verifies the **lock**, not the environment (**D297**). The gate
printed a green environment line and then died in collection.

**The one live defect, and it is not a test problem (D298, ADR 0095).** Ten of
the twenty were `AP401: the request identity is no longer current`. Migration
0013's hook compares a token's subject against `app_private.users`, and
`bin/dev-token.py` derives a subject that is in no row and carries none of the
three other claims `auth_claims_are_current` compares. **`bin/api.sh list-notes`
answered 401 against a correct deployment** and had done since 0013 was applied.
No role gets a subject now; the proofs that need an owner authenticate as one.

**The pair worth carrying, and they arrived in consecutive runs.**

- **D294** — ADR 0090 asked D276's question of the **proof** and left the
  **product** reading the document. The command that signs went on reading
  `active_kid` for one more run, and that is the run in which the second key
  arrived.
- **D299** — D293 measured a container selector matching nothing, fixed the
  **product**, and left the identical selector in the **proof**.

**When a defect class is fixed, ask which side of the system got the fix.** Run
12's question — *whose identity, and through which tool, does the proof run* —
has a sibling, and neither is answered by the tests that ship with the fix.

**Four more were boundaries Session 6 moved on purpose** (D300, D301): the
authenticator's `project_admin` membership, `auth_service`'s LOGIN, the hook's
comparison helper, the auth service's own signing key. Every one had its reason
recorded when it was made and none of those reasons reached the proof that would
contradict it. Re-derived rather than relaxed (ADR 0096) — the tempting fix in
each case was a subset check, which turns *exactly what is granted* into *at
least what is granted*.

**Two were genuine disagreements, and the product lost one of them.**

- **D303 / ADR 0097** — a duplicate JSON member in an **unauthenticated** login
  body was answered `422 invalid_request` **naming the duplicated field**, which
  is the shape `errors.py` reserves for an authenticated administrator. Two
  proofs disagreed about the status and neither knew about the other (D264's
  shape); the one that was right was the one that had never run. Structural
  refusals are now 400 with nothing in them.
- **D304 / ADR 0098** — `GET /auth/jwks.json` serves the key the service signs
  with; `jwt.verification_kids` is every live issuer's key. Two proofs asserted
  they were equal, which was true only while there was one issuer. Containment
  now, which also survives a cutover — under the old assertion ADR 0088's own
  machinery would have turned both red at the moment they mattered most.

**D302** is D283's fix reaching the materializer and not the proof that
describes its output. Same one-sided repair as D299, same run.

**What this run cannot claim.** Nineteen of these proofs run only on a host, so
everything above is *written* and only the offline suite (3124 passed) and the
