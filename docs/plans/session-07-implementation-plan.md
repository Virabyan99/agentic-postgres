...
| **D394** | (recorded because a security control fired once and would not reproduce.) `provision-host`'s baseline resolves sshd's policy by asking **sshd**, not by reading the file — `sshd -T`, greped for `^passwordauthentication no$`. | **One run reported `DEVIATE sshd did not resolve passwordauthentication no; an earlier include may be winning`, and nothing else in the baseline deviated.** Measured immediately after, and it does not reproduce: `sshd -T` printed `passwordauthentication no` on three consecutive runs; the check's own anchored pattern matches (`grep -c` → 1); `openssh-server` was last touched **2026-08-05** and sshd last started **2026-08-08**, both well before the trip; neither drop-in file's mtime had moved. The next gate run resolved all five pairs `ok`. | Recorded, not fixed. The check is **correctly designed** — it asks the daemon rather than the file, which is the distinction its own comment exists for, and the other four pairs resolved in the same invocation, so `sshd -T` produced full output and only that key disagreed. Nothing measured explains it. | **Written down precisely because it cannot be explained.** A control that reports a deviation once and clears on retest is either a real transient or a flaky control, and the difference matters: the first is an exposure window, the second erodes the control's credibility every time it cries wolf. The evidence available says the host was correctly configured throughout. **What is missing is any record of what `sshd -T` actually printed** — the check keeps the pass/fail and discards the output, so a one-off leaves nothing to diagnose. That is the fix a later session should make. | — |
