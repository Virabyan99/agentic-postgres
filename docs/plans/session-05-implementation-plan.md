...
| **D199** | *(`compose.yaml`, Run 1)* "The three dangerous defaults, all set explicitly… the hoisted transaction settings default to three rather than none." `PGRST_DB_HOISTED_TX_SETTINGS: ""`, and `tests/contract/test_postgrest_service.py` asserts the model sets it to the empty string. | **An empty value is read as unset, so the setting was never disabled.** Measured on the locked PostgREST image with `--dump-config`, three ways: **empty** → `statement_timeout,plan_filter.statement_cost_limit,default_transaction_isolation`; **unset** → the same string; **`statement_timeout`** → `statement_timeout`. So the value the product ships is indistinguishable from not shipping one, and the test asserting it is set asserts only that the *variable* is present. It is inert either way — separately measured, with `db-config` false a request role's `statement_timeout` stays `0` whether the hoist list names it or not — which is why nothing downstream of it ever went wrong. | **Recorded, and the comment corrected rather than the value changed.** There is no measured value that disables hoisting, and finding one would buy nothing while `db-config` is `false`; ADR 0068 is what makes intent sufficient, because the pre-request hook carries the timeout and nothing needs `db-config` turned on. The line stays as a declaration of intent, with the measurement beside it. | The setting was reviewed, chosen, written, asserted by a test, and had no effect — and the assertion that would have caught it is the one nobody writes, because it reads as tautological: **does the value we set come back?** `test_the_dangerous_defaults_are_what_they_were_measured_to_be` measures the *default*, correctly, and never compares it against what the product ships. This is D188 and D192's family again — a constant measured against something other than the running product — and the cheapest general defence is the one ADR 0066 already built for a different reason: read the configuration back out of the thing you configured. | no |
| **D200** | *(this run's own mutation battery, and the two tests it was aimed at)* `test_the_effective_hook_is_the_last_migration_that_defines_it` — "goes red the day a 0010 replaces the hook again" — and `test_every_down_block_refuses`, which has asserted the fix-forward rule since ADR 0028. | **Both were satisfied by a string, and a mutation walked through each.** (1) `HOOK_DEFINITION` was `"CREATE OR REPLACE FUNCTION app_private.postgrest_pre_request"` — a **prefix**, so renaming the function to `postgrest_pre_request_v2` still matched, and a migration that no longer defines the hook still counted as defining it. The control that exists to notice a replaced hook could not notice a *removed* one. (2) The down-block check asserted `"AP900" in down`; a block reading `RAISE NOTICE 'AP900: …'` satisfies it and **succeeds**, so dbmate would stamp a rollback that rolled nothing back. Both stayed green under mutations expected to go red. | `HOOK_DEFINITION` gains the `()`, which makes it an exact declaration rather than a prefix; the down check additionally requires `RAISE EXCEPTION`, because the code is not the refusal — the level is. Both re-run red, and all nine of the battery's mutations are now detected with the control green in the same invocation. | Two more of D162's shape — a prefix standing in for an exact match, which is how `PathPrefix(/api/rest)` matched `/api/restaurant` — and one of the family where **a string is checked where a behaviour is meant**. What is worth carrying is where they were found: not by review, and not by the suite, which was green both times. They were found by aiming a mutation at a test and being surprised. **The tests that most need a battery are the ones written as controls for other tests**, because nothing else is watching them. | no |
| **D201** | *(`versions.in.yaml`, since Session 1, and D128)* `SCALAR_VERSION: "1.36.4"` under `packages:`, and D128's option (b): "build `services/docs/` from the locked Node runtime **at the already-pinned 1.36.4** with a committed lock file — the repository's own precedent, and it makes the version this session ships the version Session 1 locked." | **There is no 1.36.4.** `npm view @scalar/api-reference@1.36.4` is a 404, and the published history goes **1.36.2 → 1.37.0**; the version has never existed. No Scalar *image* carries the tag either — `scalarapi/api-reference`, `scalarapi/scalar` and `ghcr.io/scalar/scalar` were each checked for `1.36.4`, `v1.36.4`, `1.36.2`, `1.64.1` and `latest`, and only `scalarapi/api-reference:latest` resolves at all. The string has sat in the version lock for four sessions and **nothing ever exercised it**, because nothing was ever built from it: the lock records package versions as strings and only `images:` entries are resolved to digests. | **The lock must be re-pinned to a version that exists**, which makes D37's rule — a session adds nothing to the lock inventory and uses what is already pinned — inapplicable rather than merely inconvenient: what is pinned cannot be used. The choice of replacement belongs to D128's ADR alongside the shape question, because the two are one decision. Recorded here so the *reason* the lock moves is a measurement rather than a preference. | The signature defect, in the one file whose entire purpose is that its values are real, and it survived four sessions of a gate that verifies the lock. What it survived **by** is the shape of the check: `bin/verify-versions.sh` proves every `images:` entry resolves to its recorded digest, and a `packages:` entry is a string nothing resolves. **A lock verifies what it can dereference**; everything else in it is a comment with a colon in it. The general repair — resolve package versions against their registry the way images are resolved against theirs — is Run 10's to weigh, because it needs network access in a check that deliberately has none. | no |
| **D202** | *(D128)* "The measurement that decides it: does 1.36.4 produce a **self-contained** bundle with no runtime fetch, given a static OpenAPI document?" and D142's proof: "a byte scan of files this deployment wrote — stronger than a browser assertion, because it holds for every visitor rather than for the one that was driven." | **Neither version is self-contained, and the byte scan cannot see the difference.** Measured on both real candidates from the locked Node runtime: `dist/browser/standalone.js` names `fonts.scalar.com` **14 times** at 1.36.2 and 14 times at 1.64.1, alongside `proxy.scalar.com`, and `withDefaultFonts` is declared `default: true` in both. So the bundle fetches a font from a third party unless configuration says otherwise — at every version, which means **self-containment was never a discriminator between an upstream image and a first-party build**; it is a configuration both must set. Worse for D142's stated proof: the hostnames stay in the bytes whether the switch is on or off, so a scan for external hosts in the served files **fails on a correctly-configured page** and would pass only if it scanned for something else. | **Deferred to D128's ADR, with the real discriminator now visible.** A `withDefaultFonts: false` is a promise about a third party's code honouring its own flag; a **Content-Security-Policy served with the page** is a rule the visitor's browser enforces against whatever the bundle attempts. The byte scan keeps the job it can do — **no credential in any served byte**, which is `SEC-DOCS-001` — and gives up the job it cannot. | D128 asked exactly the right question and the answer does not decide what it was expected to decide, which is the more useful outcome and the reason the row said *measure*. Two lessons worth the row. **A string in a bundle is not a fetch**, and a scan that cannot tell them apart will eventually be relaxed by whoever is trying to ship — D191's shape, where the obvious repair to a false positive is deleting the check. And a property is best enforced where it is *refused*, not where it is *promised*: the CSP is the only place in this design where "loads nothing from the internet" stops depending on somebody else's default. | pending |
| **D203** | *(`tests/contract/test_render_isolation.py`, since Session 1)* "Session 1 proves *generated identity* isolation… every one of these must differ across the two projects", over a 21-entry `MUST_DIFFER` table that includes `routes.docs`, `compose.networks.edge` and `database.container`. `DEP-ISO-005` rests on the same idea one layer up. | **The table compares `outputs.json`, and every Traefik router and middleware name lives in `compose.env`, which nothing compared at all.** Found by a mutation expected to go red: pointing `docs_router_name` at a constant gave both projects one router and the whole suite stayed green. The gap is far older than the documentation route -- **no route's names had ever been asserted project-scoped**, including the REST route's, because the file that carries them was outside every isolation comparison. | A second fixture parses both projects' `compose.env` and asserts that no key ending `_ROUTER_NAME` or `_MIDDLEWARE_NAME` holds the same value in both. Derived from the environment rather than from a list, so a route added in a later session is covered the day it renders rather than the day somebody extends a constant -- with a control asserting the three known routers are present, because a derivation that stopped writing names would leave the comparison with nothing to disagree about and an empty set of shared names reads exactly like a clean one. | **A Traefik name is host-wide.** Two projects sharing a middleware name do not get one each; they get whichever definition the proxy loaded last, applied to both -- which for the documentation route means one project's Basic Auth guarding the other's page, or neither. What makes this worth a row is where the boundary was drawn: the isolation tests were written against the document that *describes* a deployment, and the names that actually collide are in the file that *configures* one. **An isolation proof is only as wide as the set of documents it reads**, and this one had been reading one of two for four sessions. | no |
| **D204** | *(`src/agentic_postgres/edge_credentials.py`, Run 7, and ADR 0069's own comment in `runtime_override.py`)* "The credential middleware is referenced `@file` because it is defined by Traefik's *file* provider, not by these labels (`edge_credentials.py`)." The module is complete: it validates a bcrypt hash, refuses every other format, renders the htpasswd line and the middleware document, and names the file with an extension the file provider ignores and the middleware reads. `tests/contract/test_edge_behaviour.py` measures all of it against the locked Traefik. | **Nothing has ever called it.** The only reference to the module anywhere outside its own tests was that comment — written the day before, in the voice of evidence. So the middleware every documentation router names did not exist; Traefik does not create a router whose middleware is undefined; and the first live deploy of the documentation route answered **404 eleven times** while `observe_docs` correctly refused to record it ready. From outside, that 404 is indistinguishable from an unrouted host (D186). | **`publish_docs_credential` in the deploy**, after the secret generation is re-read and before the edge is attached. The hash is produced **inside the locked runtime image** — `crypt` left the standard library in 3.13 and the host's interpreter is past that — with the password on **stdin**, never in `argv`; measured on the locked digest first: `crypt.methods` offers `BLOWFISH`, the result is a 60-character `$2b$12$` hash that `assert_bcrypt` accepts and that verifies against its own password, with a `$6$` control refused. And the general rule: **`test_no_module_is_imported_only_by_its_own_tests`**, computed from the import graph. It found two more candidates on its first run and both were false — the first draft matched text with `[^\w.]` before the name, which excludes `from agentic_postgres.x import y`, the dominant form; a third was Python embedded in a shell heredoc. **A text scan could not have worked at all**, because the one mention of this module in the product was a comment, and a scan that reads comments would have called it imported. | The purest instance of this project's signature defect yet, and the cheapest to have caught. D192 needed a live request to find, D197 needed a `rolconfig` query, D203 needed a mutation — **this needed only the question "who imports this?"**, and four sessions passed without anyone asking it. The tests were thorough and correct and measured the module against a real proxy; what none of them could see is that the product does not contain it. **A test proves a module works. Only a caller proves it runs.** | no |
| **D205** | *(D204's own fix, and the three tests written beside it)* The credential publisher hashes inside the locked runtime image, with the password on stdin and never in `argv`. Asserted structurally with `ast`: the `subprocess.run` passes `input=`, the argument vector carries `-i` so stdin is attached, and no element of it is built from the credential. All three passed. | **The program handed to `python -c` did not parse.** It was assembled from adjacent string literals, one of which read `.rstrip("\n")` — and `\n` written into a `-c` argument *from Python source* is a newline **character**, not a two-character escape. So the program arrived split across two lines and every deploy died with `SyntaxError: unterminated string literal`, on the host, in step 5, after two images had been built. Both projects, identically. | The strip is removed rather than escaped — the caller already sends exactly the password, so the container needs none and there is no escape left to get wrong. And the rule: **`test_every_embedded_python_program_compiles`**, which lifts every `python -c` program out of `bin/*.py` through the syntax tree and `compile()`s it. Through the tree rather than by matching text, because the broken program was built from several adjacent literals and only the string Python actually concatenates is the program. Its first draft was too loose and compiled `psql -c "SELECT …"` as Python, so the interpreter must be named immediately before the flag. Verified end to end afterwards against the locked image, with the program *parsed out of the shipped source* rather than retyped: exit 0, a 60-character `$2b$12$` hash, accepted by `htpasswd_line`, verifying against its own password and refusing a different one. | `tests/contract/test_embedded_python.py` has compiled Python embedded in **shell** scripts since Session 2. Python embedded in **Python** had no such rule, and the first one shipped broken. What makes this worth a row is what the tests did assert: three of them, all correct, all about the *shape* of the call — is the secret on stdin, is `-i` present, is the credential absent from `argv`. Every answer was yes. **A shape is not a program.** The general form is D204's lesson one turn further in: a test proves a module works, a caller proves it runs, and only executing the thing proves it parses. | no |
| **D206** | *(D132, and this run's own first commit)* "`bin/session-04-check.sh`'s shape, `readonly SESSION=5`, three modes, and the two refusals kept verbatim." The gate was derived by copying Session 4's and renaming, which is what that row asks for; `shellcheck` passed, 26 contract tests passed, and `--help` printed the Session 5 merge command. | **Seven evidence paths still said `session-04`, and the tests could not see them.** They are built as `"${EVIDENCE_DIR}/session-04-${mode}-tests.xml"` — a literal prefix concatenated onto a variable — which the rename that fixed every other occurrence did not match. The gate would have written the previous session's filenames while its own `--help` named files it never writes, and the merge would have failed on a path that does not exist, after both a host run and an off-host scan. Found by reading the file for the *other* carried item, not by any test. **And nothing gave the evidence back**: host mode runs under `sudo`, so every JUnit XML and every evidence half lands `root:root` and the operator who must commit them cannot read them — D194's remainder, named in that row as belonging to this run. | The prefix is **derived once** from `SESSION` via `printf 'session-%02d'`, so the sixth gate cannot inherit the fifth's: a number that appears once cannot be left behind. The handback is a `restore_evidence_ownership` registered as an **`EXIT` trap** in both modes rather than a line at the end of the happy path — the evidence is written whether or not the suite passed, and a failing gate is the run whose output an operator most needs. A no-op without `SUDO_UID`, and its failures ignored, because it runs after the verdict and must not turn a passing gate into a failing one. | The interesting half is *why the rename missed*. Search-and-replace matched every occurrence of the string it was given, and the defect was in the occurrences that are **not that string** — a prefix and a variable, assembled at runtime into the name that matters. **A value assembled from parts is invisible to every tool that reads the parts.** That is D205's shape (a program built by concatenation is a program nobody has parsed) in a different medium, two commits apart, and the answer is the same both times: stop assembling, or execute the assembly. Here it is the former — one definition, derived. | no |
| **D207** | *(this run's own restart matrix)* Five proofs written against Session 4's convergence module, which they import rather than copy, with the environment they consume declared and the container listing taken at the point of use. They collected, they linted, and the offline suite ran them as skips. | **Four failed on their first host run, from three different causes.** (1) `KeyError: 'compose'` — `service_container` built a name prefix from `document["compose"]["project_name"]`, which is a **rendered** member; every fixture in that suite is a *deployed* document. (2) `503 57P01 terminating connection due to administrator command` — the cluster restart was asserted the instant `docker restart` returned, and PostgREST was still reconnecting the pool the restart had killed. Session 4's `assert_converged` polls to a deadline; this did not. (3) `https://alpha-dev/__apg/healthz did not identify https://alpha-db…/__apg/healthz` — `await_health(hostname, project_key)` called with `(project_key, url)`: wrong order **and** wrong value, failing on name resolution, which reads as a DNS problem rather than a swapped argument. | **(1)** Containers are found by **label** rather than by name — `apg.project.key`, which this repository's Compose model sets, and `com.docker.compose.service`, which Compose sets. Better than a corrected prefix would have been: neither is a name the test derives. **(2)** The first assertion polls to a bounded deadline and reports what it last saw; everything after it stays immediate, because a route that is wrong while nothing is happening to it is a failure and a retry there would turn a broken deployment into a slow green. **(3)** The domain, in the right position. | The value is in the count. A single fix aimed at the symptom the first failure named would have left the other two — D195's shape, which this table already records twice. What made three separate causes visible in one pass was asking for **every** `^E ` line rather than the traceback pytest printed last; the last traceback was the *least* interesting of the three. And the two that were not the readiness race are the two a plausible diagnosis would have missed: I predicted the wait, and the wait was one of three. **A confident diagnosis of a multi-cause failure is a fix for one of them.** | no |
| **D208** | *(`src/agentic_postgres/edge_credentials.py`'s module docstring, Run 7)* "…this module takes the file: the middleware then exists independently of any container's lifecycle, so a docs service that is being recreated cannot leave a router referencing a middleware that has momentarily stopped existing — which Traefik rejects route-by-route while the hostname keeps answering 404." | **That reasoning was applied to one middleware of four, and the other three are container labels.** `apg-<key>-docs-strip`, `apg-<key>-api-stripprefix` and `apg-<key>-api-buffering` are all defined by `traefik.http.middlewares.*` labels **on the container the router points at**, so while that container restarts its own router references a middleware that does not exist and Traefik drops the router. Measured: restarting the documentation container makes `/docs/rest` answer **404** — Traefik's own, indistinguishable from an unrouted host (D186) — and the route **recovers by itself**, measured at 401 afterwards. The access log is what settled it: the same run shows `RouterName: apg-alpha-dev-docs@docker` answering 401 ten seconds earlier, so the router existed, went, and came back. | **The harness fault is fixed and the design question is deliberately left open.** `assert_api_converged` polled the REST read and then checked the documentation route *once*; it now polls both, because restarting `postgrest` leaves the docs route alone — so the REST poll spent the window and the docs check that followed was never inside one. **The test that passed did so for a reason that was not the property under test.** Whether the three remaining middlewares move to the file provider is a decision with a real trade-off and belongs to whoever takes it: the route would answer **502** instead of 404 while its backend is down, which is unambiguous where a 404 is not, at the cost of three more generated files per project. | Two lessons and the second is the sharper one. **A route's availability is coupled to its backend's lifecycle through its middlewares**, which is not obvious from either the labels or the model, and it is why the credential middleware was moved in the first place — the reasoning was written down and then applied once. And the diagnosis nearly went the other way twice: an empty Traefik API response was almost read as "no router" when the API is simply not enabled, and three tests failing in sequence was read as "permanent" when all three ran inside one ten-second window. **A negative from an instrument nobody verified, and a pattern from samples nobody spaced, are both the same mistake.** | pending |
| **D209** | *(ADR 0071's sudoers rule, written the same hour)* Four `Defaults` scoped to `apg-agent`: `!requiretty` so a non-interactive SSH invocation works, `env_reset`, and `log_output` with a `logfile` so every command the agent runs is recorded and attributable — which the ADR cites as one of the reasons a *separate* account is worth having. | **Three of the four are not settings on this host, and the file was invalid.** `visudo -c`: `unknown setting: 'requiretty'`, `'log_output'`, `'logfile'` — `env_reset`, on the line between two of them, was accepted. The refusal text (`sudo: I'm sorry apg-agent. I'm afraid I can't do that`) identifies **sudo-rs**, the Rust implementation Ubuntu now ships as the default `sudo`, which does not implement them. **And the file worked anyway**: sudo-rs parsed the grant and enforced it, `docker ps` and `deploy.sh` were both refused, the allowlist answered — while `visudo -c` reported the whole file invalid and every invocation printed eight lines of parse errors. | The three are removed, leaving two effective lines: `env_reset` and the grant. A parametrized test refuses each of them by name, plus two more sudo(8) settings sudo-rs lacks, and a second test asserts the file is exactly those two lines — a sudoers file has no test harness on the host, so the less effective content it carries, the more of it a human can check by reading. What is genuinely lost is `log_output`: sudo-rs records each *invocation* through syslog, so what was run stays attributable, but the output is not captured. For a surface that mutates nothing and prints no secret, the invocation record is the half that matters. | **ADR 0019's rule, in a place nobody expected it to apply.** That ADR exists because a Traefik key was written from documentation and existed in no released version; this is the same mistake against `sudo`, where the documentation is right and the *program* is a different one. Ubuntu replaced sudo and the manual page did not move. The sharper half is the failure mode: **the file was wrong and worked.** Both refusals fired, so every functional check passed, and the only thing that said otherwise was a validator nobody would have run if the account had worked on the first try. An artefact that behaves correctly while failing its own validator is one an upgrade removes silently, in the direction that drops a boundary. | no |
| **D210** | *(`bin/apg-diag.sh`, written the same hour)* Containers are found by the labels the product sets: `apg.project.key` for the project and `com.docker.compose.service` for the service. "Not by name: a name is a string this script would have to build, and the labels are facts the product already asserts." | **`apg.project.key` is on three of thirteen services**, and they are exactly the three that carry Traefik labels -- `edge-probe`, `postgrest`, `docs`. It means *this container is routed*, not *this container belongs to this project*. Both consequences showed on the surface's first real use: `catalog` answered `no postgres container for alpha-dev` for a cluster that was running, and `containers` listed three of a project's six, so a healthy deployment looked half-absent. | Resolution is anchored on `database.container`, which the deployed document **publishes**, and the Compose project is read off that container's own `com.docker.compose.project` label -- which Compose sets on everything it creates. One published fact in, exact label matches thereafter, and no name this script builds (ADR 0002). `containers` now iterates the deployed projects and says so when a project's cluster is not running, which is more useful than omitting it. | **I read the label's name and inferred its meaning.** Nothing measured where it was applied, and the name is a good name for the thing it is not. This is the session's standing defect in its cheapest form yet -- cheaper than D204's uncalled module and D205's unparsed program, because the answer was one `grep` of the Compose model. What makes it worth a row is *when* it was caught: on the first invocation, by a verb doing its job, rather than in a review of the file that introduced it. **A diagnostic surface's first use is a test of the diagnostic.** | no |
| **D211** | *(five host runs, all green on the deployment sweep)* `pytest tests/deployment -m live_host` is how this session has measured every host run since Run 9, and it reported 105, then 110, passing. | **Two things that suite cannot see, both found the moment the gate ran.** (1) `tests/security/` is a *directory* the sweep never selects, so `test_only_the_activated_roles_may_log_in` had not executed once against a Session 5 deployment. It fails correctly: `postgrest_authenticator` holds `LOGIN` and the document records it nowhere, because `access_profiles` describes the transports a developer reaches the cluster through and a service's own identity is not one of those. **A role that can authenticate to the cluster was invisible in the document describing the deployment.** (2) The documentation restart proof failed with its *success* message on stdout and a 404 on stderr, in one test: `docker restart` returns once the container has started, and Traefik has not necessarily processed the **stop** by then -- so the first check saw the pre-restart router, the poll broke on a stale success, and the re-assertion afterwards landed in the window the restart had by then opened. | **(1) ADR 0072.** The expected login set gains a third clause derived from the published REST route: a deployment serving the surface has a service logging in as that role. Naming the role as an unconditional exception was the alternative and is the weakening this repository forbids -- it would pass on a Session 4 deployment that had somehow activated it, which is the case ADR 0046 wrote the test for. Publishing `activated_logins` as an *observation* was also considered and rejected: the deploy would populate it by querying the catalog, so the test would compare the catalog against a copy of itself (D173's tautology). **(2)** The poll requires **two consecutive** rounds, which is `postgres-bootstrap.py::await_cluster`'s rule against a different daemon: "a single success is exactly what the temporary server produces". | Both are the same mistake about *when* a thing is true. A poll that starts before the disturbance propagates measures the state it was trying to leave; a suite selected by marker measures the tests that carry it, and `-m live_host` is not "the host tests". **The gate is the only thing that runs `tests/security/`, and it had never run.** Five green sweeps were five reports about a subset nobody had stated the boundary of -- which is the same defect as a scan whose instrument was never verified, one level up. | **yes** |
| **D212** | *(Run 10, closing the D178 hole)* `test_compose_contract.py`'s module-level skip is narrowed to a per-test guard, `pytest.mark.skipif(not ALPHA.is_dir())`, so the assertions that would have caught a one-character interpolation error finally run. | **The guard asks whether the fixtures exist, never whether they match the model they test.** The Session 5 host gate resolved this module's Compose model against a fixture rendered 2026-08-10 at `schema_version: 4` -- before PostgREST existed -- and got eleven `missing a value` interpolation errors naming `POSTGREST_POOL_SIZE`, `ANON_ROLE_NAME`, `JWT_AUDIENCE` and eight more. Read as written, that is a defect in `compose.yaml`. **The model was fine and the fixture was four schema versions old.** The same existence check guards `test_project_state_roots.py` and `test_migrations.py`. | **ADR 0073.** A rendered fixture has three states and only one is a skip: *absent* skips, because a clean checkout has done nothing wrong; *stale* **fails**, naming the version gap; *current* runs. One authority, `tests/contract/rendered_fixtures.py`, reading `output_migrations.CURRENT_VERSION` rather than a number typed beside it -- a currency check with its own copy of the version is a tautology that agrees with itself while both drift. `schema_version` is recorded as the **proxy** it is: it does not catch a fixture at the current version whose `compose.env` lacks a key, because a Compose variable can be added without an outputs migration. | This is D178 one turn later, and the shape is worth more than the fix. Run 10 replaced a skip that was **too wide** with a guard that was **not about the right property** -- silently absent became silently wrong, which is worse, because a skip at least reports itself. **An existence check answers a question nobody asked.** It is also the fifth Session 5 finding where a filesystem fact stood in for a logic test. | **yes** |
| **D213** | *(`tests/security/test_session2_secrets.py`, since Session 2)* The secret proofs ask the contract what is active and who is granted: `active_secrets(contract, session=2)` and `granted_services(contract, session=2)`. | **A literal where the deployed document publishes the number.** The gate failed on `apg-alpha-dev-postgrest-1 mounts a secret it was not granted: .../postgrest/postgrest_authenticator_pgpass` -- a grant the contract declares, `introduced_in_session: 5`, which a constant stopping at 2 cannot reach. The second literal was worse and silent: session 2 declares **one** secret with **one** consumer, the sentinel, against 8 secrets and 13 consumers at session 5 -- so this proof checked 1 of 13 and `assert checked` reported success on that one. Twelve consumers' modes, owners and non-emptiness, across three sessions and including both root-plane ones from ADR 0054, were measured by nothing. | **ADR 0074.** Both read `deployed_through_session` from the deployed document, through one module fixture so a third proof cannot introduce a third constant. Stricter in both directions: a container mounting a secret from a session *later* than the deployment still fails, as does one mounting another service's copy, because the consumer's service name is a path component. `CURRENT_SESSION` was the tempting alternative and is the repository's answer to a question about the *deployment* -- a checkout at session 6 measuring a host at session 5 would expect grants never made. | **The constant is not the finding.** These thirteen proofs are gated on `--sentinel-file`, and the flag was not passed once in Session 5 -- so they reported as skips in every run and a skip is not a pass. Their first execution against a Session 5 deployment was 2026-08-13. That is the **third** Session 5 instance of a proof that did not execute, after Run 8's sixteen deselected node IDs and D211's unselected directory, and the one that hid a wrong constant for three sessions. The guide now writes the flag into the command instead of mentioning it below. | **yes** |
| **D214** | *(`tests/external/test_session5_public_api.py`, Run 9a)* The documentation route's refusal is proved from off-host: 401, and a `WWW-Authenticate: Basic` challenge, because a 401 with nothing to prompt for is not a login. | **The header was present, correct, and invisible.** `fetch` returned `dict(response.headers.items())`, and a plain dict is case-sensitive where HTTP field names are not (RFC 9110 s5.1). Go canonicalises header keys, so Traefik's challenge is on the wire as `Www-Authenticate` while the proof asked for the RFC's own `WWW-`. The assertion printed `None` **beside the mapping containing `'Www-Authenticate': 'Basic realm="alpha-dev documentation"'`** -- the answer and the failure in the same line. | `fetch` returns a `Headers` mapping that folds case on lookup and stays a `dict`, so the repr keeps the wire spelling that made this diagnosable in one read. **No ADR: the assertion is unchanged.** What changed is a lookup that was wrong about the protocol, not a property this suite proves. `tests/contract/test_external_helpers.py` proves it offline -- three cases, including that an absent header stays absent, which is how a fix like this goes wrong. | Not the product: the deployment sends the right challenge with the right realm. It is the **fourth** Run 10 finding of the same kind, and the tightest: this proof was written in Run 9a and external mode had not run since, so its first execution was its first chance to be wrong. The pattern for the run's summary is one sentence -- **everything Run 10 found, it found by executing a proof that had never executed.** | no |
