...
**Next free number after this table is D686.**
...
| **D676** | This plan's Run 10: *"three rotations; each proof given the value the window replaced."* The four rotation proofs were treated as ready to admit. | **`test_a_rotated_signing_key_is_the_only_one_the_plane_accepts` cannot run at all.** It does `dev_token(project_a, role)`; the fixture returns `bin/dev-token.py` **loaded as a module**, and every other caller in the suite reaches through it as `dev_token.mint(...)`. Measured with a rig whose control confirmed it held the real module: `TypeError: 'module' object is not callable`. It has been green in the suite since Session 5 because `APG_ROTATED_JWT_FROM_FILE` is set only inside a rotation window, and **no window has ever been run**. | **`mint_token(project_a, role, subject=None)`** — the fixture that exists for this, already used eight lines earlier in the same file, and the `subject=None` form ADR 0095 blessed. Guarded as a class by `tests/contract/test_deployment_suite_shape.py`, which scans every deployment proof for a call to a fixture that returns a module and derives the roster from the conftest rather than listing it. | **Found offline, before the trip, for the price of reading four proofs.** At the terminal it would have ERRORed *after* the signing key was rotated — the irreversible half — and CLAUDE.md's five-defective-proofs tally would have become six. This is the sixth instance of *nothing knows which proofs have never executed*, and the first one caught **before** it cost anything. | — |
| **D677** | `docs/api-operations.md`: the documentation credential is Basic Auth in front of the page, fetched with `docs:<password>`. | **The username is one constant with two spellings.** `edge_credentials.DOCS_USER = "docs"` decides it; `test_a_rotated_documentation_credential_opens_the_page_and_the_old_one_does_not` retyped the literal into an f-string. They agree today. Nothing makes them keep agreeing, and this is the shape that had cost **D674** the day before, where a status vocabulary typed from memory contained a value nothing emits. | **Imported.** One line, and the proof now fails if the product's username moves. | Caught in the same read as D676 and worth recording separately because it is not yet a defect — it is the *precondition* for one. The cheapest moment to remove a second spelling is while already editing the line. | — |
| **D678** | `docs/api-operations.md`'s rotation procedure, steps 3 and 4: *"`--through-session 5`"* for the runtime `down`, *"`--session 5`"* for the materialization, and *"Redeploy through session 5."* | **All three numbers are Session-5-era and are wrong for a Session-11 deployment**, in three different ways. `project-runtime.sh`'s own header says the flag is *"the session this project **was deployed through**"* — passing 5 brings down the Session-5 services and leaves `mcp`, `storage` and the backup plane running on the superseded generation, which is **D253's exact failure one plane over**. `materialize-secrets.sh --session N` materializes *"secrets introduced at or before session N"*, so 5 writes a generation missing every secret added since. And `deploy.sh --through-session 5` renders a Session-5 `outputs.json`, dropping the agent, storage and backup blocks that `doctor.sh` and most current proofs read. | **11 in all three places**, and the correction is written into the operator scripts rather than left for the next reader of the guide. | **D505 and D507 were both flags lost to retyping the previous session's guide, and this is the same failure caught before it ran.** The guide is not wrong — it was right in Session 5 and says so; what is missing is that its session number is a *parameter*, not a constant. CLAUDE.md's rule that a guide is derived **by diff, not by retyping** exists for exactly this, and the diff is what found it. | — |
| **D679** | `docs/api-operations.md`: *"Three credentials, one at a time, each in a maintenance window."* | **Two of the three can share a window without weakening anything, and the guide's reason says so.** The sentence's stated purpose is that *"a single flag would admit all three proofs on the strength of whichever was actually rotated"* — a claim about the **declaration flags**, which are already separate and each of which carries its own false-declaration control. The authenticator lives in the `/database` provider path and is mounted by a container; the documentation password lives in `/runtime` and is written inline into the middleware document. They share no plane, no file and no failure. | **The authenticator and the documentation credential are rotated in one window**; the signing key keeps its own, because it is a four-phase cutover with a two-key ceiling and four verifiers to recreate. Recorded rather than done silently. | A deploy is the expensive step and it is identical for both, so combining halves the operator's time at the terminal for no loss of resolution: if the deploy leaves the documentation credential stale — D252's original failure — the docs proof fails and the authenticator proof passes, which is exactly the discrimination the separate flags were for. **Not silently reconciled** (CLAUDE.md §6): the guide and the plan disagreed, and this row is the disagreement. | — |
...
| **D684** | This plan's Run 6: migration 0022 records the request id on the `database`-source audit row, so the proof reads that row back and asserts a malformed header wrote `NULL`. | **There is no row to read, and the product is right.** 0022 writes it inside `IF acting_agent IS NOT NULL THEN`; the proof authenticates as `owner_session`, a **human owner** who logged in through `/auth/login` and has no agent identity. The `database` row exists to pair with the `agent_plane` row an MCP write leaves. The query matched nothing and the assertion read `''`. **The three assertions that matter all passed**: not 401, status < 400, and the note committed — so **D633 is proved on the deployment** and 0022's guard works. | **The assertion is inverted rather than deleted**: a non-agent caller must leave **zero** rows in `app_private.agent_audit`. The absence is worth guarding — it is what keeps that record about agents — and deleting the line would leave the next reader free to add the original back. The `NULL`-request-id half stays where it can be proved, offline, where the function can be called directly. | **Third wrong assumption in one test**, and the sequence is the whole lesson: first the token (D675), then the column premise, now the row's existence condition. Each was a belief about a neighbouring component held without reading it, and each survived until the deployment answered. The write path itself was correct every time. | — |
| **D685** | Session 10's `REC-*` requirements: the restore drill restores a point in time and asserts the restored instance matches the release — schema, RLS, and the migration ledger. | **A released migration makes every restore drill fail until a new full backup is taken, and nothing says so.** The drill exited 6 with *"restored 21 versions, the release declares 22; only in the release ['20260827120022']"*. The last full backup is **2026-08-26**; migration 0022 was released on **2026-08-27**. The restore is **faithful to its point in time** — the assertion is what assumes the backup postdates the release. | **Recorded, and the operator step named**: after releasing a migration, take a full backup *before* the drill. Not repaired in code, because the alternative — comparing against what the release declared at the restore's timestamp — is not knowable from the release, and loosening the check to a subset would be exactly the weakening CLAUDE.md §6 forbids. | **The drill is coupled to backup freshness and the coupling was invisible until a session released a migration between a backup and a gate.** Session 10 could not have found it: every migration it released predated its own first full backup. The failure reads like a restore defect and is a scheduling fact, which is the more dangerous shape — an operator seeing it at 3 a.m. would reasonably suspect the backup. | — |
