...
| [0192](0192-the-restore-precedes-the-first-deploy-and-reads-the-mirror-through-its-own-environment.md) | On a replacement host the restore precedes the first deploy, and a restore from the mirror carries the mirror's key in its own container's environment | 18 | Accepted |
...
| [0197](0197-the-outsiders-bring-up-settles-one-of-the-two-open-claims-and-answers-the-other-no.md) | The outsider's bring-up settles fresh_host and answers documented_path "no": the path required source edits | 19 | Accepted |
| [0198](0198-a-project-owns-a-migration-set-a-reviewed-surface-and-a-snapshot-beside-the-releases.md) | A project owns a migration set, a reviewed surface and a snapshot beside the release's, tracked in the checkout under `projects/<slug>/` | 20 | Accepted |
| [0199](0199-a-route-the-deploy-did-not-observe-is-unobserved-and-a-migrator-never-guesses.md) | A route the deploy did not observe is `unobserved`, and a migrator never guesses which `unavailable` that was | 20 | Accepted |
| [0200](0200-the-scope-vocabulary-is-derived-from-the-reviewed-surface-and-the-roster-is-compiled-not-enumerated.md) | The scope vocabulary's data class is derived from the reviewed surface, and the roster is compiled, not enumerated | 21 | Accepted |
| [0201](0201-a-project-owns-a-capability-manifest-and-a-compiled-contract-beside-its-surface.md) | A project owns a capability manifest and a compiled contract beside its surface, and the lock is the release's joined with the project's | 21 | Accepted |
| [0202](0202-an-offline-claim-is-declared-never-inferred-and-each-half-says-which-commit-it-measured.md) | An offline claim is declared, never inferred, and the evidence document says which commit each half measured | 22 | Accepted |
...
| [0207](0207-a-walk-is-by-a-reader-holding-only-the-release-and-the-task-and-its-record-is-read-by-the-product.md) | A walk is by a reader whose context holds only the release and the task, and the record it writes is read by the product | 25 | Accepted |
| [0208](0208-operator-documentation-is-one-current-guide-and-an-upgrade-is-the-operators-sequence.md) | Operator documentation is one current guide per release, superseding the per-session set; an upgrade is the operator's sequence, with the checkout's half inside it | 26 | Accepted |
| [0209](0209-a-release-is-held-to-its-own-documentation-by-a-test.md) | A release is held to its own documentation by a test, not by a habit | 27 | Accepted |
...
| [0226](0226-the-workflow-worker-is-a-loop-in-the-auth-process.md) | The workflow worker is a loop in the auth process, and it holds nothing the auth service did not hold | 32 | Accepted |
| [0227](0227-durable-step-state-is-four-tables-nobody-may-read.md) | Durable step state is four tables nobody may read and nine definer functions, and a step is at-least-once with a key that makes it exactly-once | 32 | Accepted |
| [0228](0228-a-workflow-definition-is-a-project-artefact-installed-by-the-deploy.md) | A workflow definition is a project artefact compiled against the lock and installed by the deploy, immutable per name and version | 32 | Accepted |
| [0229](0229-a-run-is-the-agents-enqueued-with-its-token-and-stopped-by-its-revocation.md) | A run is the agent's: enqueued with its token, authorised by its own scopes, minted per step, and stopped by its revocation | 32 | Accepted |
| [0230](0230-an-approval-is-a-parked-step-and-a-humans-recorded-decision.md) | An approval is a parked step and a human's recorded decision; a rejection is a cancel | 33 | Accepted |
| [0231](0231-an-approved-call-carries-a-signed-claim-naming-its-tool-and-key.md) | An approved call carries a signed claim naming its tool and its key, and approval is a control on the governed path | 33 | Accepted |
| [0232](0232-approving-is-an-administrative-scope-and-the-owner-may-not-approve.md) | Approving is an administrative scope, and the owner of a run may not approve it | 33 | Accepted |
| [0233](0233-compensation-is-reverse-ordered-forward-steps-and-a-wait-is-a-park.md) | Compensation is reverse-ordered forward steps appended when a run fails or is cancelled; a wait is a park with a time | 33 | Accepted |
| [0234](0234-provenance-is-joined-by-request-id-and-the-audit-reader-pages-by-keyset.md) | Provenance is one definer function joined by the plane's request id across every attempt, read by an auditor; the audit reader pages with a keyset cursor and reports the window it filtered | 33 | Accepted |
