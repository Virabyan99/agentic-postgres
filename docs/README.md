...
| [Converting a pre-`projects/` fork](on-ramp.md) | An adopter whose domain is inside the release's own files, because `projects/<slug>/` did not exist when they forked: whether their set converts at all, the merge's four classes, which bytes may be edited after they have run and which become a new migration, the seven steps, and what to do when it does not convert |
| [Everything this project knows is wrong with itself](pre-stage-4-audit.md) | The pre-Stage-4 inventory, dated 2026-09-16: the seven claims that are not `passed`, and every open defect sorted by **what kind of act closes it** — code, a host trip, or a person. Drawn from the evidence, `scope-closure.md`, the session plans and an adopter's findings |
...
| [Workflows](workflows.md) | `apg workflow`: a reviewed sequence over your own capabilities, the six verbs, what a run's status document says, what `parked`, `stopped` and `replayed` mean, what the worker is and what a run may never do |
