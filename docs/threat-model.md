...
- Availability under a neighbour's load. `THR-NOISY-NEIGHBOUR` is in the table
  ...
  beside what is already here. What nobody has measured is what a neighbour
  actually feels when one of those ceilings is reached, and disk I/O has no
  ceiling at all -- there is no `blkio` limit and the two projects share one
  device. The measurement is Session 35's.
