"""The Session 4 gate has three modes, and the third one's return is a decision.

``bin/session-04-check.sh`` takes the shape the Session 2 and Session 3 gates
...
SCRIPT = REPO_ROOT / "bin" / "session-04-check.sh"
SESSION_THREE = REPO_ROOT / "bin" / "session-03-check.sh"
...
def test_the_gate_exists_and_is_executable_in_the_git_index() -> None:
    ...
    result = subprocess.run(
        ["git", "ls-files", "--stage", "--", "bin/session-04-check.sh"],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
        check=True,
...
def test_a_rotation_file_that_does_not_exist_is_refused_before_root(outputs: str) -> None:
    """The one input whose absence would otherwise be discovered as a skip.

    ``--rotated-from-file`` admits a proof that is unrunnable without it. A path
    typo would leave the proof skipped and the operator believing the rotation
    had been measured, so a named file that is not there is an argument error.
    """
    result = run(
        "--mode", "host",
        "--host", str(HOST_EXAMPLE),
        "--project-a-outputs", outputs,
        "--project-b-outputs", outputs,
        "--rotated-from-file", "/nonexistent-credential",
    )  # fmt: skip
    assert result.returncode == 2
...
def test_the_gate_resolves_claims_for_its_own_session(source: str) -> None:
    ...
    """
    assert "readonly SESSION=4" in source
    assert "static_nodeids_for_mode(sys.argv[1], int(sys.argv[2]))" in source
    assert 'claim_static_nodeids "${mode}" "${SESSION}"' in source


def test_the_session_three_gate_still_names_session_three() -> None:
    """Guard the guard: a fourth gate must not have taken the third one's."""
    assert "readonly SESSION=3" in SESSION_THREE.read_text(encoding="utf-8")
...
def test_the_gate_tells_the_operator_a_half_is_not_the_document(source: str) -> None:
    ...
    assert "--external-input evidence/session-04-external.json" in run("--help").stdout
