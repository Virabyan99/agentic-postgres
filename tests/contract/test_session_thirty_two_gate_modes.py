"""The Session 32 gate: a substrate, a loop, three routes and their readers.

**Derived from `test_session_thirty_one_gate_modes.py` by diff**, and what is
...
**THE PREVIOUS GATE IS 31'S, AND THERE IS NO GAP** -- the second consecutive
pair in a row, after 30 -> 31. D1627's test is kept with its literal moved,
and `SESSION_PREVIOUS_NUMBER` stays a literal for the reason it always has.

**EIGHT claims are declared offline, two MORE than any session has
declared**, and each is a property of a checkout: a grouping over an inspect
payload this suite builds; a schema and a compiler over a committed lock;
migration 0034 under a cluster this workstation starts; the loop over fakes
that record; the auth app with fakes behind it; a command's closed tables; an
AST scan of the deploy plus `apg dev up`; and three readers over canned inputs
with a rehearsal driven against a rig.

**FIVE are host claims**, and none of their twelve proofs has ever executed.
One of them found a defect by being WRITTEN (D1696): the correlation join could
not have returned a row, because the plane mints its own request id and
ignores the one the loop sent -- and the loop's end-to-end rig ran against a
fake plane that believed what the loop believed.
...
The source-reading tests read `bin/session-32-check.sh` as text, which is the
...
SCRIPT = REPO_ROOT / "bin" / "session-32-check.sh"
SESSION_PREVIOUS = REPO_ROOT / "bin" / "session-31-check.sh"
...
SESSION_PREVIOUS_NUMBER = 31

#: The claims Session 32 introduced, by mode. Written out rather than derived
...
#:
#: **The split across the two modes is the assertion.** The eight offline
#: claims are each settled by files in this checkout and by rigs this
#: workstation stands up -- a pure grouping, a compiler over a committed lock,
#: a migration under a disposable cluster, a loop over recording fakes, an app
#: over fakes, a command's tables, an AST scan and `apg dev up`, and readers
#: over canned inputs. The five host claims are each about a RUNNING loop, a
#: RUNNING plane or a RESTORED drill: a run executed as an agent against the
#: deployed `mcp` container, a replay counted in the deployed idempotency
#: table, a process killed under a parked run, a revocation landing between
#: two steps, and runs carried by a restore. D1696 is why the line is here and
#: not one proof over: the only defect this session found before its trip was
#: invisible to every offline proof, because the plane they used was a fake.
#:
#: `external` is absent and that is also an assertion: the three new routes
#: hang off the application route that already exists and are proved from the
#: host. Inventing an external claim to make the shape symmetric is what ADR
#: 0065 refuses.
SESSION_THIRTY_TWO_CLAIMS = {
    "offline": (
        "capacity_ceilings",
        "workflow_definition",
        "workflow_substrate",
...
def test_the_previous_gate_is_the_one_before_this_one_with_no_gap() -> None:
    """**D1627, carried with its literal moved: 31 -> 32 is consecutive.**

    Session 30's copy of this module asserted that 29 has no gate, and Session
    31's replaced that with this test, because 30 -> 31 was the first
    consecutive pair since 24 -> 25 -- Sessions 26, 27 and 29 each registered
    nothing (D1063, D1514). Session 31 swept its OWN trip too, so 31 -> 32 is
    the second consecutive pair and there is again no gap to explain.
...
def test_each_environment_carries_a_claim_this_session_introduced() -> None:
    ...
    checking itself.
    """
    for mode, expected in SESSION_THIRTY_TWO_CLAIMS.items():
        resolved = set(claims.claims_for_mode(mode, SESSION))
        missing = sorted(set(expected) - resolved)
        assert not missing, (
            f"{mode} mode does not carry Session {SESSION}'s own claims {missing}. The "
...
def test_the_expectation_table_names_every_claim_this_session_introduced() -> None:
    ...
    """
    declared = {claim for group in SESSION_THIRTY_TWO_CLAIMS.values() for claim in group}
...
def test_every_session_thirty_claim_belongs_to_session_thirty() -> None:
    ...
    """
    for expected in SESSION_THIRTY_TWO_CLAIMS.values():
...
def test_exactly_this_sessions_declared_claims_are_offline() -> None:
    ...
    **The name carries no count, deliberately.** The derivation arrived here
    ...
    assert mine == set(SESSION_THIRTY_TWO_CLAIMS["offline"]), (
        "the offline claims this session introduced and the ones it declared "
        f"disagree: {sorted(mine ^ set(SESSION_THIRTY_TWO_CLAIMS['offline']))}"
    )
    ...
    # session's: each of its five host claims is about a RUNNING loop against
    # a RUNNING plane, and D1696 is what a checkout declared able to answer one
    # of them would have reported -- a correlation that was green against a
    # fake and could not have held on a deployment.
    for claim in SESSION_THIRTY_TWO_CLAIMS["host"]:
        assert claim not in claims.OFFLINE_CLAIMS, (
            f"{claim} is declared offline. It is about a running loop against a "
            "running plane, and a checkout answering it would grade the release "
            "against a fake that shares the loop's beliefs (D1696)"
        )
