"""The Session 30 gate: one exec discipline, a ref, an output path, two shapes.

**Derived from `test_session_twenty_eight_gate_modes.py` by diff**, and what is
...
**THE PREVIOUS GATE IS 28'S, AND 29 HAS NO GATE FOR A NEW REASON.** 19, 26 and
27 registered nothing because they built nothing. Session 29 took THE TRIP: it
deployed 1.7.0, swept it and tagged it on the commit that was deployed (D1425).
A trip writes evidence for the session whose release it deploys, not for
itself, so it has no half of its own and no gate to write one -- and the number
goes 28 -> 30 (D1063, D1514). `SESSION_PREVIOUS_NUMBER` therefore stays the
literal it became in Session 28's module, and the arithmetic `SESSION - 1`
would now look for `bin/session-29-check.sh`, which does not exist.
...
**Its proof has never executed anywhere.** It belonged to no requirement and
errored at setup on every sweep since Session 24, while three Studio claims
read `passed` -- which is what a proof that is a node id of no claim costs
(D386, D1236). This session registers it, and the trip is its first execution.
...
The source-reading tests read `bin/session-30-check.sh` as text, which is the
...
SCRIPT = REPO_ROOT / "bin" / "session-30-check.sh"
SESSION_PREVIOUS = REPO_ROOT / "bin" / "session-28-check.sh"
...
SESSION_PREVIOUS_NUMBER = 28

#: The claims Session 30 introduced, by mode. Written out rather than derived
...
def test_no_gate_exists_for_the_session_that_took_the_trip() -> None:
    """**The gap is the record** (D1063), so it is asserted rather than assumed.

    **One session here, not two, and the reason is not the earlier one.** 26 and
    27 have no gate because they built nothing, and Session 28's copy of this
    test asserts that where it belongs. Session 29 built nothing either, but it
    was not idle: it deployed 1.7.0, swept it and cut the tag on the commit that
    was deployed. A trip writes the evidence of the session whose RELEASE it
    deploys -- 29's sweep wrote `evidence/session-28-host.json` -- so it has no
    half of its own, and a gate for it would be a gate with nothing to report.

    Its existence is also the thing that would make `SESSION - 1` look right
    again to the next derivation, quietly, in a file nobody re-reads.

    The anti-vacuity half is the second assertion: the gate this session WAS
    derived from has to be there, or this test passes on a checkout with no
    gates at all.
    """
    absent = REPO_ROOT / "bin" / "session-29-check.sh"
    assert not absent.exists(), (
        f"{absent.name} exists. Session 29 took the trip that deployed, swept and "
        "tagged 1.7.0 and registered no requirement of its own, so it has no half to "
        "write and no gate to write it"
    )
...
def test_each_environment_carries_a_claim_this_session_introduced() -> None:
    ...
    checking itself.
    """
    for mode, expected in SESSION_THIRTY_CLAIMS.items():
...
def test_the_expectation_table_names_every_claim_this_session_introduced() -> None:
    ...
    """
    declared = {claim for group in SESSION_THIRTY_CLAIMS.values() for claim in group}
...
def test_every_session_thirty_claim_belongs_to_session_thirty() -> None:
    ...
    """
    for expected in SESSION_THIRTY_CLAIMS.values():
...
def test_exactly_this_sessions_declared_claims_are_offline() -> None:
    ...
    **The name carries no count, deliberately.** The derivation arrived here
    ...
    assert mine == set(SESSION_THIRTY_CLAIMS["offline"]), (
        "the offline claims this session introduced and the ones it declared "
        f"disagree: {sorted(mine ^ set(SESSION_THIRTY_CLAIMS['offline']))}"
    )
    ...
    for claim in SESSION_THIRTY_CLAIMS["host"]:
