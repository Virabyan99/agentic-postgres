"""The Session 28 gate: an audit acted on, the record's retention, the reading.

**Derived from `test_session_twenty_five_gate_modes.py` by diff**, and what is
...
**THE PREVIOUS GATE IS 25'S AND THE PREVIOUS SESSION NUMBER IS NOT 27.**
Sessions 26 and 27 registered no requirement and wrote no evidence document, so
neither has a gate and neither ever will; the number goes 25 -> 28 the way it
went 18 -> 20 (D1063). Every inherited assertion that read `SESSION - 1` now
reads `SESSION_PREVIOUS_NUMBER`, because the arithmetic was true for six
consecutive sessions and stopped being true here -- D719's class, in the guard
written to catch the derivation losing something.
...
deriving it. Two claims are declared offline, one fewer than Session 25: a
project set's record of the release it was frozen against is a property of a
lock file in a checkout, and the reading before a tag is a property of a clone
and of `git`. `agent_record_retention` is not, because a checkout cannot say
what two tables written to by nine sessions of trips are carrying nor that a
prune removes a row of that record, and folding it in would prove a retention
claim against a table somebody arranged (D940).

**The host mode cannot be run at this commit and that is the design** (D1425).
Migration `20260917120033` is in this release's set and nothing applies it
here; Session 29 deploys this commit and sweeps after. A gate whose host mode
was runnable the moment the bump landed would be a gate that tagged before it
deployed, which is the class that cost this project two patch releases.

The source-reading tests read `bin/session-28-check.sh` as text, which is the
...
SCRIPT = REPO_ROOT / "bin" / "session-28-check.sh"
SESSION_PREVIOUS = REPO_ROOT / "bin" / "session-25-check.sh"

SESSION = 28

#: The session the gate above was DERIVED FROM, which is not `SESSION - 1`.
#:
#: **Written out because the arithmetic stopped being true here.** Sessions 20
#: through 25 each derived from the one before, so `SESSION - 1` named the
#: right file for six consecutive derivations and read as a rule. 26 and 27
#: registered no requirement and have no gate, so this session derives from
#: 25's -- and every inherited assertion that computed the previous session
#: would have looked for `bin/session-27-check.sh`, which does not exist, and
#: for `readonly SESSION=27`, which nothing holds. A literal that was right
#: until the world gained a case (D719).
SESSION_PREVIOUS_NUMBER = 25

#: The claims Session 28 introduced, by mode. Written out rather than derived
...
#: the session ran. `project_set_release_record` is what a project's lock FILE
#: records -- whether the release it was frozen against was computed or
#: declared -- and what `bin/migrate.sh` refuses when the flag that declares it
#: is put somewhere it cannot mean anything. No deployment holds a project
#: lock, so there is nothing a running system would answer differently.
#: `release_reading` is a command that reads `git` in a clone and prints four
#: outcomes; its subject IS the checkout, and the one environment that would
#: answer it differently is CI, which has no tags (D1466) and therefore gets
#: the third outcome.
#:
#: `agent_record_retention` is about a DEPLOYMENT: what two tables that nine
#: sessions of trips have written to are carrying, and whether a prune removes
#: a row of that record and leaves the rest. The offline half seeds its own
#: history; the live half reads history nobody arranged, which is the
#: difference D940 names. A later session that moved it into the first group
...
#: refuses.
SESSION_TWENTY_EIGHT_CLAIMS = {
    "offline": ("project_set_release_record", "release_reading"),
    "host": ("agent_record_retention",),
}
...
def test_no_gate_exists_for_the_two_sessions_that_registered_nothing() -> None:
    """**The gap is the record** (D1063), so it is asserted rather than assumed.

    Sessions 26 and 27 built no plane, registered no requirement and wrote no
    evidence document. A gate for either would be a gate with no half to write
    -- and its existence is the thing that would make `SESSION - 1` look right
    again to the next derivation, quietly, in a file nobody re-reads.

    The anti-vacuity half is the third assertion: the gate this session WAS
    derived from has to be there, or this test passes on a checkout with no
    gates at all.
    """
    for absent in (26, 27):
        path = REPO_ROOT / "bin" / f"session-{absent}-check.sh"
        assert not path.exists(), (
            f"{path.name} exists. Session {absent} registered no requirement and wrote "
            "no evidence document, so it has no half to write and no gate to write it"
        )
...
def test_each_environment_carries_a_claim_this_session_introduced() -> None:
    ...
    checking itself.
    """
    for mode, expected in SESSION_TWENTY_EIGHT_CLAIMS.items():
        resolved = set(claims.claims_for_mode(mode, SESSION))
        missing = sorted(set(expected) - resolved)
        assert not missing, (
            f"{mode} mode does not carry Session 28's own claims {missing}. The "
...
def test_the_expectation_table_names_every_claim_this_session_introduced() -> None:
    ...
    """
    declared = {claim for group in SESSION_TWENTY_EIGHT_CLAIMS.values() for claim in group}
...
def test_every_session_twenty_eight_claim_belongs_to_session_twenty_eight() -> None:
    ...
    Session 28 claim either drags it into an earlier gate's evidence -- turning
    ...
    """
    for expected in SESSION_TWENTY_EIGHT_CLAIMS.values():
...
def test_exactly_this_sessions_declared_claims_are_offline() -> None:
    ...
    **Session 22's version of this test asserted an EQUALITY against the whole
    ...
    (D1281). This is the fifth offline session; it declares two, which is one
    FEWER than the session before -- and a guard shaped around the count going
    up would have been the fourth version of this same mistake.
    ...
    **The name carries no count, deliberately.** The derivation arrived here
    ...
    assert mine == set(SESSION_TWENTY_EIGHT_CLAIMS["offline"]), (
        "the offline claims this session introduced and the ones it declared "
        f"disagree: {sorted(mine ^ set(SESSION_TWENTY_EIGHT_CLAIMS['offline']))}"
    )
    ...
    # session's: `agent_record_retention` is about what a deployment's two agent
    # tables are carrying and whether a prune removes a row of that record, and
    # a checkout declared able to answer it would be proving a retention claim
    # against history the proof itself seeded (D940).
    for claim in SESSION_TWENTY_EIGHT_CLAIMS["host"]:
        assert claim not in claims.OFFLINE_CLAIMS, (
            f"{claim} is declared offline. It is about a running deployment -- what two "
            "tables nine sessions of trips have written to are carrying -- and a "
            "checkout answering it would grade the release against history it seeded "
            "itself"
        )
