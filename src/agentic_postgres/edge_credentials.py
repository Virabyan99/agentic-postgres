...
#: The user in front of the metrics route. Its own, not `docs`.
#:
#: Two credentials, two users. Sharing the username would make one rotation
#: look like the other's in every log and every operator instruction, and the
#: two secrets rotate independently -- `docs_basic_auth_password` and
#: `metrics_basic_auth_password` are separate declarations with separate
#: generations. A shared name is the sort of thing that reads as tidy until
#: somebody has to say which one they changed.
...
def middleware_document(
    ...
    **The hash is inline** (ADR 0086, D252). It was a `usersFile` naming a
    ...
    if not metrics_middleware_name:
        raise ManifestError("metrics_middleware_name is required")
    if metrics_middleware_name == middleware_name:
        raise ManifestError(
            f"the documentation and metrics middlewares share the name "
            f"{middleware_name!r}; one would silently replace the other in the "
            "document Traefik parses, and the survivor would guard both routes "
            "with one credential"
        )
    return {
        "http": {
            "middlewares": {
                ...
                # Session 14. **The metrics router has referenced this name since
                # Run 2 and nothing defined it** -- D389/D395's shape a third
                # time, one half of a pair written.
                #
                # Measured, with both controls live: Traefik logs `middleware
                # "..." does not exist`, refuses the router, and the route
                # answers 404. It fails CLOSED, so the surface was never served
                # unprotected. The bad half is the symptom: a 404 is exactly
                # what D768 says must never be read as "metrics are not
                # configured", and Traefik's own 404 is indistinguishable from a
                # routed one without the access log (D186, D187, D353).
                metrics_middleware_name: {
                    "basicAuth": {
                        "users": [htpasswd_entry(metrics_hashed, user=METRICS_USER)],
                        # The collector must hold no credential of any kind
                        # (ADR 0164), and without this the `Authorization`
                        # header travels to it -- the same clause, and the same
                        # reason, as the documentation service above.
                        "removeHeader": True,
                        "realm": f"{project_key} metrics",
                    }
                },
            }
        }
