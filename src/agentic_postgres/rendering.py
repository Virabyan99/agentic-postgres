...
def build_otel_config(router_names: tuple[str, ...], domain: str, project_key: str) -> bytes:
    """The metrics collector's configuration (ADR 0164, ADR 0167).

    **It took no project until Session 14's Run 4, and now there are two
    reversals.** The first is the scrape filter: the OTLP endpoints and the exporter's
    ...
    **The scrape target holds no credential.** Traefik's exposition endpoint is
    ...
    address.
    """
    # The project key is interpolated into the exporter's `const_labels`, so
    # it is asserted rather than trusted -- the same footing the router names
    # are on inside `_scrape_keep_regex`. A key holding a `:` or a newline
    # would not be a label value; it would be a second YAML key in the
    # exporter's block.
    if not naming.TRAEFIK_NAME_PATTERN.match(project_key):
        raise ValueError(
            f"project key {project_key!r} is not a Traefik name and may not be "
            "interpolated into the exporter's const_labels"
        )
    ...
    lines = [
        ...
        "    # Session 31, the second reversal (ADR 0223). EVERY series this",
        "    # collector exports carries the project it belongs to -- the ones",
        "    # the runtime pushes over OTLP and the ones scraped off the SHARED",
        "    # edge alike. The store scrapes this surface, so `doctor usage` can",
        "    # assert that every series it gets back names the project it asked",
        "    # about, which is the difference between reading this project's",
        "    # store and reading a store.",
        "    #",
        "    # Measured in rig 31c against otelcol-contrib 0.159.0, with a",
        "    # control that omitted the option and carried nothing: six of the",
        "    # seven series carry it, including `up` and every `scrape_*`.",
        "    # **`target_info` does not** -- the exporter synthesises it from the",
        "    # resource and const_labels are not applied to it (D1604). The",
        "    # contract test names that exception rather than folding it away.",
        "    #",
        "    # A scrape target that already emitted a `project` label would have",
        "    # its series DROPPED here, with `duplicate label names in constant",
        "    # and variable labels` in this collector's log and nothing on the",
        "    # exposition surface to show for it (D1605). Traefik emits no such",
        "    # label, and the keep-filter above bounds what arrives at all.",
        "    const_labels:",
    ...
    with project_lock(identity.key):
        ...
        try:
            ...
            write_readable(
                ...
                build_otel_config(
                    naming.project_router_names(identity.key),
                    identity.domain,
                    identity.key,
                ),
            )
