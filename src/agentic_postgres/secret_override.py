...
def build_secret_override(
    *,
    ...
    ``facilities`` is what the project has (ADR 0188), from
    `secrets_contract.enabled_facilities` over its rendered document: a
    facility-gated secret is mounted only for a project with that facility,
    because the materializer wrote it only then and Compose refuses a mount
    whose source is missing. None is the declared view, for proofs of the
    override's shape that are not about one project.
    """
    if not project_key:
